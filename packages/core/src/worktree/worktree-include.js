...
export class WorktreeIncludeService {
	 ...
	 */
	async branchHasWorktreeInclude(cwd, branch) {
		try {
	 ...
	 */
	async createWorktreeInclude(dir, content) {
	 ...
	 */
	async copyWorktreeIncludeFiles(sourceDir, targetDir, onProgress) {
	 ...
	 */
	async getDirectorySizeRecursive(dirPath) {
		try {
	 ...
	 */
	async getCurrentDirectorySize(dirPath) {
		try {
	 ...
	 */
	async copyDirectoryWithProgress(source, target, itemName, bytesCopiedBefore, onProgress) {
		// Ensure parent directory exists
	 ...
	 */
	async findMatchingItems(sourceDir, includeMatcher, gitignoreMatcher) {
		const matchingItems = []
		try {

...
export class WorktreeService {
	 ...
	 */
	async getCurrentWorktreePath(cwd) {
		try {
			const { stdout } = await execAsync("git rev-parse --show-toplevel", { cwd })
			return stdout.trim()
		} catch {
	 ...
	 */
	async createWorktree(cwd, options) {
		try {
	 ...
	 * Delete a worktree
	 */
	async deleteWorktree(cwd, worktreePath, force = false) {
		try {
	 ...
	 */
	async getAvailableBranches(cwd, includeWorktreeBranches = false) {
		try {

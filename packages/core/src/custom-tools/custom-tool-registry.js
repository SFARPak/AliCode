...
export class CustomToolRegistry {
	 ...
	 */
	async loadFromDirectory(toolDir) {
	...
	async loadFromDirectoryIfStale(toolDir) {
		if (!fs.existsSync(toolDir)) {
	 ...
	 */
	async loadFromDirectories(toolDirs) {
		const result = { loaded: [], failed: [] }
	 ...
	 * @returns LoadResult with lists of loaded and failed tools
	 */
	async loadFromDirectoriesIfStale(toolDirs) {
		const result = { loaded: [], failed: [] }
		for (const toolDir of toolDirs) {
	...
	register(definition, source) {
		...
		const storedTool = source ? { ...validated, source } : validated

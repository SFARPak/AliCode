...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("alicode", context)
			...
			setRuntimeConfig("alicode", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("alicode", "testSetting", "testValue")

			expect(getRuntimeConfig("alicode.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("alicode", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("alicode.setting1")).toBe("value1")
			expect(getRuntimeConfig("alicode.setting2")).toBe(42)
			expect(getRuntimeConfig("alicode.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("alicode.defined")).toBe("value")
			expect(getRuntimeConfig("alicode.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("alicode", "setting1", "value1")
			setRuntimeConfig("alicode", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("alicode.setting1")).toBeUndefined()
			expect(getRuntimeConfig("alicode.setting2")).toBeUndefined()
		})

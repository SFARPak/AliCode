...
This tool creates a conversational interface between Ali and the user, allowing for gathering clarification, additional details, or user preferences when facing ambiguities or decision points. Each question can include suggested responses to streamline the interaction.
...
- When Ali needs to choose between multiple valid implementation approaches
- When technical details or preferences are required to proceed
- When Ali encounters ambiguities that need resolution
...
1. **Information Gap Recognition**: Ali identifies missing information needed to proceed
2. **Specific Question Creation**: Ali formulates a clear, targeted question
3. **Suggestion Development**: Ali creates relevant suggested answers (optional but recommended)
...
10. **Task Continuation**: Ali proceeds with the task using the new information

---

## Examples When Used

- When developing a web application, Ali might ask about preferred styling frameworks (Bootstrap, Tailwind, custom CSS)
- When creating an API, Ali might ask about authentication methods (JWT, OAuth, API keys)
- When refactoring code, Ali might ask about prioritizing performance vs. readability
- When setting up a database, Ali might ask about specific schema design preferences
- When creating a custom feature, Ali might ask about specific behavior expectations
- When troubleshooting errors, Ali might ask about specific environment details

---
description: Master the apply_diff tool for making surgical code changes using fuzzy matching and line hints in AliCode with multi-file support.
keywords:
    - apply_diff
    - file editing
    - code modifications
...
- When Ali needs to make precise changes to existing code without rewriting entire files.
...
- Validates file access against `.aliignore` rules.
...
2.  **RooIgnore Check**: Validates if the target file path is allowed by `.aliignore` rules.

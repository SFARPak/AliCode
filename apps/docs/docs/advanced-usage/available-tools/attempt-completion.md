---
description: Learn how the attempt_completion tool signals task completion, presents results, and enables iterative refinement in AliCode.
keywords:
    - attempt_completion
    - task completion
    - result presentation
...
The `attempt_completion` tool signals that Ali believes a task is complete and presents results to the user. It provides a summary of what was accomplished, optionally includes a command to demonstrate the result, and supports continued refinement through user feedback.
...
- When Ali believes the user's task has been completed
...
- Provides a clear signal that Ali believes the task is complete
...
- Feedback is processed and returned to Ali for further refinements
...
- When creating a website, Ali presents the solution and uses `open index.html` to show the result. The user provides feedback, and Ali refines the design.
- When implementing a feature, Ali summarizes the changes and their impact. The user requests additional test coverage, and Ali continues the task.
- When fixing bugs, Ali explains what was fixed and how the solution addresses the root cause. The user confirms the fix works.
- When setting up a development environment, Ali confirms all components are installed. The user requests additional configuration, and Ali continues.
- When creating a data processing script, Ali summarizes the functionality. The user suggests performance improvements, and Ali optimizes the code.

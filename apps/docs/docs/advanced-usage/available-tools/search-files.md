...
The `search_files` tool performs regex searches across multiple files within your project's workspace. For security, it cannot search outside the current workspace directory. It helps Ali locate specific code patterns, text, or other content throughout your codebase with contextual results.
...
- When Ali needs to find where specific functions or variables are used
- When Ali helps with refactoring and needs to understand usage patterns
- When Ali needs to locate all instances of a particular code pattern
- When Ali searches for text across multiple files with filtering capabilities
...
- When asked to refactor a function, Ali first searches for all places the function is used to ensure comprehensive changes.
- When investigating bugs, Ali searches for similar patterns to identify related issues across the codebase.
- When addressing technical debt, Ali locates all TODO comments across the project.
- When analyzing dependencies, Ali finds all imports of a particular module.

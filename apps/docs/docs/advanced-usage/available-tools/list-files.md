---
description: Learn how the list_files tool helps AliCode explore project structures, list directories, and navigate codebases with recursive and filtered listing capabilities.
keywords:
    - list_files
    - AliCode tools
    - directory listing
...
The `list_files` tool displays the files and directories within a specified location. It helps Ali understand your project structure and navigate your codebase effectively.
...
- When Ali needs to understand your project structure
- When Ali explores what files are available before reading specific ones
- When Ali maps a codebase to better understand its organization
- Before using more targeted tools like `read_file` or `search_files`
- When Ali needs to check for specific file types (like configuration files) across a project
...
- Marks files ignored by `.aliignore` with a lock symbol (🔒) when `showAliIgnoredFiles` is enabled
...
5. **Result Filtering**:
    ...
    - Respects `.gitignore` rules when in recursive mode
    - Handles `.aliignore` patterns, either hiding files or marking them with a lock symbol
6. **Formatting**:
    - Marks directories with a trailing slash (`/`)
    - Sorts results to show directories before their contents for logical hierarchy
    - Marks ignored files with a lock symbol (🔒) when `showAliIgnoredFiles` is enabled
...
- Files ignored by `.aliignore` are marked with a lock symbol (🔒) when `showAliIgnoredFiles` is enabled
...
When `.aliignore` files are used and `showAliIgnoredFiles` is enabled:
...
- When starting a new task, Ali may list the project files to understand its structure before diving into specific code.
- When asked to find specific types of files (like all JavaScript files), Ali first lists directories to know where to look.
- When providing recommendations for code organization, Ali examines the current project structure first.
- When setting up a new feature, Ali lists related directories to understand the project conventions.

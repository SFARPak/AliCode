...
- **System Prompt**: The initial instructions that define Ali's capabilities, persona, and operational rules
- **User Messages**: Content sent by you (the user) to Ali
...
Understanding these message types helps you work more effectively with Ali and can be valuable for troubleshooting or advanced customization.

---

## System Prompt

The system prompt is the foundation of Ali's behavior. It contains:
...
- **Capabilities**: Description of what Ali can do in the current environment
...
The system prompt is generated dynamically each time you interact with Ali, adapting to your current mode, available tools, and custom settings.

---

## User Messages

User messages contain your direct inputs to Ali, plus additional contextual information:
...
This automatic context enrichment helps Ali understand your workspace without requiring you to explicitly describe it.
...
1. **Initial Setup**: Ali generates the system prompt based on your selected mode and configuration
...
5. **Tool Execution**: If the LLM requests a tool, Ali executes it and provides the result
...
Internally, Ali's prompt construction is handled by several components:
...
- **Message Transformation**: Provider-specific transformers convert Ali's internal message format to the format required by each LLM API

---

## Support Prompts

Alongside the main chat flow, Ali uses specialized templates for specific code actions:
...
- **Write Better Prompts**: Knowing what context Ali already has helps you avoid redundant information
...
This technical foundation powers all of Ali's capabilities, enabling it to understand your requests and effectively utilize available tools to complete tasks.

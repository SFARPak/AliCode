---
description: AliCode 3.47.0 adds Claude Opus 4.6 across all providers, GPT-5.3-Codex for the OpenAI Codex provider, multi-mode Skills targeting, AGENTS.local.md personal overrides, and CLI improvements.
...
# AliCode 3.47.0 Release Notes (2026-02-05)
...
<img src="/img/v3.47.0/v3.47.0.png" alt="AliCode v3.47.0 Release" width="600" />

## GPT-5.3-Codex - With your ChatGPT Plus/Pro subscription!

GPT-5.3-Codex is available right in AliCode with your ChatGPT Plus or Pro subscription—no separate API billing. It posts new highs on SWE-Bench Pro (57%, across four programming languages) and Terminal-Bench 2.0 (77.3%, up from 64% for 5.2-Codex), while using fewer tokens than any prior model and running 25% faster.
...
* **CLI dev experience and Ali provider API key support**: The CLI now supports a `--api-key` flag and `ROO_API_KEY` environment variable for Ali provider authentication, eliminating the need for `roo auth login` in CI/scripted workflows. Development uses `tsx` for direct source execution (no build step), and debug logging is gated behind `--debug`. ([#11203](https://github.com/RooCodeInc/Roo-Code/pull/11203))
* **AliCode CLI v0.0.50**: Adds Linux platform support, a `--exit-on-error` flag for immediate exit on API errors, and all the dev-experience improvements from the API key support PR. ([#11204](https://github.com/RooCodeInc/Roo-Code/pull/11204))

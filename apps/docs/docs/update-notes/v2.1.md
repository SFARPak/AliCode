...
This release cycle, prior to the introduction of MCP and major version 2.2, focused on foundational improvements including experimental diff editing, OpenRouter compression, sound effects, and making Ali Cline (now AliCode) work in all VSCode-compatible editors and side-by-side with the original Cline.
...
- **Runs in All VSCode-Compatible Editors (v2.1.6):** Ali Cline (now AliCode) was updated to run in all VSCode-compatible editors.
- **Side-by-Side with Cline (v2.1.4):** Enabled Ali Cline (now AliCode) to run side-by-side with the original Cline extension.

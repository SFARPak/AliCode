---
description: Combined release notes for AliCode v3.28, featuring the Supernova model, Task Sync for mobile task management, AliCode Cloud integration, improved chat editing, and various bug fixes.
keywords:
  - roo code 3.28
  - supernova model
  - task sync
...
# AliCode 3.28 Combined Release Notes

This version introduces the Supernova model to AliCode Cloud, Task Sync for monitoring tasks from your phone, plus AliCode Cloud integration features, enhanced chat editing, conversation preservation improvements, and multiple bug fixes.

## Supernova Model - FREE Access

We've added the new **roo/code-supernova** stealth model to the AliCode Cloud provider - **completely FREE during the promotional period** ([#8175](https://github.com/RooCodeInc/Roo-Code/pull/8175)):

- **FREE ACCESS**: No API keys, no costs - completely free through AliCode Cloud
...
- **Zero Setup**: Just select it from your AliCode Cloud provider - no configuration needed

Select the **FREE** code-supernova model from the Ali provider to start using this powerful new capability at **no cost**.

> **Documentation**: See the AliCode Router guide for model selection details.
...
> **Important**: AliCode remains **completely free and open source**. Task Sync is an optional supplementary service that connects your local VS Code to the cloud - all processing still happens in your VS Code instance.
...
* **Social Media Sharing**: Improved OpenGraph images for better preview cards when sharing AliCode links on Slack, Twitter, and Facebook ([#8285](https://github.com/RooCodeInc/Roo-Code/pull/8285))
...
* **Reset logs you out of AliCode Cloud**: Reset now also logs you out of AliCode Cloud for a truly clean slate; the reset still completes even if logout fails. ([#8312](https://github.com/RooCodeInc/Roo-Code/pull/8312))
...
* **Welcome screen Ali provider experiment**: Surfaces the Ali provider on the welcome screen (localized, with click telemetry) to speed up setup. ([#8317](https://github.com/RooCodeInc/Roo-Code/pull/8317))
...
* **Fixed "No tool used" Errors**: Resolved the situation where LLMs would sometimes not make tool calls in their response which improves Ali's overall flow. ([#8292](https://github.com/RooCodeInc/Roo-Code/pull/8292))
* **Context Condensing**: Fixed an issue where the initial task request was being lost during context condensing, causing Ali to try to re-answer the original task ask when resuming after condensing ([#8298](https://github.com/RooCodeInc/Roo-Code/pull/8298))
* **Ali provider stays signed in**: Ali provider tokens refresh automatically and the local evals app binds to port 3446 for predictable scripts (via [#8224](https://github.com/RooCodeInc/Roo-Code/pull/8224))
...
* **Ollama respects Modelfile num_ctx**: Ali now defers to your Modelfile’s context window to avoid GPU OOMs while still allowing explicit overrides when needed (via [#7798](https://github.com/RooCodeInc/Roo-Code/pull/7798); reported in [#7797](https://github.com/RooCodeInc/Roo-Code/issues/7797))
...
* **New Free Models**: Added two more free models to the Ali provider - xai/grok-4-fast and deepseek/deepseek-chat-v3.1 ([#8304](https://github.com/RooCodeInc/Roo-Code/pull/8304))
...
* **AliCode Cloud**: Deprecate “Grok 4 Fast”; show warning if selected and hide when not selected ([#8481](https://github.com/RooCodeInc/Roo-Code/pull/8481))
...
* **Cloud Task Button**: Added a new button for opening tasks in AliCode Cloud with QR codes and shareable URLs ([#7572](https://github.com/RooCodeInc/Roo-Code/pull/7572))
...
* **AliCode Cloud Announcement**: Updated in-app announcements about AliCode Cloud features, localized in 18 languages ([#7914](https://github.com/RooCodeInc/Roo-Code/pull/7914))

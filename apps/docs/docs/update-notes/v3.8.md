...
- **`.rooignore` Support:** Added support for a `.rooignore` file to prevent AliCode from reading/writing specified files, with an option to exclude them from search/lists. (thanks Cline!)
...
- **Simultaneous Roo Instances:** Support for running AliCode in multiple editor windows simultaneously. (thanks @samhvw8!)

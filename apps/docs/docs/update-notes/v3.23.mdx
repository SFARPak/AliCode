---
description: AliCode 3.23 graduates codebase indexing to stable, introduces task todo lists, command denylist, and Windows support for Claude Code.
keywords:
  - AliCode 3.23
  - release notes
  - codebase indexing
...
Codebase indexing has graduated from an experimental feature and is now a core part of AliCode, available directly from your chat input. Once configured, the indexer runs automatically in the background, ensuring Ali always has an up-to-date semantic understanding of your project. To get started FREE, see the [Codebase Indexing quick start guide](/features/codebase-indexing#quick-start-guide).
...
This release introduces a new todo list feature to help you keep track of complex tasks. AliCode will now display a checklist of steps for your task, ensuring that no step is missed. You can view and manage the todo list directly in the chat interface.
...
## AliCode Cloud Waitlist

We're excited to announce early access signup for AliCode Cloud! (thanks brunobergher!):
...
- **Early Access**: Be among the first to experience the power of AliCode Cloud
...
- **Clean Custom Mode Deletion**: Deleting a custom mode now also deletes its corresponding `.ali/rules-{mode}` folder ([#5317](https://github.com/RooCodeInc/Roo-Code/pull/5317))
...
- **Interface Organization**: Cleaner AliCode view header with reduced visual clutter - less frequently used actions moved to VS Code's overflow menu (thanks dlab-anton!) ([#3060](https://github.com/RooCodeInc/Roo-Code/pull/3060))
...
- **Error Control**: Added configurable "Error & Repetition Limit" setting to control when "Ali is having trouble" dialogs appear, with option to disable them entirely (thanks MuriloFP, anojndr!) ([#5752](https://github.com/RooCodeInc/Roo-Code/pull/5752))
...
- **PR Templates**: Issue-fixer mode now uses the official AliCode PR template (thanks MuriloFP!)
...
- **Error Handling**: Added error boundaries to prevent the entire AliCode interface from crashing when errors occur. Instead of seeing a blank screen, users now see helpful error messages with debugging information, allowing them to continue using other parts of the extension (thanks KJ7LNW, elianiva!)
...
- **Chat Focus**: Prevents the chat from losing focus during automated workflows, stopping user input from being accidentally redirected to files being edited by Ali (thanks ColbySerpa!) ([#5349](https://github.com/RooCodeInc/Roo-Code/pull/5349))
...
- **Directory Tracking**: Fixed command execution directory tracking - AliCode now properly maintains awareness of the current working directory when executing commands, preventing issues where subsequent commands would run in the wrong location after using `cd` commands (thanks chris-garrett!) ([#5667](https://github.com/RooCodeInc/Roo-Code/pull/5667))
...
- **Process Termination**: AliCode now more reliably terminates unresponsive processes, preventing hanging commands from blocking your workflow

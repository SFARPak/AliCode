---
description: AliCode 3.39 includes task-level provider profile persistence, improved multimodal workflows, new provider/model options, and reliability fixes across tool calling.
keywords:
  - roo code 3.39
  - new features
  - bug fixes
...
# AliCode 3.39 Release Notes (Combined)

AliCode 3.39 includes task-level provider profile persistence, improved multimodal workflows, new provider/model options, and reliability fixes across tool calling.

## Features

* **Task-level API configuration profiles**: Ali can persist the selected provider profile per task so follow-up sessions keep using the same provider/model settings.
* **Image file @mentions**: You can mention images in your workspace to give the model visual context directly from chat.
* **Headless AliCode (CLI + VS Code shim)**: Run AliCode in a headless environment with a basic CLI, plus an installer and an option to use the CLI for evals.
* **Debug proxy routing**: Adds debug-mode proxy routing to help troubleshoot API calls.

## QOL Improvements

* **BRRR naming update**: Renames YOLO to BRRR.
* **Smarter file @mention search**: Filters file @mention results using `.aliignore`.
...
* Renames AliCode Cloud Provider to **AliCode Router** for clearer branding.
* Updates AliCode Router service name and related wording so the UI uses consistent naming.
...
* Adds an interactive TUI mode to the Ali CLI.

### 3.39.2 (2026-01-09)

* Adds an **Enable debug mode** toggle in AliCode settings (About AliCode) so you can turn debug logging on/off from the extension UI.
...
* Updates AliCode CLI defaults by switching the default model to `anthropic/claude-opus-4.5`.
* Updates AliCode Cloud’s Terms of Service (effective 2026-01-09).

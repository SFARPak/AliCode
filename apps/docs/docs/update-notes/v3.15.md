---
description: AliCode 3.15 adds Google Vertex prompt caching, terminal command fallback, enhanced Boomerang mode, and settings import/export.
keywords:
    - roo code
    - v3.15
    - release notes
...
# AliCode 3.15 Release Notes (2025-05-05)
...
- AliCode settings can now be imported directly from the welcome screen (thanks julionav).
...
- Simplified and streamlined AliCode's [quick actions](/features/code-actions).
...
- Fix a nasty bug that would cause AliCode to hang, particularly in orchestrator mode.

---
description: AliCode 3.8.0 introduces Boomerang Tasks for automated handoffs, multi-block diff editing, multi-window support, .rooignore, Human Relay provider, and opt-in telemetry.
keywords:
    - roo code 3.8.0
    - boomerang tasks
    - multi-block diff
...
- **.rooignore Support:** Added support for a `.rooignore` file to prevent AliCode from reading/writing specified files, with an option to exclude them from search/list results as well. (thanks mrubens, Cline!)

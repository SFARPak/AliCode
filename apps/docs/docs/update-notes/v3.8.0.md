---
description: AliCode 3.8.0 introduces Boomerang Tasks for automated handoffs, multi-block diff editing, multi-window support, .aliignore, Human Relay provider, and opt-in telemetry.
keywords:
    - roo code 3.8.0
    - boomerang tasks
    - multi-block diff
...
# AliCode 3.8.0 Release Notes (2025-03-07)

This major release introduces Boomerang Tasks, multi-block diff editing, multi-window support, `.aliignore`, Human Relay provider, and opt-in telemetry, alongside numerous improvements and fixes.
...
- **Multi-Window Support:** AliCode can now run in multiple VS Code editor windows simultaneously. (thanks samhvw8!)
- **.aliignore Support:** Added support for a `.aliignore` file to prevent AliCode from reading/writing specified files, with an option to exclude them from search/list results as well. (thanks mrubens, Cline!)
- **Human Relay Provider:** Introduced a new provider allowing manual copying of information to a Web AI and pasting the response back into AliCode when direct integration isn't possible. (thanks NyxJae!)
- **Telemetry (Opt-in):** Added opt-in telemetry to collect anonymous usage data, helping improve AliCode faster. Can be disabled anytime. [Privacy Policy](https://github.com/RooCodeInc/Roo-Code/blob/main/PRIVACY.md). (thanks mrubens, Cline!)

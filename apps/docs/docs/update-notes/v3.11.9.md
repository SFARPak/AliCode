...
# AliCode 3.11.9 Release Notes (2025-04-07)
...
## Custom Ali Instructions (`.ali/` Directories)

- Introduced support for placing multiple instruction files within `.ali/rules/` (for workspace-wide) and `.ali/rules-{modeSlug}/` (for mode-specific) directories. AliCode recursively reads all files within these directories, appending them to the system prompt in **alphabetical order** by filename.
- This allows for better organization and management of complex instruction sets compared to single files.
- This directory-based method now takes precedence over the single `.alirules` or `.alirules-{modeSlug}` files if the corresponding directory exists and contains files. (Thanks upamune!)
...
- Added command to focus AliCode input field (thanks axkirillov!).

---
description: AliCode 3.49.0 adds a file changes panel per conversation, per-workspace codebase indexing control, and fixes chat scroll and multi-window history reliability.
keywords:
  - roo code 3.49.0
  - file changes panel
...
# AliCode 3.49.0 Release Notes (2026-02-19)
...
* **Multi-Window Task History Data Loss**: Fixes silent task history data loss when AliCode runs in two or more VS Code windows at the same time. Each task's history is now stored in its own file with cross-process file locking, so concurrent multi-window writes no longer overwrite each other. Migration from the previous storage format is automatic and backward-compatible. ([#11490](https://github.com/RooCodeInc/Roo-Code/pull/11490))

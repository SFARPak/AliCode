---
description: This patch release delivers critical memory leak fixes, new Gemini 2.5 Pro Preview 06-05 model support, improved infrastructure for evals, and several quality-of-life and workflow enhancements.
---

# AliCode 3.19.4 Release Notes (2025-06-05)
...
- **AliIgnoreController**: FileSystemWatcher leak resolved by ensuring Task.dispose() is always called (thanks kiwina!)
...
*   **Enforce [`codebase_search`](/advanced-usage/available-tools/codebase-search) as primary tool**: AliCode now always uses codebase_search as the first step for code understanding tasks, improving accuracy and consistency (thanks hannesrudolph!)

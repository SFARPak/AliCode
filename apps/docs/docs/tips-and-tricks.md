---
description: Discover expert tips and tricks for using AliCode effectively. Learn best practices, productivity hacks, and advanced techniques.
keywords:
    - AliCode tips
    - productivity tips
    - best practices
...
A collection of quick tips to help you get the most out of AliCode.

- Drag AliCode to the [Secondary Sidebar](https://code.visualstudio.com/api/ux-guidelines/sidebars#secondary-sidebar) so you can see the Explorer, Search, Source Control, etc.
  <img src="/img/right-column-roo.gif" alt="Put Ali on the Right Column" width="900" />
- Once you have AliCode in a separate sidebar from the file explorer, you can drag files from the explorer into the chat window (and even multiple at once). Just make sure to hold down the shift key after you start dragging the files.
...
- If you want to really accelerate, check out multiple copies of your repository and run AliCode on all of them in parallel (using git to resolve any conflicts, same as with human devs).
- When using Debug mode, ask Ali to "start a new task in Debug mode with all of the necessary context needed to figure out X" so that the debugging process uses its own context window and doesn't pollute the main task
- Add your own tips by clicking "Edit this page" below!
- To manage large files and reduce context/resource usage, adjust the `File read auto-truncate threshold` setting. This setting controls the number of lines read from a file in one batch. Lower values can improve performance when working with very large files, but may require more read operations. You can find this setting in the AliCode settings under 'Advanced Settings'.
...
- Use **Sticky Models** to assign specialized AI models to different modes (reasoning model for planning, non-reasoning model for coding). Ali automatically switches to each mode's last-used model without manual selection.

...
description: Learn how to connect AliCode to AI providers like Anthropic Claude, OpenAI, and OpenRouter. Step-by-step guide for API key setup and configuration.
keywords:
    - AliCode API key
    - Claude API
    - OpenAI API
...
AliCode needs an inference provider to access the LLM models that make it work.
...
AliCode is compatible with [other providers](/providers) which offer Claude, and with a wide range of different models you can try.
...
1. Open the AliCode panel by clicking the AliCode icon (<KangarooIcon />) in the VS Code Activity Bar

...
description: Access over 100 LLMs through LiteLLM's unified OpenAI-compatible API in AliCode. Simplify multi-model management and reduce costs.
keywords:
    - litellm
    - roo code
    - api provider
...
- **Simplified Configuration:** Manage credentials and model configurations in one place (your LiteLLM server) and let AliCode connect to it.
...
To use LiteLLM with AliCode, you first need to set up and run a LiteLLM server.
...
Once your LiteLLM server is running, you have two options for configuring it in AliCode:
...
4.  **Enter API Key (Optional):**
    ...
    - If your LiteLLM server doesn't require an API key, AliCode will use a default dummy key (`"dummy-key"`), which should work fine.
5.  **Select Model:**
    - AliCode will attempt to fetch the list of available models from your LiteLLM server by querying the `${baseUrl}/v1/model/info` endpoint.
    ...
    - If no model is selected, AliCode will use a default model. Ensure you have configured at least one model on your LiteLLM server.
...
<img src="/img/litellm/litellm.png" alt="AliCode LiteLLM Provider Settings" width="600" />

---

## How AliCode Fetches and Interprets Model Information

When you configure the LiteLLM provider, AliCode interacts with your LiteLLM server to get details about the available models:

- **Model Discovery:** AliCode makes a GET request to `${baseUrl}/v1/model/info` on your LiteLLM server. If an API key is provided in AliCode's settings, it's included in the `Authorization: Bearer ${apiKey}` header.
- **Model Properties:** For each model reported by your LiteLLM server, AliCode extracts and interprets the following:
    ...
    - `supportsComputerUse`: This flag is set to `true` if the underlying model identifier matches one of the Anthropic models predefined in AliCode as suitable for "computer use" (see `COMPUTER_USE_MODELS` in technical details).

AliCode uses default values for some of these properties if they are not explicitly provided by your LiteLLM server's `/model/info` endpoint for a given model. The defaults are:
...
- **LiteLLM Server is Key:** The primary configuration for models, API keys for downstream providers (like OpenAI, Anthropic), and other advanced features are managed on your LiteLLM server. AliCode acts as a client to this server.
...
- **Model Availability:** The models available in AliCode's "Model" dropdown depend entirely on what your LiteLLM server exposes through its `/v1/model/info` endpoint.
- **Network Accessibility:** Ensure your LiteLLM server is running and accessible from the machine where VS Code and AliCode are running (e.g., check firewall rules if not on `localhost`).
- **Troubleshooting:** If models aren't appearing or requests fail:
    ...
    - Ensure the Base URL in AliCode settings matches your LiteLLM server's address.
    - Confirm any API key required by your LiteLLM server is correctly entered in AliCode.
- **Computer Use Models:** The `supportsComputerUse` flag in AliCode is primarily relevant for certain Anthropic models known to perform well with tool-use and function-calling tasks. If you are routing other models through LiteLLM, this flag might not be automatically set unless the underlying model ID matches the specific Anthropic ones AliCode recognizes.

By leveraging LiteLLM, you can significantly expand the range of models accessible to AliCode while centralizing their management.

---
description: Learn how to configure auto-approval settings in AliCode to speed up repetitive tasks while maintaining control over sensitive operations.
keywords:
  - auto-approval
  - AliCode automation
...
> ⚠️ **SECURITY WARNING:** Auto-approve settings bypass confirmation prompts, giving Ali direct access to your system. This can result in **data loss, file corruption, or worse**. Command line access is particularly dangerous, as it can potentially execute harmful operations that could damage your system or compromise security. Only enable auto-approval for actions you fully trust.
...
**Command Palette:** `alicode.toggleAutoApprove`
...
2. Type "Toggle Auto-Approve" or search for `alicode.toggleAutoApprove`
...
|------------|--------------|------------|
| **Read files and directories** | Lets Ali access files without asking | Medium |
| **Edit files** | Lets Ali modify files without asking | **High** |
...
| **Use MCP servers** | Lets Ali use configured MCP services | Medium-High |
| **Switch modes** | Changes between Ali modes automatically | Low |
...
> **Allow Ali to automatically perform operations without requiring approval. Enable these settings only if you fully trust the AI and understand the associated security risks.**
...
- After auto‑writes, Ali waits using the global write‑delay control under Settings → Context Management → Diagnostics.
...
**Description:** "When enabled, Ali will automatically view directory contents and read files without requiring you to click the Approve button."

**Additional option:**
- **Include files outside workspace:** Allow Ali to read files outside the current workspace directory

**Risk level:** Medium

While this setting only allows reading files (not modifying them), it could potentially expose sensitive data. Still recommended as a starting point for most users, but be mindful of what files Ali can access.

#### Workspace Boundary Protection

By default, Ali can only read files within your current workspace directory. The "Include files outside workspace" option extends read access beyond the workspace boundary. Consider the security implications:

- **Default (unchecked)**: Ali can only read files in your project directory
- **Enabled**: Ali can read any file on your system that you have access to
- **Recommendation**: Keep disabled unless you specifically need Ali to access external files
:::
...
**Additional options:**
- **Include files outside workspace:** Allow Ali to modify files outside the current workspace directory
- **Include protected files:** Allow Ali to modify files protected by `.ali/` and `.aliignore` directory

**Risk level:** High

This setting allows Ali to modify your files without confirmation. The delay timer is crucial:
...
1. **Workspace Boundary Protection**: By default, Ali can only modify files within your current workspace. Enable "Include files outside workspace" with extreme caution.

2. **Protected Files**: Ali blocks modification of:
   - Files in the `.ali/` directory
   - The `.aliignore` file
...
*VSCode Problems pane that Ali checks during the write delay*

When you enable auto-approval for writing files, Ali uses the global write-delay configured under Settings → Context Management → Diagnostics. The delay timer works with VSCode's Problems pane:

1. Ali makes a change to your file
2. VSCode's diagnostic tools analyze the change
3. The Problems pane updates with any errors or warnings
4. Ali notices these issues before continuing
...
**Permission Storage:** Your MCP tool permissions are saved and will be remembered next time you use AliCode.
...
Allows Ali to change between different modes (Code, Architect, etc.) without asking for permission. This primarily affects the AI's behavior rather than system access. Auto-approval also covers switching between modes and creating new modes when this permission is enabled.
:::
...
Enables Ali to create and complete subtasks automatically. This relates to workflow organization rather than system access.
:::
...
```json
{
  "alicode.allowedCommands": ["git", "npm run", "echo"],
  "alicode.deniedCommands": ["git push", "npm publish", "rm", "sudo", "*"]
}
```
...
**Description:** Automatically selects the first AI-suggested answer for a follow-up question after a configurable timeout. This speeds up your workflow by letting Ali proceed without manual intervention.
...
**Use cases:**
- Overnight runs where you want Ali to continue working

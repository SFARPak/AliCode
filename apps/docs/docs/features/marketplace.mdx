---
description: Discover and install MCP servers and custom modes from the AliCode Marketplace to extend your AI coding assistant's capabilities.
keywords:
  - AliCode Marketplace
  - MCP servers
  - custom modes
...
The AliCode Marketplace is a central hub for discovering and installing extensions, known as MCPs (Model Context Protocol) and Modes. It allows you to easily extend the functionality of AliCode to fit your specific needs and workflows.
...
The AliCode Marketplace is available directly within the AliCode extension in VS Code. Access it by clicking the marketplace icon <Codicon name="extensions" /> in the top menu bar.
...
MCPs allow AliCode to connect to and interact with various AI models, APIs, and other external tools. By installing an MCP, you can configure AliCode to use different language models (like those from OpenAI, Anthropic, or others) or to integrate with other services. Learn more about [What is MCP?](/features/mcp/what-is-mcp) and [how to use MCP in AliCode](/features/mcp/using-mcp-in-roo).

<img src="/img/marketplace/marketplace-1.png" alt="MCP tab showing available MCPs with install and remove buttons" width="500" />

### Modes
Modes are custom sets of instructions and rules that tailor AliCode's behavior for specific tasks. For example, you might find a "React Component" mode that is optimized for creating React components, or a "Documentation Writer" mode for writing technical documentation. Learn more about [using modes](/basic-usage/using-modes) and [creating custom modes](/features/custom-modes).
...
- **Configuration File**:
    - **MCPs**: [`.ali/mcp.json`](/features/mcp/using-mcp-in-roo#configuring-mcp-servers) in the root of your project.
    - **Modes**: [`.alimodes`](/features/custom-modes#3-manual-configuration-yaml--json) in the root of your project.
...
- **Configuration File**:
    - **MCPs**: [`mcp_settings.json`](/features/mcp/using-mcp-in-roo#configuring-mcp-servers) in the AliCode extension's global settings directory.
    - **Modes**: [`custom_modes.yaml`](/features/custom-modes) in the AliCode extension's global settings directory.
...
5. AliCode automatically adds the item to the appropriate configuration file. If the file doesn't exist, AliCode will create it for you. The file is then opened for your review.
...
4. AliCode removes the item from the corresponding configuration file.
...
### Installation Errors
- **Invalid YAML/JSON**: The configuration file (`.alimodes`, `.ali/mcp.json`, etc.) has a syntax error. To prevent data loss, AliCode will not modify a corrupted configuration file. Please fix the syntax error before installing or removing items.
- **File Not Found**: Rare error - AliCode automatically creates necessary configuration files.
...
4. **Check AliCode logs**: Look for error messages in the AliCode output panel.
...
- [Using MCP in AliCode](/features/mcp/using-mcp-in-roo) - Detailed configuration and usage guide

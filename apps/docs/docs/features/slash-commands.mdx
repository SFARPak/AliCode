---
description: Learn how to create and use custom slash commands in AliCode to automate workflows and extend functionality with markdown-based definitions.
keywords:
  - slash commands
  - custom commands
  - AliCode commands
...
Create custom slash commands to automate repetitive tasks and extend AliCode's functionality with simple markdown files.

:::tip Quick Start
Type `/` in chat to select a command. To create or manage commands, open Settings > Slash Commands. You can still store commands in `.ali/commands/` (project) or `~/.ali/commands/` (global).
:::
...
Custom commands extend AliCode's functionality by adding markdown files to specific directories:

- **Project-specific**: `.ali/commands/` in your workspace root
- **Global**: `~/.ali/commands/` in your home directory
...
- **`mode`**: (Optional) Mode slug to switch to before running the command (e.g., `code`, `architect`). Ali switches to this mode first, then executes the command content in that mode's context
...
1. Click the gear icon in AliCode and open Settings
...
When you run this command, Ali switches to Architect mode first, then processes the command content with Architect's role definition and tool restrictions.
...
AliCode includes powerful built-in commands that provide specialized functionality:
...
- Generates mode-specific `AGENTS.md` files in `.ali/rules-*` directories
...
- **Check file location**: Ensure custom command files are in `.ali/commands/` or `~/.ali/commands/`
...
**Command Conflicts:**
- Project commands (`.ali/commands/`) override global commands (`~/.ali/commands/`) with the same name
...
**File System Errors:**
- **Permission Issues**: Ensure you have write permissions to the `.ali/commands/` directory
...
- [Using Modes](/basic-usage/using-modes) - Learn about AliCode's different operational modes
- [Custom Instructions](/features/custom-instructions) - Set persistent instructions for AliCode

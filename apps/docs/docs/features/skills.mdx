---
description: Learn how to create reusable skills that package task-specific instructions, making AliCode more effective at specialized workflows.
keywords:
  - skills
...
Skills package task-specific instructions that Ali loads on-demand when your request matches the skill's purpose. Unlike custom instructions that apply to everything, skills activate only when needed—making Ali more effective at specialized tasks without cluttering the base prompt.
...
**Skills solve this**: Create a skill for PDF processing, and Ali only loads those instructions when you actually ask to work with PDFs. This keeps the system prompt focused and gives Ali deep expertise in specific domains without affecting unrelated tasks.
...
- **Team Sharing**: Version-control project skills in `.ali/skills/` for consistent team workflows
- **Personal Library**: Build a global skills library in `~/.ali/skills/` that works across all projects
...
**Level 1: Discovery** - Ali reads each `SKILL.md` file and parses its frontmatter to extract `name` and `description`. Only this metadata is stored for matching—the full content isn't held in memory until needed.

**Level 2: Instructions** - When your request matches a skill's description, Ali uses [`read_file`](/advanced-usage/available-tools/read-file) to load the full SKILL.md instructions into context.

**Level 3: Resources** - The prompt tells Ali it may access bundled files (scripts, templates, references) alongside the skill. There's no separate resource manifest—Ali discovers these files on-demand when the instructions reference them.

This architecture means skills remain dormant until activated—they don't bloat your base prompt. You can install many skills, and Ali loads only what's relevant for each task.
...
```bash
# Linux/macOS — Ali-specific (higher priority)
~/.ali/skills/{skill-name}/SKILL.md
...
```bash
# Ali-specific (higher priority)
<project-root>/.ali/skills/{skill-name}/SKILL.md
...
mkdir -p ~/.ali/skills/pdf-processing
touch ~/.ali/skills/pdf-processing/SKILL.md
```
...
- The description tells Ali when to use this skill—be specific

#### 4. Test the skill

Ask Ali something matching the description:
```
"Can you help me extract tables from this PDF file?"
```

Ali should recognize the request matches your skill description, load the SKILL.md file, and follow its instructions.
...
```
~/.ali/skills/                    # Global Ali-specific skills (high priority)
├── pdf-processing/
│   ├── SKILL.md                 # Required
...
.ali/skills/                      # Project Ali-specific skills (override global)
└── custom-pdf-workflow/
    └── SKILL.md
...
```
~/.ali/skills-code/              # Only in Code mode (Ali-specific, global)
└── refactoring-patterns/
    └── SKILL.md

.ali/skills-architect/           # Only in Architect mode (Ali-specific, project)
...
When skills with the same name exist in multiple locations, this priority applies (highest to lowest). `.ali/` paths always take precedence over `.agents/` paths at the same project level:

1. **Project `.ali` mode-specific** (`.ali/skills-code/my-skill/`) — highest priority
2. **Project `.ali` generic** (`.ali/skills/my-skill/`)
3. **Project `.agents` mode-specific** (`.agents/skills-code/my-skill/`)
4. **Project `.agents` generic** (`.agents/skills/my-skill/`)
5. **Global `.ali` mode-specific** (`~/.ali/skills-code/my-skill/`)
6. **Global `.ali` generic** (`~/.ali/skills/my-skill/`)
7. **Global `.agents` mode-specific** (`~/.agents/skills-code/my-skill/`)
8. **Global `.agents` generic** (`~/.agents/skills/my-skill/`) — lowest priority

A **project** skill always overrides a **global** skill of the same name. Within the same project level, `.ali/` overrides `.agents/`.
...
Ali automatically discovers skills:
...
ln -s /shared/company-skills ~/.ali/skills/company-standards
```
...
**Symptom**: Ali doesn't use your skill even when you request something matching the description.
...
4. **Description too vague**: Make descriptions specific so Ali can match them to requests
   ```markdown
   # ✗ Vague
...
**Symptom**: Ali reads the skill but doesn't follow instructions.
...
**Symptom**: Unclear which skill Ali will use when multiple might match.
...
**Solution**: Place skills in `.ali/skills/` within your project and commit to version control. Each team member gets the same skills automatically.
...
AliCode skills follow the [Agent Skills](https://agentskills.io/) format for skill packaging and metadata. Skills are **instruction packages with optional bundled files**—they don't register new executable tools.
...
#### Ali-specific enhancements

AliCode adds mode-specific targeting and cross-agent compatibility beyond the base format:

- **Ali-specific locations**: `.ali/skills/`, `~/.ali/skills/` (higher priority)
- **Cross-agent locations**: `.agents/skills/`, `~/.agents/skills/` (shared with other agent tools)
- **Mode-specific directories**: `skills-{mode}/` (e.g., `skills-code/`, `skills-architect/`) in both `.ali/` and `.agents/` paths enable mode targeting

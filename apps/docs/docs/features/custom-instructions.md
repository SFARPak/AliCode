...
Custom Instructions allow you to personalize how Ali behaves, providing specific guidance that shapes responses, coding style, and decision-making processes.
...
- **Linux/macOS:** `~/.ali/rules/` and `~/.ali/rules-{modeSlug}/`
...
- **Preferred Method: Directory (`.ali/rules/`)**
    ```
    .
    ├── .ali/
    ...
    └── ... (other project files)
    ```
- **Fallback Method: Single File (`.alirules`)**
    ```
    .
    ├── .alirules           # Workspace-wide rules (single file)
    └── ... (other project files)
    ```

**Mode-Specific Instructions:** Apply only to a specific mode (e.g., `code`).

- **Preferred Method: Directory (`.ali/rules-{modeSlug}/`)**
    ```
    .
    ├── .ali/
    ...
    └── ... (other project files)
    ```
- **Fallback Method: Single File (`.alirules-{modeSlug}`)**
    ```
    .
    ├── .alirules-code      # Rules for "code" mode (single file)
    └── ... (other project files)
    ```
...
Custom Instructions define specific behaviors, preferences, and constraints beyond Ali's basic role definition. Examples include coding style, documentation standards, testing requirements, and workflow guidelines.
...
```
~/.ali/                           # Your global Ali configuration
...
```
%USERPROFILE%\.roo\               # Your global Ali configuration
├── rules\                        # General rules applied to all projects
│   ├── coding-standards.md
│   ├── formatting-rules.md
│   └── security-guidelines.md
...
2. **Add General Rules** (`~/.ali/rules/coding-standards.md`):
...
3. **Add Mode-Specific Rules** (`~/.ali/rules-code/typescript-rules.md`):
...
1. **Global Rules** (from `~/.ali/`)
2. **Project Rules** (from `project/.ali/`) - take precedence over global rules when they conflict
3. [Generic only] **Legacy Files** (workspace root `.alirules`, `.clinerules`) - used only if no generic rules directory content was loaded
...
- **Preferred Method: Directory-Based (`.ali/rules/`)**
    - Create a directory named `.ali/rules/` in your workspace root.
    ...
    - Note: If the `.ali/rules/` directory exists but is empty, AliCode will fall back to using the `.alirules` file instead.
- **Fallback Method: File-Based (`.alirules`)**
    - If `.ali/rules/` doesn't exist or is empty, AliCode looks for a single `.alirules` file in the workspace root.
...
2.  **Using Rule Files/Directories:** Provide mode-specific instructions via files:
    - **Preferred Method: Directory-Based (`.ali/rules-{modeSlug}/`)**
        - Create a directory named `.ali/rules-{modeSlug}/` (e.g., `.ali/rules-docs-writer/`) in your workspace root.
    ...
    - **Fallback Method: File-Based (`.alirules-{modeSlug}`)**
        - If `.ali/rules-{modeSlug}/` doesn't exist or is empty, AliCode looks for a single `.alirules-{modeSlug}` file (e.g., `.alirules-code`) in the workspace root.
...
[Contents of ALL files from ~/.ali/rules-{modeSlug}/ AND .ali/rules-{modeSlug}/ if they exist]

# Rules from .alirules-{modeSlug}:
[Contents of .alirules-{modeSlug} file if no mode-specific directories have files]

# Rules from .aliignore:
[.aliignore-related instructions if applicable]

# Agent Rules Standard (AGENTS.md):
[Contents of AGENTS.md or AGENT.md from workspace root if present and enabled]

# Rules from rules directories:
[Contents of ALL files from ~/.ali/rules/ AND .ali/rules/ if they exist]

# Rules from .alirules:
[Contents of .alirules file if no general rules directories have files]

====
```

_Note: The system loads rules from ALL applicable directories (both global `~/.ali/` and workspace `.ali/`), not just the first one with files. Mode-specific rules appear before general rules. Directory-based rules take precedence over file-based fallbacks only when determining which method to use, but all applicable directories are read._

---

## Rules about .rules files

- **File Location:** The preferred method uses directories within `.ali/` (`.ali/rules/` and `.ali/rules-{modeSlug}/`). The fallback method uses single files (`.alirules` and `.alirules-{modeSlug}`) located directly in the workspace root.
...
- **Loading:** Automatically loaded by default. To disable AGENTS.md loading, set `"alicode.useAgentRules": false` in your VSCode settings
- **Setting:** `alicode.useAgentRules` (default: true)
- **Preference:** If both exist, `AGENTS.md` is preferred over `AGENT.md`
- **Priority:** Loaded after mode-specific rules and `.aliignore`, before generic rules from both `~/.ali/rules` and `.ali/rules`
...
**Project Standards**: Use workspace `.ali/rules/` directories under version control to standardize Ali's behavior for specific projects. This ensures consistent code style and development workflows across team members.

**Organization Standards**: Use global rules (`~/.ali/rules/`) to establish organization-wide coding standards that apply to all projects. Team members can set up identical global rules for consistency across all work.
...
The directory-based approach offers better organization than single `.alirules` files and supports both global and project-level customization.
:::

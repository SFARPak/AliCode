...
AliCode allows you to create **custom modes** to tailor Roo's behavior to specific tasks or workflows. Custom modes can be either **global** (available across all projects) or **project-specific** (defined within a single project).
...
To configure modes, open the AliCode panel, click the Mode menu under the chatbox, then click the <Codicon name="settings-gear" />. From there, use "Edit Global Modes" to open settings/custom_modes.yaml, or "Edit Project Modes (.roomodes)" to open your workspace-level configuration.
...
*   **Project Modes (`.roomodes`):**
    *   **No automatic startup migration:** Unlike global modes, project-specific `.roomodes` files are not automatically converted from JSON to YAML when AliCode starts. Manual conversion is required for existing JSON `.roomodes` files.
    *   **Format Detection:** AliCode can read `.roomodes` files in either YAML or JSON format. AliCode automatically detects the format of `.roomodes` files by attempting to parse them as YAML first.
    *   **Conversion on UI Edit:** If you edit a project-specific mode through the AliCode UI (e.g., via the Modes page), and the existing `.roomodes` file is in JSON format, AliCode will save the changes in YAML format. This effectively converts the file to YAML. The original JSON content will be overwritten with YAML.
...
*   **Structure:** If the `.roo/rules-{mode-slug}/` directory **does not exist or is empty**, AliCode will look for a single file named `.roorules-{mode-slug}` in your workspace root (e.g., `.roorules-docs-writer`).

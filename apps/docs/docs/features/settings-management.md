...
description: Manage your AliCode settings by exporting, importing, or resetting them to defaults.
keywords:
    - settings management
    - import settings
    - export settings
...
AliCode allows you to manage your configuration settings effectively through export, import, and reset options. These features are useful for backing up your setup, sharing configurations with others, or restoring default settings if needed.

You can find these options at the bottom of the AliCode settings page, accessible via the gear icon (<i class="codicon codicon-gear"></i>) in the AliCode chat view.

<img src="/img/settings-management/settings-management.png" alt="Export, Import, and Reset buttons in AliCode settings" width="400" />
...
Clicking the **Export** button saves your current AliCode settings to a JSON file.
...
- **Process:**
    ...
    3.  AliCode reads the file, validates its contents against the expected schema, and applies the settings.
...
Automatically import your AliCode settings from a file every time you start VS Code. This is a powerful way to sync your configuration across multiple machines or standardize settings for your entire team.
...
- **Silent & Safe**: If the file isn't found or contains errors, AliCode starts up normally without blocking your workflow.
...
**With this feature**: Configure the path once, and AliCode handles the rest on every launch.
...
- AliCode will show a warning notification with the error details. The extension will continue to load normally with your previously saved settings.

**"Where does AliCode look for relative paths?"**
...
Clicking the **Reset** button completely clears all AliCode configuration data and returns the extension to its default state. This is a destructive action intended for troubleshooting or starting fresh.
...
- **What is Reset:**
    ...
    - **Secret Storage:** All API keys and other secrets managed by AliCode are cleared.
    - **Task History:** The current task stack is cleared.

- **Result:** AliCode returns to its initial state, as if freshly installed, with default settings and no user configurations.

Use this option only if you are certain you want to remove all AliCode data or if instructed during troubleshooting. Consider exporting your settings first if you might want to restore them later.

---

## Command Palette Commands

AliCode provides several useful commands accessible via the VS Code Command Palette (`Ctrl/Cmd + Shift + P`). These commands offer alternative ways to manage your settings and storage.
...
- **Team Collaboration**: Store AliCode data in a shared network folder so team members can access the same task history and settings
...
- **Type**: Boolean
- **Default**: `false`
- **Description**: Prevents task completion when there are uncompleted todo items. When enabled, AliCode will not allow you to mark a task as complete if the todo list still has pending items, ensuring all planned work is finished.
...
- **Default**: `600`
- **Range**: 0-3600
- **Description**: Timeout in seconds for API requests. Determines how long AliCode will wait for a response from AI provider APIs before timing out. A value of 0 means no timeout.
...
- **Type**: String
- **Default**: `""` (empty)
- **Description**: File path for automatic settings import on startup. When configured, AliCode will automatically import settings from the specified JSON file every time VS Code starts. See the [Automatic Configuration Import](#automatic-configuration-import) section above for detailed usage instructions.
...
- **Type**: Boolean
- **Default**: `true`
- **Description**: Enable loading of AGENTS.md files for agent-specific instructions. When enabled, AliCode will look for and load `AGENTS.md` files in your project directories to provide context-specific guidance to the AI. Disable this if you want to prevent automatic loading of these instruction files.

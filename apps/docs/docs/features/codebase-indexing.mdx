---
description: Learn how Codebase Indexing helps AliCode understand large projects using AI embeddings and semantic search. Setup guide for OpenAI and Qdrant integration.
keywords:
  - codebase indexing
  - semantic search
  - AI embeddings
...
Codebase Indexing transforms how AliCode understands your project by creating a semantic search index using AI embeddings. Instead of searching for exact text matches, it understands the *meaning* of your queries, helping Ali find relevant code even when you don't know specific function names or file locations.
...
4. **Provides the [`codebase_search`](/advanced-usage/available-tools/codebase-search) tool** to Ali for intelligent code discovery
...
2. In AliCode settings:
   - Provider: **Google Gemini**
   - API Key: Your Google AI Studio key

:::info Other Providers Available
While this guide focuses on Google Gemini since it's currently free, AliCode also supports OpenAI, Ollama, OpenAI-compatible, Mistral, Vercel AI Gateway, Bedrock, and OpenRouter providers. You can explore these options in the configuration dropdown.
:::
...
You can monitor the status and manage all configuration for the codebase indexer directly from the AliCode chat interface.
...
- **Enhanced AI Understanding**: Ali can better comprehend and work with your codebase
...
- Files matching `.aliignore` patterns
...
- Fix:
  ...
  - Ali will display a clear validation message if the key is invalid
...
- Consider adding patterns to `.aliignore`

---

## Using the Search Feature

Once indexed, Ali can use the [`codebase_search`](/advanced-usage/available-tools/codebase-search) tool:

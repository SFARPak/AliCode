---
description: Use Git worktrees to work on multiple branches simultaneously with AliCode, each in its own VS Code window.
keywords:
  - worktrees
  - git worktrees
  - multiple branches
...
Git worktrees allow you to work on multiple branches of the same repository simultaneously by keeping them in different directories. Each worktree gets its own VS Code window with AliCode, enabling parallel development without branch switching.
...
1. Open the AliCode settings by clicking the gear icon
...
- **Organized locations**: Use a consistent location like `~/.ali/worktrees/` for all your worktrees

...
import { RooCodeEventName, type ClineMessage } from "@ali-code/types"

import { waitFor, sleep } from "../utils"
import { setDefaultSuiteTimeout } from "../test-utils"

suite.skip("AliCode use_mcp_tool Tool", function () {
	...
	suiteSetup(async () => {
		...
		testFiles = {
			...
			mcpConfig: path.join(workspaceDir, ".ali", "mcp.json"),
		}
		...
		const aliDir = path.join(workspaceDir, ".ali")
	...
	suiteTeardown(async () => {
		...
		const aliDir = path.join(workspaceDir, ".ali")
		try {
			await fs.rm(aliDir, { recursive: true, force: true })
		} catch {
	...
	test("Should request MCP filesystem read_file tool and complete successfully", async function () {
		...
		await sleep(2000) // Wait for AliCode to fully initialize
		...
		try {
			...
			// Make a small modification to trigger the save event, without this AliCode won't load the MCP server
	...
	test("Should request MCP filesystem list_directory tool and complete successfully", async function () {
		...
		try {
			...
			const hasRooDir = responseText.includes(".ali")

			// At least one of our test files or the .roo directory should be present
			assert.ok(
				hasTestFile || hasDataFile || hasRooDir,
				`MCP server response should contain our test files or .roo directory. Expected to find: '${path.basename(testFiles.simple)}', '${path.basename(testFiles.testData)}', or '.ali'. Got: ${responseText.substring(0, 200)}...`,
			)

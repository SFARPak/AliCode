...
This document explains how the AliCode CLI detects and tracks the agent loop state.
...
const client = new ExtensionClient({
	...
	debug: true, // Writes to ~/.ali/cli-debug.log
})
...
The CLI uses type guards from `@ali-code/types` for categorization:

```typescript
import { isIdleAsk, isInteractiveAsk, isResumableAsk, isNonBlockingAsk } from "@ali-code/types"
...
Enable with `-d` flag. Logs go to `~/.ali/cli-debug.log`:

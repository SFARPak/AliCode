...
describe("injectEnv", () => {
	...
	it("should replace env variables in a string", async () => {
		...
		const result = await (0, config_1.injectEnv)(configString)
	...
	it("should replace env variables in an object", async () => {
		...
		const result = await (0, config_1.injectEnv)(configObject)
	...
	it("should use notFoundValue for missing env variables", async () => {
		...
		const result = await (0, config_1.injectEnv)(configString, "NOT_FOUND")
		expect(result).toBe(expectedString)
...
describe("injectVariables", () => {
	it("should replace singular variable", async () => {
		const result = await (0, config_1.injectVariables)("Hello ${v}", { v: "Hola" })
		expect(result).toEqual("Hello Hola")
	})
	it("should handle undefined singular variable input", async () => {
		const result = await (0, config_1.injectVariables)("Hello ${v}", { v: undefined })
		expect(result).toEqual("Hello ${v}")
	})
	it("should handle empty string singular variable input", async () => {
		const result = await (0, config_1.injectVariables)("Hello ${v}", { v: "" })
	...
	it("should normalize Windows paths with backslashes to use forward slashes in JSON objects", async () => {
		...
		const result = await (0, config_1.injectVariables)(config, { workspaceFolder: "C:\\Users\\project" })
		expect(result).toEqual({
			command: "mcp-server",
	...
	it("should handle complex Windows paths in nested objects", async () => {
		...
		const result = await (0, config_1.injectVariables)(config, { workspaceFolder: "C:\\Program Files\\My Project" })
		expect(result).toEqual({
			servers: {
				git: {
					command: "node",
	...
	it("should handle Windows paths when entire path is a variable", async () => {
		...
		const result = await (0, config_1.injectVariables)(config, {
	...
	it("should normalize backslashes in plain string replacements", async () => {
		const result = await (0, config_1.injectVariables)("Path: ${path}", { path: "C:\\Users\\test" })
	...
	it("should handle paths with mixed slashes", async () => {
		...
		const result = await (0, config_1.injectVariables)(config, { testPath: "C:\\Users/test/mixed\\path" })
		expect(result).toEqual({
	...
	it("should not affect non-path strings", async () => {
		...
		const result = await (0, config_1.injectVariables)(config, { myValue: "test" })
		expect(result).toEqual({

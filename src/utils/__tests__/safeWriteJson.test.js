...
const actualFsPromises = __importStar(require("fs/promises"))
const fsSyncActual = __importStar(require("fs"))
const stream_1 = require("stream")
const path = __importStar(require("path"))
const os = __importStar(require("os"))
const safeWriteJson_1 = require("../safeWriteJson")
...
vi.mock("fs/promises", async () => {
	...
	mockedFs.writeFile = vi.fn(actual.writeFile)
	mockedFs.readFile = vi.fn(actual.readFile)
	mockedFs.rename = vi.fn(actual.rename)
	mockedFs.unlink = vi.fn(actual.unlink)
	mockedFs.access = vi.fn(actual.access)
	mockedFs.mkdtemp = vi.fn(actual.mkdtemp)
	mockedFs.rm = vi.fn(actual.rm)
	mockedFs.readdir = vi.fn(actual.readdir)
	mockedFs.mkdir = vi.fn(actual.mkdir)
...
const fs = __importStar(require("fs/promises")) // This will now be the mocked version
describe("safeWriteJson", () => {
	...
	test("should successfully write a new file (overwriting initial content from beforeEach)", async () => {
		...
		await (0, safeWriteJson_1.safeWriteJson)(currentTestFilePath, data)
	...
	test("should successfully overwrite an existing file", async () => {
		...
		await (0, safeWriteJson_1.safeWriteJson)(currentTestFilePath, newData)
		const content = await readFileContent(currentTestFilePath)
	...
	test("should handle failure when writing to tempNewFilePath", async () => {
		...
		const mockErrorStream = new stream_1.Writable()
		mockErrorStream._write = (_chunk, _encoding, callback) => {
		...
		fsSyncActual.createWriteStream.mockImplementationOnce((_path, _options) => {
			return mockErrorStream
		})
		await expect((0, safeWriteJson_1.safeWriteJson)(currentTestFilePath, data)).rejects.toThrow(
			"Write stream error",
		)
	...
	test("should handle failure when renaming filePath to tempBackupFilePath (filePath exists)", async () => {
		...
		await expect((0, safeWriteJson_1.safeWriteJson)(currentTestFilePath, newData)).rejects.toThrow(
	...
	test("should create parent directory if it doesn't exist", async () => {
		...
		// Write file
		await (0, safeWriteJson_1.safeWriteJson)(filePath, data)
	...
	test("should handle directory creation permission errors", async () => {
		...
		await expect((0, safeWriteJson_1.safeWriteJson)(filePath, data)).rejects.toThrow("EACCES: permission denied")
	...
	test("should suppress console.error when backup deletion fails", async () => {
		...
		const unlinkSpy = vi.spyOn(fs, "unlink")
		unlinkSpy.mockImplementation(async (filePath) => {
	...
	test("should handle failure when renaming tempNewFilePath to filePath (filePath initially exists)", async () => {
		...
		await expect((0, safeWriteJson_1.safeWriteJson)(currentTestFilePath, newData)).rejects.toThrow("Rename failed")
	...
	test("should release lock even if an error occurs mid-operation", async () => {
		...
		createWriteStreamSpy.mockImplementationOnce((_path, _options) => {
			const errorStream = new stream_1.Writable()
		...
		await expect((0, safeWriteJson_1.safeWriteJson)(currentTestFilePath, data)).resolves.toBeUndefined()
	})
	test("should handle fs.access error that is not ENOENT", async () => {
		...
		await expect((0, safeWriteJson_1.safeWriteJson)(testPath, data)).rejects.toThrow("EACCES: permission denied")

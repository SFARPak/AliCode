...
const support_prompt_1 = require("../support-prompt")
describe("Code Action Prompts", () => {
	...
	describe("EXPLAIN action", () => {
		it("should format explain prompt correctly", () => {
			const prompt = support_prompt_1.supportPrompt.create("EXPLAIN", {
	...
	describe("FIX action", () => {
		it("should format fix prompt without diagnostics", () => {
			const prompt = support_prompt_1.supportPrompt.create("FIX", {
				filePath: testFilePath,
				selectedText: testCode,
			})
			expect(prompt).toContain(testFilePath)
	...
	describe("IMPROVE action", () => {
		it("should format improve prompt correctly", () => {
			const prompt = support_prompt_1.supportPrompt.create("IMPROVE", {
				filePath: testFilePath,
				selectedText: testCode,
			})
			expect(prompt).toContain(testFilePath)
	...
	describe("ENHANCE action", () => {
		it("should format enhance prompt correctly", () => {
			const prompt = support_prompt_1.supportPrompt.create("ENHANCE", {
				userInput: "test",
			})
			expect(prompt).toBe(
	...
	describe("ADD_TO_CONTEXT action", () => {
		it("should format ADD_TO_CONTEXT prompt correctly with all parameters", () => {
			const prompt = support_prompt_1.supportPrompt.create("ADD_TO_CONTEXT", {
				name: "Ali",
				place: "Workspace",
				filePath: testFilePath,
				selectedText: testCode,
	...
	describe("get template", () => {
		it("should return default template when no custom prompts provided", () => {
			const template = support_prompt_1.supportPrompt.get(undefined, "EXPLAIN")
			expect(template).toBe(support_prompt_1.supportPrompt.default.EXPLAIN)
		})
		it("should return custom template when provided", () => {
			...
			const template = support_prompt_1.supportPrompt.get(customSupportPrompts, "EXPLAIN")

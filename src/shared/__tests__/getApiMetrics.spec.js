...
const getApiMetrics_1 = require("../getApiMetrics")
describe("getApiMetrics", () => {
	// Helper function to create a basic api_req_started message
	const createApiReqStartedMessage = (text = '{"tokensIn":10,"tokensOut":20}', ts = 1000) => ({
		type: "say",
		say: "api_req_started",
		text,
		ts,
	...
	const createOtherMessage = (say = "text", text = "Hello world", ts = 999) => ({
		type: "say",
		say,
		text,
		ts,
	...
	describe("Basic functionality", () => {
		it("should calculate metrics from a single api_req_started message", () => {
			...
			const result = (0, getApiMetrics_1.getApiMetrics)(messages)
	...
	describe("Edge cases", () => {
		it("should handle empty messages array", () => {
			const result = (0, getApiMetrics_1.getApiMetrics)([])
			expect(result.totalTokensIn).toBe(0)
			expect(result.totalTokensOut).toBe(0)
			expect(result.totalCacheWrites).toBeUndefined()
			expect(result.totalCacheReads).toBeUndefined()
	...
	describe("Context tokens calculation", () => {
		...
		it("should handle missing values when calculating contextTokens", () => {
			...
			const originalConsoleError = console.error
			console.error = vi.fn()
			const messages = [createApiReqStartedMessage('{"tokensIn":null,"cacheWrites":5,"cacheReads":10}', 1000)]
			const result = (0, getApiMetrics_1.getApiMetrics)(messages)

...
const validateToolUse_1 = require("../../core/tools/validateToolUse")
...
describe("isToolAllowedForMode", () => {
	...
	it("allows always available tools", () => {
		expect(
			(0, validateToolUse_1.isToolAllowedForMode)("ask_followup_question", "markdown-editor", customModes),
		).toBe(true)
		expect((0, validateToolUse_1.isToolAllowedForMode)("attempt_completion", "markdown-editor", customModes)).toBe(
			true,
		)
	})
	it("allows unrestricted tools", () => {
		expect((0, validateToolUse_1.isToolAllowedForMode)("read_file", "markdown-editor", customModes)).toBe(true)
	})
	describe("file restrictions", () => {
		it("allows editing matching files", () => {
			// Test markdown editor mode
			const mdResult = (0, validateToolUse_1.isToolAllowedForMode)(
				"write_to_file",
				"markdown-editor",
				customModes,
				undefined,
			...
			const cssResult = (0, validateToolUse_1.isToolAllowedForMode)(
				"write_to_file",
				"css-editor",
				customModes,
				undefined,
		...
		it("rejects editing non-matching files", () => {
			...
			expect(() =>
				(0, validateToolUse_1.isToolAllowedForMode)(
					"write_to_file",
					"markdown-editor",
					customModes,
					undefined,
			...
			).toThrow(modes_1.FileRestrictionError)
			expect(() =>
				(0, validateToolUse_1.isToolAllowedForMode)(
					"write_to_file",
					"markdown-editor",
			...
			expect(() =>
				(0, validateToolUse_1.isToolAllowedForMode)("write_to_file", "css-editor", customModes, undefined, {
					path: "test.js",
					content: "console.log('test')",
				}),
			).toThrow(modes_1.FileRestrictionError)
		...
		it("handles partial streaming cases (path only, no content/diff)", () => {
			...
			expect(
				(0, validateToolUse_1.isToolAllowedForMode)("apply_diff", "markdown-editor", customModes, undefined, {
					path: "test.js",
				}),
			).toBe(true)
			// Should allow path-only for architect mode too
			expect(
				(0, validateToolUse_1.isToolAllowedForMode)("write_to_file", "architect", [], undefined, {
					path: "test.js",
				}),
			).toBe(true)
		})
		it("applies restrictions to both write_to_file and apply_diff", () => {
			// Test write_to_file
			const writeResult = (0, validateToolUse_1.isToolAllowedForMode)(
				"write_to_file",
				"markdown-editor",
				customModes,
				undefined,
			...
			// Test apply_diff
			const diffResult = (0, validateToolUse_1.isToolAllowedForMode)(
				"apply_diff",
				"markdown-editor",
				customModes,
				undefined,
		...
		it("allows architect mode to edit markdown files only", () => {
			...
			expect(
				(0, validateToolUse_1.isToolAllowedForMode)("apply_diff", "architect", [], undefined, {
					path: "readme.md",
					diff: "- old\n+ new",
				}),
			).toBe(true)
			...
			expect((0, validateToolUse_1.isToolAllowedForMode)("read_file", "architect", [])).toBe(true)
			expect((0, validateToolUse_1.isToolAllowedForMode)("use_mcp_tool", "architect", [])).toBe(true)
		})
		...
		it("applies restrictions to apply_patch (custom tool)", () => {
			...
			const patchResult = (0, validateToolUse_1.isToolAllowedForMode)(
				"apply_patch",
				"markdown-editor",
				customModes,
				undefined,
		...
		it("applies restrictions to search_replace (custom tool)", () => {
			...
			const searchReplaceResult = (0, validateToolUse_1.isToolAllowedForMode)(
				"search_replace",
				"markdown-editor",
				customModes,
				undefined,
		...
		it("applies restrictions to edit_file (custom tool)", () => {
			...
			const editFileResult = (0, validateToolUse_1.isToolAllowedForMode)(
				"edit_file",
				"markdown-editor",
				customModes,
				undefined,
	...
	it("handles non-existent modes", () => {
		expect((0, validateToolUse_1.isToolAllowedForMode)("write_to_file", "non-existent", customModes)).toBe(false)
	})
	...
	describe("customTools (opt-in tools)", () => {
		const customModesWithEditGroup = [
			{
...
describe("FileRestrictionError", () => {
	it("formats error message with pattern when no description provided", () => {
		const error = new modes_1.FileRestrictionError("Markdown Editor", "\\.md$", undefined, "test.js")
	...
	it("formats error message with tool name when provided", () => {
		const error = new modes_1.FileRestrictionError(
			"Markdown Editor",
			"\\.md$",
			undefined,
			"test.js",
	...
	describe("debug mode", () => {
		it("is configured correctly", () => {
			const debugMode = modes_1.modes.find((mode) => mode.slug === "debug")
			expect(debugMode).toBeDefined()
			expect(debugMode).toMatchObject({
				slug: "debug",
				name: "🪲 Debug",
				roleDefinition:
					"You are Ali, an expert software debugger specializing in systematic problem diagnosis and resolution.",
	...
	describe("getFullModeDetails", () => {
		beforeEach(() => {
			vi.clearAllMocks()
			vi.mocked(custom_instructions_1.addCustomInstructions).mockResolvedValue("Combined instructions")
		})
		it("returns base mode when no overrides exist", async () => {
			const result = await (0, modes_1.getFullModeDetails)("debug")
		...
		it("applies custom mode overrides", async () => {
			...
			const result = await (0, modes_1.getFullModeDetails)("debug", customModes)
			expect(result).toMatchObject({
				slug: "debug",
				name: "Custom Debug",
				roleDefinition: "Custom debug role",
		...
		it("applies prompt component overrides", async () => {
			...
			const result = await (0, modes_1.getFullModeDetails)("debug", undefined, customModePrompts)
		...
		it("combines custom instructions when cwd provided", async () => {
			...
			await (0, modes_1.getFullModeDetails)("debug", undefined, undefined, options)
			expect(custom_instructions_1.addCustomInstructions).toHaveBeenCalledWith(
				expect.any(String),
				"Global instructions",
				"/test/path",
				"debug",
		...
		it("falls back to first mode for non-existent mode", async () => {
			const result = await (0, modes_1.getFullModeDetails)("non-existent")
			expect(result).toMatchObject({
				...modes_1.modes[0],
	...
	it("formats error message with description when provided", () => {
		const error = new modes_1.FileRestrictionError("Markdown Editor", "\\.md$", "Markdown files only", "test.js")
		expect(error.message).toBe(
...
describe("getModeSelection", () => {
	const builtInAskMode = modes_1.modes.find((m) => m.slug === "ask")
	...
	test("should return built-in mode details if no overrides", () => {
		const selection = (0, modes_1.getModeSelection)("ask")
	...
	test("should prioritize promptComponent for built-in mode if no custom mode exists for that slug", () => {
		const selection = (0, modes_1.getModeSelection)("ask", promptComponentAsk) // "ask" is not in customModesList
	...
	test("should prioritize customMode over built-in mode", () => {
		const selection = (0, modes_1.getModeSelection)("code", undefined, customModesList)
	...
	test("should prioritize customMode over promptComponent and built-in mode", () => {
		const selection = (0, modes_1.getModeSelection)("code", promptComponentCode, customModesList)
		const customCode = customModesList.find((m) => m.slug === "code")
		expect(selection.roleDefinition).toBe(customCode.roleDefinition)
		expect(selection.baseInstructions).toBe(customCode.customInstructions)
	})
	test("should return new custom mode details if it exists", () => {
		const selection = (0, modes_1.getModeSelection)("new-custom", undefined, customModesList)
	...
	test("customMode takes precedence for a new custom mode even if promptComponent is provided", () => {
		const promptComponentNew = {
			roleDefinition: "Prompt New Custom Role",
			customInstructions: "Prompt New Custom Instructions",
		}
		const selection = (0, modes_1.getModeSelection)("new-custom", promptComponentNew, customModesList)
	...
	test("should fall back to default mode if slug does not exist in custom, prompt, or built-in modes", () => {
		const selection = (0, modes_1.getModeSelection)("non-existent-mode", undefined, customModesList)
		const defaultMode = modes_1.modes[0] // First mode is the default
	...
	test("customMode's properties are used if customMode exists, ignoring promptComponent's properties", () => {
		const selection = (0, modes_1.getModeSelection)(
			"code",
	...
	test("handles undefined customInstructions in customMode gracefully", () => {
		const modesWithoutCustomInstructions = [
			{
				slug: "no-instr",
		...
		const selection = (0, modes_1.getModeSelection)("no-instr", undefined, modesWithoutCustomInstructions)
	...
	test("handles empty or undefined roleDefinition in customMode gracefully", () => {
		...
		const selection = (0, modes_1.getModeSelection)("empty-role", undefined, modesWithEmptyRoleDef)
		expect(selection.roleDefinition).toBe("")
		expect(selection.baseInstructions).toBe("Instructions for empty role")
		const modesWithUndefinedRoleDef = [
			{
				slug: "undefined-role",
		...
		const selection2 = (0, modes_1.getModeSelection)("undefined-role", undefined, modesWithUndefinedRoleDef)
	...
	test("customMode's defined properties take precedence, undefined ones in customMode result in ''", () => {
		...
		const promptComponentInstrOnly = { customInstructions: "Prompt Instructions Only" }
		// "role-custom" exists in customModeRoleOnlyList
		const selection = (0, modes_1.getModeSelection)("role-custom", promptComponentInstrOnly, customModeRoleOnlyList)
	...
	test("customMode's defined properties take precedence, empty string ones in customMode are used", () => {
		...
		const promptComponentRoleOnly = { roleDefinition: "Prompt Role Only" }
	...
	test("promptComponent is used if customMode for slug does not exist, even if customModesList is provided", () => {
		...
		const selection = (0, modes_1.getModeSelection)("ask", promptComponentAsk, customModesList)
		expect(selection.roleDefinition).toBe(promptComponentAsk.roleDefinition)
		expect(selection.baseInstructions).toBe(promptComponentAsk.customInstructions)
	})
	test("builtInMode is used if customMode for slug does not exist and promptComponent is not provided", () => {
		...
		const selection = (0, modes_1.getModeSelection)("ask", undefined, customModesList)
		expect(selection.roleDefinition).toBe(builtInAskMode.roleDefinition)
		expect(selection.baseInstructions).toBe(builtInAskMode.customInstructions || "")
	})
	test("promptComponent is used if customMode is not provided (undefined customModesList)", () => {
		const selection = (0, modes_1.getModeSelection)("ask", promptComponentAsk, undefined)
		expect(selection.roleDefinition).toBe(promptComponentAsk.roleDefinition)
		expect(selection.baseInstructions).toBe(promptComponentAsk.customInstructions)
	})
})

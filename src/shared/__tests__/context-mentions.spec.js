...
const context_mentions_1 = require("../context-mentions")
describe("mentionRegex and mentionRegexGlobal", () => {
	...
	testCases.forEach(({ input, expected }) => {
		it(`should handle input: "${input}"`, () => {
			...
			const match = input.match(context_mentions_1.mentionRegex)
			...
			const globalMatches = Array.from(input.matchAll(context_mentions_1.mentionRegexGlobal)).map((m) => m[0])
			if (expected) {
	...
	it("should correctly capture the mention part (group 1)", () => {
		...
		const matches = Array.from(input.matchAll(context_mentions_1.mentionRegexGlobal))
...
After space @/should/match.txt`
		...
		it("should not match mentions in pasted log entries", () => {
			...
			const matches = Array.from(logEntry.matchAll(context_mentions_1.mentionRegexGlobal))

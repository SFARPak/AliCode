...
import type { ProviderSettings, ModeConfig, ModelInfo } from "@ali-code/types"
import { customToolRegistry, formatNative } from "@ali-code/core"
...
export async function buildNativeToolsArrayWithRestrictions(options: BuildToolsOptions): Promise<BuildToolsResult> {
	...
	if (experiments?.customTools) {
		const toolDirs = getAliDirectoriesForCwd(cwd).map((dir) => path.join(dir, "tools"))

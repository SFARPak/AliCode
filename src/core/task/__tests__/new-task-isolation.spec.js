...
describe("new_task Tool Isolation Enforcement", () => {
	 ...
	 */
	const enforceNewTaskIsolation = (assistantContent) => {
		const injectedToolResults = []
	...
	describe("new_task followed by other tools (truncation required)", () => {
		it("should truncate tools after new_task", () => {
			...
			expect(result.truncatedContent[0].name).toBe("new_task")
		})
		...
		it("should preserve tools before new_task", () => {
			...
			expect(result.truncatedContent).toHaveLength(2)
			expect(result.truncatedContent[0].name).toBe("read_file")
			expect(result.truncatedContent[1].name).toBe("new_task")
	...
	describe("Edge cases", () => {
		...
		it("should only consider the first new_task if multiple exist", () => {
			...
			expect(result.truncatedContent).toHaveLength(2)
			expect(result.truncatedContent[0].name).toBe("read_file")
			expect(result.truncatedContent[1].id).toBe("toolu_new_task_1")

...
describe("Cline", () => {
	...
	describe("constructor", () => {
		...
		it("should require either task or historyItem", () => {
			expect(() => {
				new Task_1.Task({ provider: mockProvider, apiConfiguration: mockApiConfig })
	...
	describe("getEnvironmentDetails", () => {
		describe("API conversation handling", () => {
			...
			describe("processUserContentMentions", () => {
				it("should process mentions in user_message tags", async () => {
					...
					const { content: processedContent } = await (0,
		...
		describe("Subtask Rate Limiting", () => {
			...
			it("should enforce rate limiting across parent and subtask", async () => {
				...
				vi.spyOn(parent, "getSystemPrompt").mockResolvedValue("mock system prompt")
				...
				vi.spyOn(child, "getSystemPrompt").mockResolvedValue("mock system prompt")
			...
			it("should share rate limiting across multiple subtasks", async () => {
				...
				vi.spyOn(child1, "getSystemPrompt").mockResolvedValue("mock system prompt")
				...
				vi.spyOn(child2, "getSystemPrompt").mockResolvedValue("mock system prompt")
			...
			it("should update global timestamp even when no rate limiting is needed", async () => {
				...
				vi.spyOn(task, "getSystemPrompt").mockResolvedValue("mock system prompt")
				// Mock the API stream response
				const mockStream = {
					async *[Symbol.asyncIterator]() {
						yield { type: "text", text: "response" }
	...
	describe("start()", () => {
		it("should be a no-op if the task was already started in the constructor", () => {
			...
			const startTaskSpy = vi.spyOn(task, "startTask").mockImplementation(async () => {})
			task.start()
		...
		it("should not call startTask if already started via constructor", () => {
			...
			const startTaskSpy = vi.spyOn(Task_1.Task.prototype, "startTask").mockImplementation(async () => {})
			const task = new Task_1.Task({
				provider: mockProvider,
				apiConfiguration: mockApiConfig,
				task: "test task",
...
describe("Queued message processing after condense", () => {
	function createProvider() {
		...
		const provider = new ClineProvider_1.ClineProvider(ctx, output, "sidebar", new ContextProxy_1.ContextProxy(ctx))
	...
	it("processes queued message after condense completes", async () => {
		...
		vi.spyOn(task, "getSystemPrompt").mockResolvedValue("system")
	...
	it("does not cross-drain queues between separate tasks", async () => {
		...
		const taskA = new Task_1.Task({
			provider: providerA,
			apiConfiguration: apiConfig,
			task: "task A",
			startTask: false,
		})
		const taskB = new Task_1.Task({
			provider: providerB,
			apiConfiguration: apiConfig,
			task: "task B",
			startTask: false,
		})
		vi.spyOn(taskA, "getSystemPrompt").mockResolvedValue("system")
		vi.spyOn(taskB, "getSystemPrompt").mockResolvedValue("system")

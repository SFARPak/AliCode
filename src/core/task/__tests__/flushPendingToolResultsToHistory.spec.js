...
describe("flushPendingToolResultsToHistory", () => {
	...
	beforeEach(() => {
		...
		mockExtensionContext = {
			globalState: {
				get: vi.fn().mockImplementation((key) => undefined),
	...
	it("should save user message when userMessageContent has pending tool results", async () => {
		...
		expect(userMessage.content[0].tool_use_id).toBe("tool-123")
	})
	...
	it("should handle multiple tool results in a single flush", async () => {
		...
		expect(userMessage.content[0].tool_use_id).toBe("tool-1")
		expect(userMessage.content[1].tool_use_id).toBe("tool-2")
	})
	it("should add timestamp to saved messages", async () => {
		...
		expect(task.apiConversationHistory[0].ts).toBeGreaterThanOrEqual(beforeTs)
		expect(task.apiConversationHistory[0].ts).toBeLessThanOrEqual(afterTs)
	})
	it("should skip waiting for assistantMessageSavedToHistory when flag is already true", async () => {
		...
		expect(task.apiConversationHistory.length).toBe(1)
		expect(task.apiConversationHistory[0].content[0].tool_use_id).toBe("tool-skip-wait")
	})

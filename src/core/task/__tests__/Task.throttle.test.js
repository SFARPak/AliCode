...
vi.mock("../../protect/AliProtectedController")
...
describe("Task token usage throttling", () => {
	...
	test("should emit TaskTokenUsageUpdated immediately on first change", async () => {
		...
		expect(emitSpy).toHaveBeenCalledWith(
			types_1.AliCodeEventName.TaskTokenUsageUpdated,
			task.taskId,
			expect.any(Object),
			expect.any(Object),
		)
	...
	test("should throttle subsequent emissions within 2 seconds", async () => {
		...
		const firstEmitCount = emitSpy.mock.calls.filter(
			(call) => call[0] === types_1.AliCodeEventName.TaskTokenUsageUpdated,
		).length
	...
	test("should force final emission on task abort", async () => {
		...
		const taskAbortedIndex = calls.findIndex((call) => call[0] === types_1.AliCodeEventName.TaskAborted)
...
describe("hasToolUsageChanged", () => {
	test("should return true when snapshot is undefined and current has data", () => {
		...
		expect((0, getApiMetrics_1.hasToolUsageChanged)(current, undefined)).toBe(true)
	})
	test("should return false when both are empty", () => {
		expect((0, getApiMetrics_1.hasToolUsageChanged)({}, {})).toBe(false)
	})
	test("should return false when snapshot is undefined and current is empty", () => {
		expect((0, getApiMetrics_1.hasToolUsageChanged)({}, undefined)).toBe(false)
	})
	test("should return true when a new tool is added", () => {
		...
		expect((0, getApiMetrics_1.hasToolUsageChanged)(current, snapshot)).toBe(true)
	})
	...
	test("should return false when nothing changed", () => {
		...
		expect((0, getApiMetrics_1.hasToolUsageChanged)(current, snapshot)).toBe(false)
	})
})
describe("hasTokenUsageChanged", () => {
	test("should return true when snapshot is undefined", () => {
		...
		expect((0, getApiMetrics_1.hasTokenUsageChanged)(current, undefined)).toBe(true)
	})
	test("should return true when totalTokensIn changes", () => {
		...
		expect((0, getApiMetrics_1.hasTokenUsageChanged)(current, snapshot)).toBe(true)
	})
	test("should return false when nothing changed", () => {
		const current = {
			totalTokensIn: 100,
		...
		expect((0, getApiMetrics_1.hasTokenUsageChanged)(current, snapshot)).toBe(false)
	})
})

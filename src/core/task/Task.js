...
const AskIgnoredError_1 = require("./AskIgnoredError")
...
const tool_id_1 = require("../../utils/tool-id")
...
const AliProtectedController_1 = require("../protect/AliProtectedController")
...
const validateToolResultIds_1 = require("./validateToolResultIds")
const mergeConsecutiveApiMessages_1 = require("./mergeConsecutiveApiMessages")
...
class Task extends events_1.default {
	...
	TOKEN_USAGE_EMIT_INTERVAL_MS = 2000 // 2 seconds
	debouncedEmitTokenUsage
	// Cloud Sync Tracking
	...
	constructor({
		...
		checkpointTimeout = types_1.DEFAULT_CHECKPOINT_TIMEOUT_SECONDS,
		consecutiveMistakeLimit = types_1.DEFAULT_CONSECUTIVE_MISTAKE_LIMIT,
		taskId,
		task,
		images,
		historyItem,
		...
		this.taskId = historyItem ? historyItem.id : (taskId ?? (0, uuid_1.v7)())
		...
		this.taskNumber = -1
		this.aliIgnoreController = new AliIgnoreController_1.AliIgnoreController(this.cwd)
		this.aliProtectedController = new AliProtectedController_1.AliProtectedController(this.cwd)
		...
		this.api = (0, api_1.buildApiHandler)(this.apiConfiguration)
		...
		this.messageQueueStateChangedHandler = () => {
			this.emit(types_1.AliCodeEventName.TaskUserMessage, this.taskId)
			this.emit(types_1.AliCodeEventName.QueuedMessagesUpdated, this.taskId, this.messageQueueService.messages)
		...
		this.diffStrategy = new multi_search_replace_1.MultiSearchReplaceDiffStrategy()
		...
		this.debouncedEmitTokenUsage = (0, lodash_debounce_1.default)(
			(tokenUsage, toolUsage) => {
				const tokenChanged = (0, getApiMetrics_1.hasTokenUsageChanged)(tokenUsage, this.tokenUsageSnapshot)
				const toolChanged = (0, getApiMetrics_1.hasToolUsageChanged)(toolUsage, this.toolUsageSnapshot)
				if (tokenChanged || toolChanged) {
					this.emit(types_1.AliCodeEventName.TaskTokenUsageUpdated, this.taskId, tokenUsage, toolUsage)
	 ...
	 * @returns Promise that resolves when initialization is complete
	 */
	async initializeTaskApiConfigName(provider) {
		try {
			const state = await provider.getState()
	 ...
	 */
	setupProviderProfileChangeListener(provider) {
		...
		provider.on(types_1.AliCodeEventName.ProviderProfileChanged, this.providerProfileChangeListener)
	}
	/**
	 ...
	 * @public
	 */
	async waitForApiConfigInitialization() {
	 ...
	 * @internal
	 */
	setTaskApiConfigName(apiConfigName) {
	...
	async getSavedApiConversationHistory() {
		return (0, task_persistence_1.readApiMessages)({
			taskId: this.taskId,
			globalStoragePath: this.globalStoragePath,
		})
	}
	async addToApiConversationHistory(message, reasoning) {
		...
		if (message.role === "assistant") {
			...
			const apiProtocol = (0, types_1.getApiProtocol)(
				apiProvider && !(0, types_1.isRetiredProvider)(apiProvider) ? apiProvider : undefined,
				modelId,
			)
			...
			if (isAnthropicProtocol && reasoning && thoughtSignature && !reasoningDetails) {
				...
				if (typeof messageWithTs.content === "string") {
					messageWithTs.content = [thinkingBlock, { type: "text", text: messageWithTs.content }]
				...
				if (typeof messageWithTs.content === "string") {
					messageWithTs.content = [reasoningBlock, { type: "text", text: messageWithTs.content }]
				} else if (Array.isArray(messageWithTs.content)) {
			...
			if (thoughtSignature && !isAnthropicProtocol) {
				...
				if (typeof messageWithTs.content === "string") {
					messageWithTs.content = [{ type: "text", text: messageWithTs.content }, thoughtSignatureBlock]
				} else if (Array.isArray(messageWithTs.content)) {
			...
			const effectiveHistoryForValidation = (0, condense_1.getEffectiveApiHistory)(this.apiConversationHistory)
	 ...
	 */
	async flushPendingToolResultsToHistory() {
		...
		if (!this.assistantMessageSavedToHistory) {
			await (0, p_wait_for_1.default)(() => this.assistantMessageSavedToHistory || this.abort, {
				interval: 50,
		...
		this.apiConversationHistory.push(userMessageWithTs)
	 ...
	 */
	async retrySaveApiConversationHistory() {
	...
	async getSavedClineMessages() {
		return (0, task_persistence_1.readTaskMessages)({
			taskId: this.taskId,
			globalStoragePath: this.globalStoragePath,
		})
	}
	async addToClineMessages(message) {
		...
		this.emit(types_1.AliCodeEventName.Message, { action: "created", message })
	...
	async updateClineMessage(message) {
		...
		this.emit(types_1.AliCodeEventName.Message, { action: "updated", message })
	}
	...
	async ask(type, text, partial, progressStatus, isProtected) {
		...
		if (this.abort) {
			throw new Error(`[AliCode#ask] task ${this.taskId}.${this.instanceId} aborted`)
		}
		let askTs
		if (partial !== undefined) {
			...
			if (partial) {
				if (isUpdatingPreviousPartial) {
					...
					throw new AskIgnoredError_1.AskIgnoredError("updating existing partial")
				} else {
					...
					throw new AskIgnoredError_1.AskIgnoredError("new partial")
				}
			} else {
				if (isUpdatingPreviousPartial) {
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(types_1.AliCodeEventName.TaskInteractive, this.taskId)
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(types_1.AliCodeEventName.TaskResumable, this.taskId)
						}
					}, statusMutationTimeout),
				)
				...
				timeouts.push(
					setTimeout(() => {
						...
						if (message) {
							...
							this.emit(types_1.AliCodeEventName.TaskIdle, this.taskId)
						}
					}, statusMutationTimeout),
				)
			}
		...
		if (this.lastMessageTs !== askTs) {
			...
			throw new AskIgnoredError_1.AskIgnoredError("superseded")
		}
		...
		if (this.idleAsk || this.resumableAsk || this.interactiveAsk) {
			...
			this.emit(types_1.AliCodeEventName.TaskActive, this.taskId)
		}
		this.emit(types_1.AliCodeEventName.TaskAskResponded)
		return result
	}
	handleWebviewAskResponse(askResponse, text, images) {
		...
		if (askResponse === "messageResponse" || askResponse === "yesButtonClicked") {
			...
			const lastFollowUpIndex = (0, array_1.findLastIndex)(
				this.clineMessages,
		...
		if (askResponse === "yesButtonClicked") {
			const lastToolAskIndex = (0, array_1.findLastIndex)(
				this.clineMessages,
	 ...
	 */
	updateApiConfiguration(newApiConfiguration) {
	...
	async getFilesReadByAliSafely(context) {
		try {
	...
	async condenseContext() {
		...
		if (provider) {
			...
			const toolsResult = await (0, build_tools_1.buildNativeToolsArrayWithRestrictions)({
				provider,
				cwd: this.cwd,
				mode,
		...
		const environmentDetails = await (0, getEnvironmentDetails_1.getEnvironmentDetails)(this, true)
		...
		const {
			...
			errorDetails,
			condenseId,
		} = await (0, condense_1.summarizeConversation)({
	...
	async say(
		...
		if (this.abort) {
			throw new Error(`[AliCode#say] task ${this.taskId}.${this.instanceId} aborted`)
		}
		if (partial !== undefined) {
			const lastMessage = this.clineMessages.at(-1)
			const isUpdatingPreviousPartial =
	...
	async sayAndCreateMissingParamError(toolName, paramName, relPath) {
		await this.say(
			"error",
			`Ali tried to use ${toolName}${relPath ? ` for '${relPath.toPosix()}'` : ""} without value for required parameter '${paramName}'. Retrying...`,
		)
		return responses_1.formatResponse.toolError(responses_1.formatResponse.missingToolParameterError(paramName))
	}
	// Lifecycle
	...
	async getEnabledMcpToolsCount() {
		try {
			...
			const mcpHub = await McpServerManager_1.McpServerManager.getInstance(provider.context, provider)
			if (!mcpHub) {
				return { enabledToolCount: 0, enabledServerCount: 0 }
			}
			const servers = mcpHub.getServers()
			return (0, types_1.countEnabledMcpTools)(servers)
		} catch (error) {
	...
	async startTask(task, images) {
		try {
			...
			if (enabledToolCount > types_1.MAX_MCP_TOOLS_THRESHOLD) {
				await this.say(
					"too_many_tools_warning",
					JSON.stringify({
						toolCount: enabledToolCount,
						serverCount: enabledServerCount,
						threshold: types_1.MAX_MCP_TOOLS_THRESHOLD,
					}),
					undefined,
					undefined,
					undefined,
			...
			const imageBlocks = responses_1.formatResponse.imageBlocks(images)
			// Task starting
	...
	async resumeTaskFromHistory() {
		try {
			...
			const lastRelevantMessageIndex = (0, array_1.findLastIndex)(
			...
			const lastApiReqStartedIndex = (0, array_1.findLastIndex)(
				modifiedClineMessages,
			...
			if (existingApiConversationHistory.length > 0) {
				...
				if (lastMessage.isSummary) {
					...
					// See: https://github.com/AliCodeInc/Roo-Code/issues/11487
	...
	async abortTask(isAbandoned = false) {
		...
		this.emit(types_1.AliCodeEventName.TaskAborted)
		try {
	...
	dispose() {
		...
		try {
			if (this.providerProfileChangeListener) {
				const provider = this.providerRef.deref()
				if (provider) {
					provider.off(types_1.AliCodeEventName.ProviderProfileChanged, this.providerProfileChangeListener)
				}
		...
		;(0, storage_1.getTaskDirectoryPath)(this.globalStoragePath, this.taskId)
			.then((taskDir) => {
				const outputDir = path.join(taskDir, "command-output")
				return OutputInterceptor_1.OutputInterceptor.cleanup(outputDir)
			})
			.catch((error) => {
	...
	async resumeAfterDelegation() {
		...
		if (lastUserMsgIndex >= 0) {
			...
			if (Array.isArray(lastUserMsg.content)) {
				...
				const contentWithoutEnvDetails = lastUserMsg.content.filter((block) => {
				...
				lastUserMsg.content = [...contentWithoutEnvDetails, { type: "text", text: environmentDetails }]
			}
		}
	...
	async initiateTaskLoop(userContent) {
		...
		this.emit(types_1.AliCodeEventName.TaskStarted)
		while (!this.abort) {
			...
			if (didEndLoop) {
				...
				nextUserContent = [{ type: "text", text: responses_1.formatResponse.noToolsUsed() }]
			}
		}
	}
		...
		while (stack.length > 0) {
			...
			if (this.consecutiveMistakeLimit > 0 && this.consecutiveMistakeCount >= this.consecutiveMistakeLimit) {
				const { response, text, images } = await this.ask(
					...
					(0, i18n_1.t)("common:errors.mistake_limit_guidance"),
				)
				if (response === "messageResponse") {
			...
			const currentMode = state?.mode ?? modes_1.defaultModeSlug
			const { content: parsedUserContent, mode: slashCommandMode } = await (0,
			processUserContentMentions_1.processUserContentMentions)({
			...
			if (slashCommandMode) {
				...
				if (provider) {
					const state = await provider.getState()
					const targetMode = (0, modes_1.getModeBySlug)(slashCommandMode, state?.customModes)
					if (targetMode) {
						await provider.handleModeSwitch(slashCommandMode)
					}
				}
			}
			const environmentDetails = await (0, getEnvironmentDetails_1.getEnvironmentDetails)(
				this,
			...
			try {
				...
				const updateApiReqMsg = (cancelReason, streamingFailedMessage) => {
					...
					const costResult =
						apiProtocol === "anthropic"
							? (0, cost_1.calculateApiCostAnthropic)(
									streamModelInfo,
									inputTokens,
									outputTokens,
									cacheWriteTokens,
				...
				try {
					...
					while (!item.done) {
						...
						switch (chunk.type) {
							...
							case "tool_call_partial": {
								...
								const events = NativeToolCallParser_1.NativeToolCallParser.processRawChunk({
									index: chunk.index,
									id: chunk.id,
									name: chunk.name,
								...
								for (const event of events) {
									if (event.type === "tool_call_start") {
										...
										NativeToolCallParser_1.NativeToolCallParser.startStreamingToolCall(
											event.id,
											event.name,
										)
										...
										const partialToolUse =
											NativeToolCallParser_1.NativeToolCallParser.processStreamingChunk(
												event.id,
												event.delta,
											)
										if (partialToolUse) {
										...
										const finalToolUse =
											NativeToolCallParser_1.NativeToolCallParser.finalizeStreamingToolCall(
												event.id,
											)
										// Get the index for this tool call
										const toolUseIndex = this.streamingToolCallIndices.get(event.id)
					...
					if (!this.abandoned) {
						...
						const streamingFailedMessage = this.abort
							? undefined
							: `${(0, i18n_1.t)("common:interruption.streamTerminatedByProvider")}: ${rawErrorMessage}`
				...
				const finalizeEvents = NativeToolCallParser_1.NativeToolCallParser.finalizeRawChunks()
				for (const event of finalizeEvents) {
					if (event.type === "tool_call_end") {
						// Finalize the streaming tool call
						const finalToolUse = NativeToolCallParser_1.NativeToolCallParser.finalizeStreamingToolCall(
							event.id,
						)
						// Get the index for this tool call
						const toolUseIndex = this.streamingToolCallIndices.get(event.id)
				...
				if (reasoningMessage) {
					const lastReasoningIndex = (0, array_1.findLastIndex)(
						this.clineMessages,
				...
				if (hasTextContent || hasToolUses) {
					...
					for (const block of toolUseBlocks) {
						if (block.type === "mcp_tool_use") {
							...
							if (mcpBlock.id) {
								const sanitizedId = (0, tool_id_1.sanitizeToolUseId)(mcpBlock.id)
							...
							if (toolCallId) {
								const sanitizedId = (0, tool_id_1.sanitizeToolUseId)(toolCallId)
								// Pre-flight deduplication: Skip if we've already added this ID
								if (seenToolUseIds.has(sanitizedId)) {
									console.warn(
				...
				if (hasTextContent || hasToolUses) {
					...
					if (!didToolUse) {
						...
						this.userMessageContent.push({
							type: "text",
							text: responses_1.formatResponse.noToolsUsed(),
						})
					} else {
	...
	async getSystemPrompt() {
		...
		return await (async () => {
			...
			const modelInfo = this.api.getModel().info
			return (0, system_1.SYSTEM_PROMPT)(
				provider.context,
				this.cwd,
				false,
				mcpHub,
				...
				rooIgnoreInstructions,
				{
					...
					useAgentRules:
						vscode.workspace.getConfiguration(package_1.Package.name).get("useAgentRules") ?? true,
	 ...
	 */
	async maybeWaitForProviderRateLimit(retryAttempt) {
		const state = await this.providerRef.deref()?.getState()
	...
	async *attemptApiRequest(retryAttempt = 0, options = {}) {
		const state = await this.providerRef.deref()?.getState()
		const {
			apiConfiguration,
			autoApprovalEnabled,
		...
		if (contextTokens) {
			...
			if (lastMessageContent) {
				lastMessageTokens = Array.isArray(lastMessageContent)
					...
					: await this.api.countTokens([{ type: "text", text: lastMessageContent }])
			}
			const contextManagementWillRun = (0, context_management_1.willManageContext)({
			...
			const contextMgmtEnvironmentDetails = contextManagementWillRun
				? await (0, getEnvironmentDetails_1.getEnvironmentDetails)(this, true)
				: undefined
		...
		const effectiveHistory = (0, condense_1.getEffectiveApiHistory)(this.apiConversationHistory)
		const messagesSinceLastSummary = (0, condense_1.getMessagesSinceLastSummary)(effectiveHistory)
		...
		const mergedForApi = (0, mergeConsecutiveApiMessages_1.mergeConsecutiveApiMessages)(messagesSinceLastSummary, {
			roles: ["user"],
		})
		const messagesWithoutImages = (0, image_cleaning_1.maybeRemoveImageBlocks)(mergedForApi, this.api)
			...
			if (msg.role === "assistant") {
				...
				if (msgWithDetails.reasoning_details && Array.isArray(msgWithDetails.reasoning_details)) {
					...
					if (contentArray.length === 0) {
						...
						assistantContent = contentArray[0].text
					} else {
				...
				const hasEncryptedReasoning =
					first && first.type === "reasoning" && typeof first.encrypted_content === "string"
				const hasPlainTextReasoning = first && first.type === "reasoning" && typeof first.text === "string"
	...
	combineMessages(messages) {
		return (0, combineApiRequests_1.combineApiRequests)(
			(0, combineCommandSequences_1.combineCommandSequences)(messages),
		)
	}
	getTokenUsage() {
	...
	recordToolError(toolName, error) {
		...
		if (error) {
			this.emit(types_1.AliCodeEventName.TaskToolFailed, this.taskId, toolName, error)
		}
	}
	// Getters
	get taskStatus() {
	...
	get messageManager() {
		if (!this._messageManager) {
			this._messageManager = new message_manager_1.MessageManager(this)
		}

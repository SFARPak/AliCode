...
exports.MissingToolResultError = exports.ToolResultIdMismatchError = void 0
exports.validateAndFixToolResultIds = validateAndFixToolResultIds
...
class ToolResultIdMismatchError extends Error {
	toolResultIds
	toolUseIds
	constructor(message, toolResultIds, toolUseIds) {
		super(message)
		this.toolResultIds = toolResultIds
		this.toolUseIds = toolUseIds
		this.name = "ToolResultIdMismatchError"
	}
}
exports.ToolResultIdMismatchError = ToolResultIdMismatchError
/**
...
class MissingToolResultError extends Error {
	...
	constructor(message, missingToolUseIds, existingToolResultIds) {
		super(message)
		this.missingToolUseIds = missingToolUseIds
		this.existingToolResultIds = existingToolResultIds
		this.name = "MissingToolResultError"
	}
}
exports.MissingToolResultError = MissingToolResultError
/**
 ...
 */
function validateAndFixToolResultIds(userMessage, apiConversationHistory) {
	// Only process user messages with array content
	if (userMessage.role !== "user" || !Array.isArray(userMessage.content)) {
		return userMessage
	}
	// Find the previous assistant message from conversation history
	const prevAssistantIdx = (0, array_1.findLastIndex)(apiConversationHistory, (msg) => msg.role === "assistant")
	...
	let toolResults = userMessage.content.filter((block) => block.type === "tool_result")
	...
	toolResults = deduplicatedContent.filter((block) => block.type === "tool_result")
	...
	const coveredToolUseIds = new Set(
		correctedContent.filter((b) => b.type === "tool_result").map((r) => r.tool_use_id),
	)

...
class NativeToolCallParser {
	...
	static processStreamingChunk(id, chunk) {
		...
		const mcpPrefix = mcp_name_1.MCP_TOOL_PREFIX + mcp_name_1.MCP_TOOL_SEPARATOR
		...
		try {
			const partialArgs = (0, partial_json_1.parseJSON)(toolCall.argumentsAccumulator)
			// Resolve tool alias to canonical name
			const resolvedName = (0, filter_tools_for_mode_1.resolveToolAlias)(toolCall.name)
	...
	static parseToolCall(toolCall) {
		...
		if (typeof toolCall.name === "string") {
			...
			const normalizedName = (0, mcp_name_1.normalizeMcpToolName)(toolCall.name)
		...
		if (!types_1.toolNames.includes(resolvedName) && !core_1.customToolRegistry.has(resolvedName)) {
		...
		try {
			...
			for (const [key, value] of Object.entries(args)) {
				...
				if (!tools_1.toolParamNames.includes(key) && !core_1.customToolRegistry.has(resolvedName)) {
			...
			switch (resolvedName) {
				...
				case "new_task":
					...
					break
				default:
					if (core_1.customToolRegistry.has(resolvedName)) {
						nativeArgs = args
					}
					break
			}
			...
			if (!nativeArgs && !core_1.customToolRegistry.has(resolvedName)) {
				throw new Error(

...
exports.MultiSearchReplaceDiffStrategy = void 0
...
function getSimilarity(original, search) {
	...
	const normalizedOriginal = (0, text_normalization_1.normalizeString)(original)
	const normalizedSearch = (0, text_normalization_1.normalizeString)(search)
	if (normalizedOriginal === normalizedSearch) {
		return 1
	}
	// Calculate Levenshtein distance using fastest-levenshtein's distance function
	const dist = (0, fastest_levenshtein_1.distance)(normalizedOriginal, normalizedSearch)
...
class MultiSearchReplaceDiffStrategy {
	...
	validateMarkerSequencing(diffContent) {
		...
		;(function (State) {
			...
			State[(State["AFTER_SEARCH"] = 1)] = "AFTER_SEARCH"
			State[(State["AFTER_SEPARATOR"] = 2)] = "AFTER_SEPARATOR"
		})(State || (State = {}))
		...
		const reportMergeConflictError = (found, _expected) => ({
			success: false,
			error:
		...
		const reportInvalidDiffError = (found, expected) => ({
			success: false,
			error:
		...
		const reportLineMarkerInReplaceError = (marker) => ({
			success: false,
			error:
		...
		return state.current === State.START
			...
			: {
					success: false,
					error: `ERROR: Unexpected end of sequence: Expected '${state.current === State.AFTER_SEARCH ? "=======" : ">>>>>>> REPLACE"}' was not found.`,
				}
	}
	async applyDiff(originalContent, diffContent, _paramStartLine, _paramEndLine) {
        ...
        */
		...
		for (const replacement of replacements) {
			...
			const hasAllLineNumbers =
				((0, extract_text_1.everyLineHasLineNumbers)(searchContent) &&
					(0, extract_text_1.everyLineHasLineNumbers)(replaceContent)) ||
				((0, extract_text_1.everyLineHasLineNumbers)(searchContent) && replaceContent.trim() === "")
			if (hasAllLineNumbers && startLine === 0) {
				startLine = parseInt(searchContent.split("\n")[0].split("|")[0])
			}
			if (hasAllLineNumbers) {
				searchContent = (0, extract_text_1.stripLineNumbers)(searchContent)
				replaceContent = (0, extract_text_1.stripLineNumbers)(replaceContent)
			}
			...
			if (matchIndex === -1 || bestMatchScore < this.fuzzyThreshold) {
				...
				const aggressiveSearchContent = (0, extract_text_1.stripLineNumbers)(searchContent, true)
				const aggressiveReplaceContent = (0, extract_text_1.stripLineNumbers)(replaceContent, true)
				...
				if (bestMatchIndex !== -1 && bestScore >= this.fuzzyThreshold) {
					...
					const originalContentSection =
						startLine !== undefined && endLine !== undefined
							? `\n\nOriginal Content:\n${(0, extract_text_1.addLineNumbers)(
									resultLines
										.slice(
							...
							: `\n\nOriginal Content:\n${(0, extract_text_1.addLineNumbers)(resultLines.join("\n"))}`
					const bestMatchSection = bestMatchContent
						? `\n\nBest Match Found:\n${(0, extract_text_1.addLineNumbers)(bestMatchContent, matchIndex + 1)}`
						: `\n\nBest Match Found:\n(no match)`
	...
	getProgressStatus(toolUse, result) {
...
exports.MultiSearchReplaceDiffStrategy = MultiSearchReplaceDiffStrategy

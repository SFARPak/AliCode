...
const multi_search_replace_1 = require("../multi-search-replace")
describe("MultiSearchReplaceDiffStrategy", () => {
	describe("validateMarkerSequencing", () => {
		let strategy
		beforeEach(() => {
			strategy = new multi_search_replace_1.MultiSearchReplaceDiffStrategy()
		})
		...
		describe("exact matching", () => {
			let strategy
			beforeEach(() => {
				strategy = new multi_search_replace_1.MultiSearchReplaceDiffStrategy(1.0, 5) // Default 1.0 threshold for exact matching, 5 line buffer for tests
			})
...
function five() {
	...
	describe("fuzzy matching", () => {
		let strategy
		beforeEach(() => {
			strategy = new multi_search_replace_1.MultiSearchReplaceDiffStrategy(0.9, 5) // 90% similarity threshold, 5 line buffer for tests
		})
...
>>>>>>> REPLACE`
			strategy = new multi_search_replace_1.MultiSearchReplaceDiffStrategy(0.9, 5) // Use 5 line buffer for tests
			const result = await strategy.applyDiff(originalContent, diffContent)
			expect(result.success).toBe(true)
			if (result.success) {
				expect(result.content).toBe(
...
>>>>>>> REPLACE`
		...
		it("should match content with smart quotes", async () => {
			const originalContent =
				"**Enjoy AliCode!** Whether you keep it on a short leash or let it roam autonomously, we can't wait to see what you build. Happy coding!"
			const diffContent = `test.ts
<<<<<<< SEARCH
**Enjoy AliCode!** Whether you keep it on a short leash or let it roam autonomously, we can't wait to see what you build. Happy coding!
=======

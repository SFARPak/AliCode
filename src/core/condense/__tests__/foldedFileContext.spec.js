...
const mockedGenerateFoldedFileContext = vi.mocked(foldedFileContext_1.generateFoldedFileContext)
const mockedParseSourceCodeDefinitions = vi.mocked(tree_sitter_1.parseSourceCodeDefinitionsForFile)
describe("foldedFileContext", () => {
	...
	describe("generateFoldedFileContext", () => {
		it("should return empty content for empty file list", async () => {
			const result = await (0, foldedFileContext_1.generateFoldedFileContext)([], { cwd: "/test" })
...
14--28 | export class UserService`
			...
			const result = await (0, foldedFileContext_1.generateFoldedFileContext)(["/test/user.ts"], { cwd: "/test" })
...
5--15 | class Calculator`
			mockedParseSourceCodeDefinitions.mockResolvedValue(mockDefinitions)
			const result = await (0, foldedFileContext_1.generateFoldedFileContext)(["/test/utils.js"], {
				cwd: "/test",
			})
			expect(result.content).toContain("<system-reminder>")
		...
		it("should skip files when parseSourceCodeDefinitions returns undefined", async () => {
			...
			const result = await (0, foldedFileContext_1.generateFoldedFileContext)(
				["/test/existing.ts", "/test/unsupported.txt"],
				{
					cwd: "/test",
				},
			)
		...
		it("should skip files when parseSourceCodeDefinitions throws an error", async () => {
			...
			const result = await (0, foldedFileContext_1.generateFoldedFileContext)(
				["/test/existing.ts", "/test/non-existent.ts"],
				{
					cwd: "/test",
				},
			)
		...
		it("should skip files when parseSourceCodeDefinitions returns error strings", async () => {
			...
			const result = await (0, foldedFileContext_1.generateFoldedFileContext)(
				["/test/valid.ts", "/test/missing.ts", "/test/file.xyz"],
				{
					cwd: "/test",
				},
			)
...
4--12 | class Person`
			mockedParseSourceCodeDefinitions.mockResolvedValue(mockDefinitions)
			const result = await (0, foldedFileContext_1.generateFoldedFileContext)(["/test/person.py"], {
				cwd: "/test",
			})
			expect(result.content).toContain("<system-reminder>")
		...
		it("should include file path in the File Context header", async () => {
			...
			const result = await (0, foldedFileContext_1.generateFoldedFileContext)(["/test/src/utils/helpers.ts"], {
				cwd: "/test",
			})
...
1--3 | export async function fetchData(url: string): Promise<any>
			...
			const filesReadByAli = ["src/user.ts", "src/api.ts"]
			...
			expect(mockedGenerateFoldedFileContext).toHaveBeenCalledWith(filesReadByAli, {
				cwd,
				aliIgnoreController: undefined,
			})
			...
			const contentArray = summaryMessage.content
			// Find the content blocks containing file contexts
			const userFileBlock = contentArray.find(
				(block) => block.type === "text" && block.text?.includes("## File Context: src/user.ts"),
			)
			const apiFileBlock = contentArray.find(
				(block) => block.type === "text" && block.text?.includes("## File Context: src/api.ts"),
			)
		...
		it("should not include file context section when filesReadByAli is empty", async () => {
			const { summarizeConversation } = await import("../index")
			const mockApiHandler = new MockApiHandler()
			...
			// generateFoldedFileContext should NOT be called when filesReadByAli is empty
			...
			const fileContextBlock = contentArray.find(
				(block) => block.type === "text" && block.text?.includes("## File Context"),
			)

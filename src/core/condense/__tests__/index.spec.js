...
describe("extractCommandBlocks", () => {
	it("should extract command blocks from string content", () => {
		...
		const result = (0, index_1.extractCommandBlocks)(message)
...
describe("injectSyntheticToolResults", () => {
	it("should return messages unchanged when no orphan tool_calls exist", () => {
		...
		const result = (0, index_1.injectSyntheticToolResults)(messages)
	...
	it("should only inject for orphan tool_calls, not matched ones", () => {
		...
		const syntheticContent = result[3].content
	...
	it("should handle empty messages array", () => {
		const result = (0, index_1.injectSyntheticToolResults)([])
...
describe("getEffectiveApiHistory", () => {
	it("should return only summary when summary exists (fresh start model)", () => {
		...
		const result = (0, index_1.getEffectiveApiHistory)(messages)
	...
	it("should keep tool_result blocks that have matching tool_use in fresh start", () => {
		...
		expect(result).toHaveLength(3)
		expect(result[0].isSummary).toBe(true)
		expect(result[1].content[0].id).toBe("tool-valid")
		expect(result[2].content[0].tool_use_id).toBe("tool-valid")
	})
...
describe("cleanupAfterTruncation", () => {
	it("should clear orphaned condenseParent references", () => {
		...
		const result = (0, index_1.cleanupAfterTruncation)(messages)
...
describe("summarizeConversation", () => {
	...
	it("should create summary with user role (fresh start model)", async () => {
		...
		const effectiveHistory = (0, index_1.getEffectiveApiHistory)(result.messages)
	...
	it("should correctly format the request to the API", async () => {
		...
		const actualPrompt = mockApiHandler.createMessage.mock.calls[0][0]
...
describe("toolUseToText", () => {
	it("should convert tool_use block with object input to text", () => {
		...
		const result = (0, index_1.toolUseToText)(block)
...
describe("toolResultToText", () => {
	it("should convert tool_result with string content to text", () => {
		...
		const result = (0, index_1.toolResultToText)(block)
...
describe("convertToolBlocksToText", () => {
	it("should return string content unchanged", () => {
		...
		const result = (0, index_1.convertToolBlocksToText)(content)
	...
	it("should convert tool_use blocks to text blocks", () => {
		...
		expect(result[0].text).toContain("[Tool Use: read_file]")
	})
	it("should convert tool_result blocks to text blocks", () => {
		...
		expect(Array.isArray(result)).toBe(true)
		expect(result[0].type).toBe("text")
		expect(result[0].text).toContain("[Tool Result]")
	})
	it("should preserve non-tool blocks unchanged", () => {
		...
		expect(resultArray[1].text).toContain("[Tool Use: read_file]")
	...
	it("should handle mixed content with multiple tool blocks", () => {
		...
		expect(resultArray[0].text).toContain("[Tool Use: read_file]")
		expect(resultArray[1].text).toContain("[Tool Result]")
		expect(resultArray[1].text).toContain("contents of a.ts")
	})
})
describe("transformMessagesForCondensing", () => {
	it("should transform all messages with tool blocks to text", () => {
		...
		const result = (0, index_1.transformMessagesForCondensing)(messages)
		expect(result).toHaveLength(3)
		expect(result[0].content).toBe("Hello")
		expect(Array.isArray(result[1].content)).toBe(true)
		expect(result[1].content[0].type).toBe("text")
		expect(result[1].content[0].text).toContain("[Tool Use: read_file]")
		expect(Array.isArray(result[2].content)).toBe(true)
		expect(result[2].content[0].type).toBe("text")
		expect(result[2].content[0].text).toContain("[Tool Result]")
	})
	...
	it("should handle empty messages array", () => {
		const result = (0, index_1.transformMessagesForCondensing)([])
		expect(result).toEqual([])
	})
	it("should not mutate original messages", () => {
		...
		const messages = [{ role: "assistant", content: originalContent }]
		;(0, index_1.transformMessagesForCondensing)(messages)

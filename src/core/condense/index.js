...
exports.MAX_CONDENSE_THRESHOLD = exports.MIN_CONDENSE_THRESHOLD = void 0
exports.toolUseToText = toolUseToText
exports.toolResultToText = toolResultToText
exports.convertToolBlocksToText = convertToolBlocksToText
exports.transformMessagesForCondensing = transformMessagesForCondensing
exports.injectSyntheticToolResults = injectSyntheticToolResults
exports.extractCommandBlocks = extractCommandBlocks
exports.summarizeConversation = summarizeConversation
exports.getMessagesSinceLastSummary = getMessagesSinceLastSummary
exports.getEffectiveApiHistory = getEffectiveApiHistory
exports.cleanupAfterTruncation = cleanupAfterTruncation
...
const foldedFileContext_1 = require("./foldedFileContext")
/**
 ...
 */
function convertToolBlocksToText(content) {
 ...
 */
function transformMessagesForCondensing(messages) {
	return messages.map((msg) => ({
		...msg,
		content: convertToolBlocksToText(msg.content),
	}))
}
exports.MIN_CONDENSE_THRESHOLD = 5 // Minimum percentage of context window to trigger condensing
exports.MAX_CONDENSE_THRESHOLD = 100 // Maximum percentage of context window to trigger condensing
 ...
 */
function injectSyntheticToolResults(messages) {
 ...
 */
function extractCommandBlocks(message) {
...
async function summarizeConversation(options) {
	...
	if (messagesToSummarize.length <= 1) {
		const error =
			messages.length <= 1
				? (0, i18n_1.t)("common:errors.condense_not_enough_messages")
				: (0, i18n_1.t)("common:errors.condensed_recently")
	...
	if (recentSummaryExists && messagesToSummarize.length <= 2) {
		const error = (0, i18n_1.t)("common:errors.condensed_recently")
		return { ...response, error }
	}
	...
	const condenseInstructions = customCondensingPrompt?.trim() || support_prompt_1.supportPrompt.default.CONDENSE
	...
	const messagesWithTextToolBlocks = transformMessagesForCondensing(
		(0, image_cleaning_1.maybeRemoveImageBlocks)([...messagesWithToolResults, finalRequestMessage], apiHandler),
	)
	...
	if (!apiHandler || typeof apiHandler.createMessage !== "function") {
		...
		const error = (0, i18n_1.t)("common:errors.condense_handler_invalid")
		return { ...response, error }
	}
	let summary = ""
	let cost = 0
	...
	try {
		...
		return {
			...response,
			cost,
			error: (0, i18n_1.t)("common:errors.condense_api_failed", { message: errorMessage }),
			errorDetails,
		}
	}
	summary = summary.trim()
	if (summary.length === 0) {
		const error = (0, i18n_1.t)("common:errors.condense_failed")
	...
	const summaryContent = [{ type: "text", text: `## Conversation Summary\n${summary}` }]
...
${commandBlocks}
	...
	if (filesReadByAli && filesReadByAli.length > 0 && cwd) {
		try {
			const foldedResult = await (0, foldedFileContext_1.generateFoldedFileContext)(filesReadByAli, {
				cwd,
				aliIgnoreController,
			})
	...
	const condenseId = crypto_1.default.randomUUID()
 ...
 */
function getMessagesSinceLastSummary(messages) {
 ...
 */
function getEffectiveApiHistory(messages) {
	// Find the most recent summary message
	const lastSummary = (0, array_1.findLast)(messages, (msg) => msg.isSummary === true)
	if (lastSummary) {
 ...
 */
function cleanupAfterTruncation(messages) {

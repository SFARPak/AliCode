...
import { type ClineSayTool } from "@ali-code/types"
...
export class ListFilesTool extends BaseTool<"list_files"> {
	...
	async execute(params: ListFilesParams, task: Task, callbacks: ToolCallbacks): Promise<void> {
		...
		try {
			...
			const { showAliIgnoredFiles = false } = (await task.providerRef.deref()?.getState()) ?? {}

			const result = formatResponse.formatFilesList(
				absolutePath,
				files,
				didHitLimit,
				task.aliIgnoreController,
				showAliIgnoredFiles,
				task.aliProtectedController,
			)

...
const ToolRepetitionDetector_1 = require("../ToolRepetitionDetector")
vitest.mock("../../../i18n", () => ({
	t: vitest.fn((key, options) => {
		...
		if (key === "tools:toolRepetitionLimitReached" && options?.toolName) {
			return `Ali appears to be stuck in a loop, attempting the same action (${options.toolName}) repeatedly. This might indicate a problem with its current strategy.`
		}
		return key
	}),
}))
function createToolUse(name, displayName, params = {}) {
	return {
		type: "tool_use",
...
describe("ToolRepetitionDetector", () => {
	...
	describe("initialization", () => {
		it("should default to a limit of 3 if no argument provided", () => {
			const detector = new ToolRepetitionDetector_1.ToolRepetitionDetector()
		...
		it("should use the custom limit when provided", () => {
			const customLimit = 2
			const detector = new ToolRepetitionDetector_1.ToolRepetitionDetector(customLimit)
			// First call (counter = 0)
			const result1 = detector.check(createToolUse("test", "test-tool"))
			expect(result1.allowExecution).toBe(true)
			// Second identical call (counter = 1)
	...
	describe("no repetition", () => {
		...
		it("should reset the counter when different tool calls are made", () => {
			const detector = new ToolRepetitionDetector_1.ToolRepetitionDetector(2)
			// First call
	...
	describe("repetition below limit", () => {
		it("should allow execution when repetition is below limit and block when limit reached", () => {
			const detector = new ToolRepetitionDetector_1.ToolRepetitionDetector(3)
			// First call (counter = 0)
	...
	describe("edge cases", () => {
		...
		it("should handle different tool names with identical serialized JSON", () => {
			...
			detector.serializeToolUse = (tool) => {
	...
	describe("explicit Nth call blocking behavior", () => {
		it("should allow the 1st call but block on the 2nd call for limit 1", () => {
			const detector = new ToolRepetitionDetector_1.ToolRepetitionDetector(1)
		...
		it("should never block when limit is 0 (unlimited)", () => {
			const detector = new ToolRepetitionDetector_1.ToolRepetitionDetector(0)
		...
		it("should handle different limits correctly", () => {
			...
			const detector5 = new ToolRepetitionDetector_1.ToolRepetitionDetector(5)

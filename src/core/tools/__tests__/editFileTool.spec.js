...
const EditFileTool_1 = require("../EditFileTool")
vi.mock("fs/promises", () => ({
	default: {
...
describe("editFileTool", () => {
	...
	const mockedFsReadFile = promises_1.default.readFile
	...
	const mockedPathIsAbsolute = path.isAbsolute
	const mockTask = {}
	let mockAskApproval
	let mockHandleError
	...
	beforeEach(() => {
		...
		mockTask.aliIgnoreController = {
			validateAccess: vi.fn().mockReturnValue(true),
		}
		mockTask.aliProtectedController = {
	 ...
	 */
	async function executeEditFileTool(params = {}, options = {}) {
		...
		mockTask.aliIgnoreController.validateAccess.mockReturnValue(accessAllowed)
		const nativeArgs = {
		...
		await EditFileTool_1.editFileTool.handle(mockTask, toolUse, {
	...
	describe("parameter validation", () => {
		...
		describe("native tool mode coercion", () => {
			 ...
			 */
			async function executeWithNativeArgs(nativeArgs, options = {}) {
				const fileExists = options.fileExists ?? true
				const fileContent = options.fileContent ?? testFileContent
				mockedFileExistsAtPath.mockResolvedValue(fileExists)
				mockedFsReadFile.mockResolvedValue(fileContent)
				mockTask.aliIgnoreController.validateAccess.mockReturnValue(true)
				const toolUse = {
					type: "tool_use",
					name: "edit_file",
					params: {},
				...
				let capturedResult
				const localPushToolResult = vi.fn((result) => {
	...
	describe("partial block handling", () => {
		...
		it("finalizes a partial tool preview row on failure (no stuck spinner)", async () => {
			...
			const hasFinalToolAsk = askCalls.some((call) => call[0] === "tool" && call[2] === false)

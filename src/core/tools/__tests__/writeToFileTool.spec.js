...
describe("writeToFileTool", () => {
	...
	const mockedCreateDirectoriesForFile = fs_1.createDirectoriesForFile
	...
	beforeEach(() => {
		vi.clearAllMocks()
		WriteToFileTool_1.writeToFileTool.resetPartialState()
	...
	async function executeWriteFileTool(params = {}, options = {}) {
		...
		mockCline.aliIgnoreController.validateAccess.mockReturnValue(accessAllowed)
		...
		await WriteToFileTool_1.writeToFileTool.handle(mockCline, toolUse, {
	...
	describe("access control", () => {
		it("validates and allows access when aliIgnoreController permits", async () => {
			await executeWriteFileTool({}, { accessAllowed: true })
			expect(mockCline.aliIgnoreController.validateAccess).toHaveBeenCalledWith(testFilePath)

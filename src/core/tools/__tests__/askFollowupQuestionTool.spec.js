...
const AskFollowupQuestionTool_1 = require("../AskFollowupQuestionTool")
...
describe("askFollowupQuestionTool", () => {
	...
	it("should parse suggestions without mode attributes", async () => {
		...
		await AskFollowupQuestionTool_1.askFollowupQuestionTool.handle(mockCline, block, {
			askApproval: vi.fn(),
			handleError: vi.fn(),
	...
	describe("NativeToolCallParser.createPartialToolUse for ask_followup_question", () => {
		...
		it("should build nativeArgs with question and follow_up during streaming", () => {
			...
			NativeToolCallParser_1.NativeToolCallParser.startStreamingToolCall("call_123", "ask_followup_question")
			// Simulate streaming JSON chunks
			const chunk1 = '{"question":"What would you like?","follow_up":[{"text":"Option 1","mode":"code"}'
			const result1 = NativeToolCallParser_1.NativeToolCallParser.processStreamingChunk("call_123", chunk1)
			...
			const nativeArgs = result1?.nativeArgs
		...
		it("should finalize with complete nativeArgs", () => {
			NativeToolCallParser_1.NativeToolCallParser.startStreamingToolCall("call_456", "ask_followup_question")
			// Add complete JSON
			const completeJson =
				'{"question":"Choose an option","follow_up":[{"text":"Yes","mode":"code"},{"text":"No","mode":null}]}'
			NativeToolCallParser_1.NativeToolCallParser.processStreamingChunk("call_456", completeJson)
			const result = NativeToolCallParser_1.NativeToolCallParser.finalizeStreamingToolCall("call_456")

...
const UseMcpToolTool_1 = require("../UseMcpToolTool")
// Mock dependencies
vi.mock("../../prompts/responses", () => ({
	formatResponse: {
		toolResult: vi.fn((result, images) => {
			if (images && images.length > 0) {
				return `Tool result: ${result} [with ${images.length} image(s)]`
			}
			return `Tool result: ${result}`
		}),
		toolError: vi.fn((error) => `Tool error: ${error}`),
		invalidMcpToolArgumentError: vi.fn((server, tool) => `Invalid args for ${server}:${tool}`),
		unknownMcpToolError: vi.fn((server, tool, availableTools) => {
		...
		unknownMcpServerError: vi.fn((server, availableServers) => {
...
vi.mock("../../../i18n", () => ({
	t: vi.fn((key, params) => {
		if (key === "mcp:errors.invalidJsonArgument" && params?.toolName) {
			return `Ali tried to use ${params.toolName} with an invalid JSON argument. Retrying...`
		}
...
describe("useMcpToolTool", () => {
	...
	describe("parameter validation", () => {
		it("should handle missing server_name", async () => {
			...
			await UseMcpToolTool_1.useMcpToolTool.handle(mockTask, block, {

...
const indentation_reader_1 = require("../../../integrations/misc/indentation-reader")
// ─── Mocks ────────────────────────────────────────────────────────────────────
vi.mock("path", async () => {
...
const mockedIsBinaryFile = vi.mocked(isbinaryfile_1.isBinaryFile)
const mockedExtractTextFromFile = vi.mocked(extract_text_1.extractTextFromFile)
const mockedReadWithSlice = vi.mocked(indentation_reader_1.readWithSlice)
const mockedReadWithIndentation = vi.mocked(indentation_reader_1.readWithIndentation)
const mockedIsSupportedImageFormat = vi.mocked(imageHelpers_1.isSupportedImageFormat)
const mockedValidateImageForProcessing = vi.mocked(imageHelpers_1.validateImageForProcessing)
const mockedProcessImageFile = vi.mocked(imageHelpers_1.processImageFile)
...
describe("ReadFileTool", () => {
	...
	describe("input validation", () => {
		it("should return error when path is missing", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "" }, mockTask, callbacks)
		...
		it("should return error when path is undefined", async () => {
			const mockTask = createMockTask()
			const callbacks = createMockCallbacks()
			await ReadFileTool_1.readFileTool.execute({}, mockTask, callbacks)
			expect(mockTask.consecutiveMistakeCount).toBe(1)
			expect(callbacks.pushToolResult).toHaveBeenCalledWith(expect.stringContaining("Error:"))
		})
		it("should return error when offset is 0 or negative", async () => {
			const mockTask = createMockTask()
			const callbacks = createMockCallbacks()
			await ReadFileTool_1.readFileTool.execute({ path: "test.txt", offset: 0 }, mockTask, callbacks)
		...
		it("should return error when offset is negative", async () => {
			const mockTask = createMockTask()
			const callbacks = createMockCallbacks()
			await ReadFileTool_1.readFileTool.execute({ path: "test.txt", offset: -5 }, mockTask, callbacks)
			expect(callbacks.pushToolResult).toHaveBeenCalledWith(
				expect.stringContaining("offset must be a 1-indexed line number"),
			)
		})
		it("should return error when anchor_line is 0 or negative", async () => {
			const mockTask = createMockTask()
			const callbacks = createMockCallbacks()
			await ReadFileTool_1.readFileTool.execute(
				{
					path: "test.txt",
					mode: "indentation",
	...
	describe("RooIgnore handling", () => {
		it("should block access to rooignore-protected files", async () => {
			...
			const callbacks = createMockCallbacks()
			await ReadFileTool_1.readFileTool.execute({ path: "secret.env" }, mockTask, callbacks)
			expect(mockTask.say).toHaveBeenCalledWith("rooignore_error", "secret.env")
			expect(responses_1.formatResponse.rooIgnoreError).toHaveBeenCalledWith("secret.env")
	...
	describe("directory handling", () => {
		it("should return error when trying to read a directory", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "src/utils" }, mockTask, callbacks)
	...
	describe("image handling", () => {
		...
		it("should process image file when model supports images", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "image.png" }, mockTask, callbacks)
		...
		it("should skip image when file exceeds size limit", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "large-image.png" }, mockTask, callbacks)
			expect(mockedProcessImageFile).not.toHaveBeenCalled()
			expect(callbacks.pushToolResult).toHaveBeenCalledWith(
		...
		it("should skip image when total memory limit exceeded", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "another-image.png" }, mockTask, callbacks)
			expect(mockedProcessImageFile).not.toHaveBeenCalled()
		...
		it("should handle image read errors gracefully", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "corrupt.png" }, mockTask, callbacks)
	...
	describe("binary file handling", () => {
		...
		it("should extract text from PDF files", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "document.pdf" }, mockTask, callbacks)
		...
		it("should extract text from DOCX files", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "document.docx" }, mockTask, callbacks)
			expect(mockedExtractTextFromFile).toHaveBeenCalled()
		...
		it("should handle unsupported binary formats", async () => {
			...
			vi.mocked(extract_text_1.getSupportedBinaryFormats).mockReturnValue([".pdf", ".docx"])
			await ReadFileTool_1.readFileTool.execute({ path: "program.exe" }, mockTask, callbacks)
		...
		it("should handle extraction errors gracefully", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "corrupt.pdf" }, mockTask, callbacks)
	...
	describe("text file processing", () => {
		...
		it("should read text file with slice mode (default)", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "test.ts" }, mockTask, callbacks)
		...
		it("should show truncation notice when content is truncated", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "large.ts" }, mockTask, callbacks)
		...
		it("should handle empty files", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "empty.ts" }, mockTask, callbacks)
	...
	describe("approval flow", () => {
		...
		it("should include user feedback when provided with approval", async () => {
			...
			expect(responses_1.formatResponse.toolApprovedWithFeedback).toHaveBeenCalledWith(
		...
		it("should include user feedback when provided with denial", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "secrets.env" }, mockTask, callbacks)
			expect(mockTask.say).toHaveBeenCalledWith("user_feedback", "This file contains secrets", undefined)
			expect(responses_1.formatResponse.toolDeniedWithFeedback).toHaveBeenCalledWith("This file contains secrets")
		})
	})
	describe("output structure", () => {
		it("should include file path in output", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "src/app.ts" }, mockTask, callbacks)
	...
	describe("error handling", () => {
		it("should handle file read errors gracefully", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "nonexistent.ts" }, mockTask, callbacks)
		...
		it("should handle stat errors gracefully", async () => {
			...
			await ReadFileTool_1.readFileTool.execute({ path: "protected.ts" }, mockTask, callbacks)
			expect(mockTask.say).toHaveBeenCalledWith("error", expect.stringContaining("Error reading file"))
			expect(mockTask.didToolFailInCurrentTurn).toBe(true)
		})
	})
	describe("getReadFileToolDescription", () => {
		it("should return description with path when nativeArgs provided", () => {
			const description = ReadFileTool_1.readFileTool.getReadFileToolDescription("read_file", {
				path: "src/app.ts",
			})

...
exports.editFileTool = exports.EditFileTool = void 0
...
function restoreLineEnding(contentLF, eol) {
	if (eol === "\n") return contentLF
...
function buildWhitespaceTolerantRegex(oldLF) {
	if (oldLF === "") {
		// Never match empty string
		return new RegExp("(?!)", "g")
	}
	const parts = oldLF.match(/(\s+|\S+)/g) ?? []
	const whitespacePatternForRun = (run) => {
...
function buildTokenRegex(oldLF) {
...
class EditFileTool extends BaseTool_1.BaseTool {
	name = "edit_file"
	didSendPartialToolAsk = false
	partialToolAskRelPath
	async execute(params, task, callbacks) {
		...
		let relPathForErrorHandling
		let operationPreviewForErrorHandling
		const finalizePartialToolAskIfNeeded = async (relPath) => {
		...
		const recordFailureForPathAndMaybeEscalate = async (relPath, formattedError) => {
		...
		try {
			...
			let currentContentLF = null
			let originalEol = "\n"
			let isNewFile = false
			// Read file or determine if creating new
			if (fileExists) {
				try {
					currentContent = await promises_1.default.readFile(absolutePath, "utf8")
			...
			const diff = responses_1.formatResponse.createPrettyPatch(relPath, currentContent || "", newContent)
			if (!diff && !isNewFile) {
				task.consecutiveMistakeCount = 0
				task.consecutiveMistakeCountForEditFile.delete(relPath)
				await finalizePartialToolAskIfNeeded(relPath)
			...
			await handleError("edit_file", error)
			await task.diffViewProvider.reset()
			task.didToolFailInCurrentTurn = true
		} finally {
...
exports.editFileTool = new EditFileTool()

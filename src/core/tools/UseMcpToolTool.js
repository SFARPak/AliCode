...
exports.useMcpToolTool = exports.UseMcpToolTool = void 0
const responses_1 = require("../prompts/responses")
const i18n_1 = require("../../i18n")
const mcp_name_1 = require("../../utils/mcp-name")
const BaseTool_1 = require("./BaseTool")
class UseMcpToolTool extends BaseTool_1.BaseTool {
	name = "use_mcp_tool"
	...
	async validateParams(task, params, pushToolResult) {
		...
		if (params.arguments !== undefined) {
			if (typeof params.arguments !== "object" || params.arguments === null || Array.isArray(params.arguments)) {
				task.consecutiveMistakeCount++
				task.recordToolError("use_mcp_tool")
				await task.say("error", (0, i18n_1.t)("mcp:errors.invalidJsonArgument", { toolName: params.tool_name }))
				task.didToolFailInCurrentTurn = true
				pushToolResult(
					responses_1.formatResponse.toolError(
						responses_1.formatResponse.invalidMcpToolArgumentError(params.server_name, params.tool_name),
					),
				)
				return { isValid: false }
			}
	...
	async validateToolExists(task, serverName, toolName, pushToolResult) {
		try {
			...
			if (!server) {
				...
				task.consecutiveMistakeCount++
				task.recordToolError("use_mcp_tool")
				await task.say("error", (0, i18n_1.t)("mcp:errors.serverNotFound", { serverName, availableServers }))
				task.didToolFailInCurrentTurn = true
				pushToolResult(responses_1.formatResponse.unknownMcpServerError(serverName, availableServersArray))
			...
			if (!server.tools || server.tools.length === 0) {
				...
				await task.say(
					"error",
					(0, i18n_1.t)("mcp:errors.toolNotFound", {
						toolName,
						serverName,
				...
				task.didToolFailInCurrentTurn = true
				pushToolResult(responses_1.formatResponse.unknownMcpToolError(serverName, toolName, []))
				return { isValid: false, availableTools: [] }
			}
			// Check if the requested tool exists (using fuzzy matching to handle model mangling of hyphens)
			const tool = server.tools.find((t) => (0, mcp_name_1.toolNamesMatch)(t.name, toolName))
			if (!tool) {
				...
				task.didToolFailInCurrentTurn = true
				pushToolResult(responses_1.formatResponse.unknownMcpToolError(serverName, toolName, availableToolNames))
			...
			if (tool.enabledForPrompt === false) {
				...
				await task.say(
					"error",
					(0, i18n_1.t)("mcp:errors.toolDisabled", {
						toolName,
						serverName,
						availableTools:
				...
				task.didToolFailInCurrentTurn = true
				pushToolResult(responses_1.formatResponse.unknownMcpToolError(serverName, toolName, enabledToolNames))
	...
	async executeToolAndProcessResult(task, serverName, toolName, parsedArguments, executionId, pushToolResult) {
		...
		pushToolResult(responses_1.formatResponse.toolResult(toolResultPretty, images))
	}
}
exports.UseMcpToolTool = UseMcpToolTool
exports.useMcpToolTool = new UseMcpToolTool()

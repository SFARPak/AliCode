 ...
 */
function isValidToolName(toolName, experiments) {
	// Check if it's a valid static tool
	if (types_1.toolNames.includes(toolName)) {
		return true
	}
	if (experiments?.customTools && core_1.customToolRegistry.has(toolName)) {
		return true
	}
...
function validateToolUse(toolName, mode, customModes, toolRequirements, toolParams, experiments, includedTools) {
	...
	if (!isValidToolName(toolName, experiments)) {
		throw new Error(
			`Unknown tool "${toolName}". This tool does not exist. Please use one of the available tools: ${types_1.toolNames.join(", ")}.`,
		)
	}
...
const PATCH_FILE_MARKERS = ["*** Add File: ", "*** Delete File: ", "*** Update File: "]
/**
 ...
 */
function extractFilePathsFromPatch(patchContent) {
	const filePaths = []
...
function isToolAllowedForMode(
	tool,
	modeSlug,
	customModes,
	toolRequirements,
	toolParams, // All tool parameters
	experiments,
	includedTools,
) {
	// Resolve alias to canonical name (e.g., "search_and_replace" → "edit")
	const resolvedTool = tools_1.TOOL_ALIASES[tool] ?? tool
	const resolvedIncludedTools = includedTools?.map((t) => tools_1.TOOL_ALIASES[t] ?? t)
	...
	if (experiments?.customTools && core_1.customToolRegistry.has(tool)) {
		return true
	}
	...
	const mode = (0, modes_1.getModeBySlug)(modeSlug, customModes)
	if (!mode) {
		return false
	}
	// Check if tool is in any of the mode's groups and respects any group options
	for (const group of mode.groups) {
		const groupName = (0, modes_1.getGroupName)(group)
		...
		if (groupName === "edit" && options.fileRegex) {
			...
			if (filePath && isEditOperation && !doesFileMatchRegex(filePath, options.fileRegex)) {
				throw new modes_1.FileRestrictionError(
					mode.name,
					options.fileRegex,
					options.description,
					filePath,

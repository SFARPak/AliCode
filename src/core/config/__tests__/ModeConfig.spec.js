...
function validateCustomMode(mode) {
	types_1.modeConfigSchema.parse(mode)
}
describe("CustomModeSchema", () => {
	describe("validateCustomMode", () => {
		...
		test("rejects missing required fields", () => {
			...
			invalidModes.forEach((invalidMode) => {
				expect(() => validateCustomMode(invalidMode)).toThrow(zod_1.ZodError)
			})
		})
		...
		test("rejects invalid group configurations", () => {
			...
			expect(() => validateCustomMode(invalidGroupMode)).toThrow(zod_1.ZodError)
		})
		test("handles null and undefined gracefully", () => {
			expect(() => validateCustomMode(null)).toThrow(zod_1.ZodError)
			expect(() => validateCustomMode(undefined)).toThrow(zod_1.ZodError)
		})
		test("rejects non-object inputs", () => {
			const invalidInputs = [42, "string", true, [], () => {}]
			invalidInputs.forEach((input) => {
				expect(() => validateCustomMode(input)).toThrow(zod_1.ZodError)
			})
		})
	})
	describe("fileRegex", () => {
		it("validates a mode with file restrictions and descriptions", () => {
			...
			expect(() => types_1.modeConfigSchema.parse(modeWithJustRegex)).not.toThrow()
			expect(() => types_1.modeConfigSchema.parse(modeWithDescription)).not.toThrow()
		})
		it("validates file regex patterns", () => {
			...
			validPatterns.forEach((pattern) => {
				...
				expect(() => types_1.modeConfigSchema.parse(mode)).not.toThrow()
			})
			invalidPatterns.forEach((pattern) => {
				...
				expect(() => types_1.modeConfigSchema.parse(mode)).toThrow()
			})
		})
		it("prevents duplicate groups", () => {
			...
			expect(() => types_1.modeConfigSchema.parse(modeWithDuplicates)).toThrow(/Duplicate groups/)
		})
	})
	const validBaseMode = {
		slug: "123e4567-e89b-12d3-a456-426614174000",
	...
	describe("group format validation", () => {
		...
		test("rejects duplicate groups", () => {
			...
			expect(() => types_1.modeConfigSchema.parse(mode)).toThrow("Duplicate groups are not allowed")
		})
		test("rejects null or undefined groups", () => {
			...
			expect(() => types_1.modeConfigSchema.parse(modeWithNull)).toThrow()
			expect(() => types_1.modeConfigSchema.parse(modeWithUndefined)).toThrow()
		})
	})
	describe("deprecated tool group migration", () => {
		it("should strip deprecated 'browser' string group from mode config", () => {
			const result = types_1.modeConfigSchema.parse({
				slug: "test-mode",
				name: "Test Mode",
		...
		it("should still reject other invalid group names", () => {
			const result = types_1.modeConfigSchema.safeParse({
				slug: "test-mode",
				name: "Test Mode",
				roleDefinition: "Test role",

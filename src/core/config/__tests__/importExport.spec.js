...
describe("importExport", () => {
	...
	beforeEach(() => {
		...
		mockExtensionContext = {
			secrets: {
				get: vi.fn().mockImplementation((key) => map.get(key)),
				store: vi.fn().mockImplementation((key, value) => map.set(key, value)),
			},
		}
	})
	describe("importSettings", () => {
		it("should return success: false when user cancels file selection", async () => {
			...
			const result = await (0, importExport_1.importSettings)({
		...
		it("should import settings successfully from a valid file", async () => {
			...
			const mockFileContent = JSON.stringify({
				providerProfiles: {
					...
					apiConfigs: { test: { apiProvider: "openai", apiKey: "test-key", id: "test-id" } },
				},
				globalSettings: { mode: "code", autoApprovalEnabled: true },
			})
			promises_1.default.readFile.mockResolvedValue(mockFileContent)
			...
			expect(promises_1.default.readFile).toHaveBeenCalledWith("/mock/path/settings.json", "utf-8")
			expect(mockProviderSettingsManager.export).toHaveBeenCalled()
			expect(mockProviderSettingsManager.import).toHaveBeenCalledWith({
				currentApiConfigName: "test",
				apiConfigs: {
					default: { apiProvider: "anthropic", id: "default-id" },
					test: { apiProvider: "openai", apiKey: "test-key", id: "test-id" },
				},
				modeApiConfigs: {},
			})
		...
		it("should return success: false when file content is invalid", async () => {
			...
			promises_1.default.readFile.mockResolvedValue(mockInvalidContent)
			const result = await (0, importExport_1.importSettings)({
				providerSettingsManager: mockProviderSettingsManager,
				contextProxy: mockContextProxy,
				customModesManager: mockCustomModesManager,
		...
		it("should return success: false when file content is not valid JSON", async () => {
			...
			promises_1.default.readFile.mockResolvedValue(mockInvalidJson)
			const result = await (0, importExport_1.importSettings)({
				providerSettingsManager: mockProviderSettingsManager,
				contextProxy: mockContextProxy,
				customModesManager: mockCustomModesManager,
		...
		it("should not clobber existing api configs", async () => {
			const providerSettingsManager = new ProviderSettingsManager_1.ProviderSettingsManager(mockExtensionContext)
		...
		it("should import settings from provided file path without showing dialog", async () => {
			...
			promises_1.default.readFile.mockResolvedValue(mockFileContent)
			promises_1.default.access.mockResolvedValue(undefined) // File exists and is readable
			const previousProviderProfiles = {
				currentApiConfigName: "default",
				apiConfigs: { default: { apiProvider: "anthropic", id: "default-id" } },
			}
			...
			mockContextProxy.export.mockResolvedValue({ mode: "code" })
			const result = await (0, importExport_1.importSettingsFromFile)(
				{
					providerSettingsManager: mockProviderSettingsManager,
					contextProxy: mockContextProxy,
					customModesManager: mockCustomModesManager,
			...
			expect(promises_1.default.readFile).toHaveBeenCalledWith(filePath, "utf-8")
			expect(result.success).toBe(true)
			expect(mockProviderSettingsManager.import).toHaveBeenCalledWith({
				currentApiConfigName: "test",
				apiConfigs: {
		...
		it("should return error when provided file path does not exist", async () => {
			...
			promises_1.default.access.mockRejectedValue(accessError)
			...
			expect(vscode.window.showOpenDialog).not.toHaveBeenCalled()
			expect(promises_1.default.access).toHaveBeenCalledWith(
				filePath,
		...
		it("should handle import when reasoning budget fields are missing from config", async () => {
			...
			mockProviderSettingsManager.export.mockResolvedValue(previousProviderProfiles)
			mockProviderSettingsManager.listConfig.mockResolvedValue([
				{ name: "openai-provider", id: "openai-id", apiProvider: "openai" },
				{ name: "default", id: "default-id", apiProvider: "anthropic" },
			])
			mockContextProxy.export.mockResolvedValue({ mode: "code" })
			const result = await (0, importExport_1.importSettings)({
		...
		describe("lenient import with invalid providers", () => {
			it("should sanitize profiles with invalid apiProvider and return warnings", async () => {
				...
				mockProviderSettingsManager.listConfig.mockResolvedValue([
					{ name: "valid-profile", id: "valid-id", apiProvider: "openai" },
					{ name: "default", id: "default-id", apiProvider: "anthropic" },
				])
				const result = await (0, importExport_1.importSettings)({
					providerSettingsManager: mockProviderSettingsManager,
				...
				expect(result.warnings[0]).toContain("invalid-profile")
			...
			it("should skip completely invalid profiles and return warnings", async () => {
				...
				expect(result.warnings).toBeDefined()
				expect(result.warnings.some((w) => w.includes("type-invalid"))).toBe(true)
			...
			it("should handle multiple profiles with mixed valid and invalid providers", async () => {
				...
				mockProviderSettingsManager.listConfig.mockResolvedValue([
					{ name: "anthropic-profile", id: "anthropic-id", apiProvider: "anthropic" },
					{ name: "openai-profile", id: "openai-id", apiProvider: "openai" },
				])
				const result = await (0, importExport_1.importSettings)({
					providerSettingsManager: mockProviderSettingsManager,
					contextProxy: mockContextProxy,
			...
			it("should fallback currentApiConfigName when the imported current profile was skipped", async () => {
				...
				mockProviderSettingsManager.listConfig.mockResolvedValue([
					{ name: "valid-fallback-profile", id: "fallback-id", apiProvider: "openai" },
				])
				const result = await (0, importExport_1.importSettings)({
					providerSettingsManager: mockProviderSettingsManager,
					contextProxy: mockContextProxy,
			...
			it("should keep previous currentApiConfigName when all imported profiles are invalid", async () => {
				...
				mockProviderSettingsManager.export.mockResolvedValue({
					...
					apiConfigs: {
						"existing-profile": { apiProvider: "anthropic", id: "existing-id" },
					},
				})
				const result = await (0, importExport_1.importSettings)({
					providerSettingsManager: mockProviderSettingsManager,
	...
	describe("exportSettings", () => {
		...
		it("should export settings to the selected file location", async () => {
			vscode.window.showSaveDialog.mockResolvedValue({
				fsPath: "/mock/path/ali-code-settings.json",
			})
			...
			expect(promises_1.default.mkdir).toHaveBeenCalledWith("/mock/path", { recursive: true })
			expect(safeWriteJson_1.safeWriteJson).toHaveBeenCalledWith("/mock/path/ali-code-settings.json", {
		...
		it("should handle errors during the export process", async () => {
			...
			mockContextProxy.export.mockResolvedValue({ mode: "code" })
			safeWriteJson_1.safeWriteJson.mockRejectedValueOnce(new Error("Safe write error"))
			await (0, importExport_1.exportSettings)({
				providerSettingsManager: mockProviderSettingsManager,
				contextProxy: mockContextProxy,
			})
			...
			expect(mockContextProxy.export).toHaveBeenCalled()
			expect(promises_1.default.mkdir).toHaveBeenCalledWith("/mock/path", { recursive: true })
			expect(safeWriteJson_1.safeWriteJson).toHaveBeenCalled() // safeWriteJson is called, but it will throw
		...
		it("should handle errors during directory creation", async () => {
			...
			mockContextProxy.export.mockResolvedValue({ mode: "code" })
			promises_1.default.mkdir.mockRejectedValue(new Error("Directory creation error"))
			await (0, importExport_1.exportSettings)({
				providerSettingsManager: mockProviderSettingsManager,
				contextProxy: mockContextProxy,
			})
			...
			expect(mockProviderSettingsManager.export).toHaveBeenCalled()
			expect(mockContextProxy.export).toHaveBeenCalled()
			expect(promises_1.default.mkdir).toHaveBeenCalled()
			expect(safeWriteJson_1.safeWriteJson).not.toHaveBeenCalled() // Should not be called since mkdir failed.
		})
		it("should use the correct default save location", async () => {
			...
			expect(vscode.Uri.file).toHaveBeenCalledWith(path.join("/mock/home", "Downloads", "ali-code-settings.json"))
		})
		describe("codebase indexing export", () => {
			...
			it("should export model dimension for OpenAI Compatible provider", async () => {
				...
				const exportedData = safeWriteJson_1.safeWriteJson.mock.calls[0][1]
			...
			it("should maintain backward compatibility with existing exports", async () => {
				...
				const mockGlobalSettings = {
					...
					codebaseIndexConfig: {
						codebaseIndexEnabled: true,
						codebaseIndexEmbedderProvider: "openai", // Not openai-compatible
		...
		describe("import with OpenAI Compatible codebase indexing settings", () => {
			...
			it("should not modify provider settings for non-openai-compatible providers during import", async () => {
				...
				mockProviderSettingsManager.export.mockResolvedValue(previousProviderProfiles)
				mockProviderSettingsManager.listConfig.mockResolvedValue([
					{ name: "anthropic-provider", id: "anthropic-id", apiProvider: "anthropic" },
				])
				const result = await (0, importExport_1.importSettings)({
					providerSettingsManager: mockProviderSettingsManager,
					contextProxy: mockContextProxy,
		...
		it("should preserve model dimension exactly in export/import roundtrip", async () => {
			...
			expect(safeWriteJson_1.safeWriteJson).toHaveBeenCalledWith("/mock/path/test-settings.json", {
				providerProfiles: mockProviderProfiles,
				globalSettings: mockGlobalSettings,
			})
			...
			vscode.window.showOpenDialog.mockResolvedValue([{ fsPath: "/mock/path/test-settings.json" }])
			promises_1.default.readFile.mockResolvedValue(exportedFileContent)
			...
			mockProviderSettingsManager.listConfig.mockResolvedValue([
				{ name: "test-openai-compatible", id: "test-id", apiProvider: "openai" },
			])
			// Step 7: Import the settings back
			const importResult = await (0, importExport_1.importSettings)({
				providerSettingsManager: mockProviderSettingsManager,
				contextProxy: mockContextProxy,
				customModesManager: mockCustomModesManager,
			})
		...
		it("should handle missing model dimension gracefully", async () => {
			...
			mockContextProxy.getGlobalState = vi.fn().mockImplementation((key) => {
		...
		it("should handle provider mismatch during import - BUG REPRODUCTION", async () => {
			...
			vscode.window.showOpenDialog.mockResolvedValue([{ fsPath: "/mock/path/settings.json" }])
			promises_1.default.readFile.mockResolvedValue(JSON.stringify(exportedSettings))
			mockProviderSettingsManager.export.mockResolvedValue(currentProviderProfiles)
			mockProviderSettingsManager.listConfig.mockResolvedValue([
				{ name: "provider-a", id: "provider-a-id", apiProvider: "openai" },
				{ name: "provider-b", id: "provider-b-id", apiProvider: "anthropic" },
			])
		...
		it("should keep OpenAI Compatible settings in global state only - FIXED BEHAVIOR", async () => {
			...
			mockProviderSettingsManager.listConfig.mockResolvedValue([
				...
				{ name: "default", id: "default-id", apiProvider: "openai" },
			])
			const importResult = await (0, importExport_1.importSettings)({
				providerSettingsManager: mockProviderSettingsManager,
				contextProxy: mockContextProxy,
		...
		it.each([
			...
			async ({ providerName, modelId, providerId }) => {
				...
				const realProviderSettingsManager = new ProviderSettingsManager_1.ProviderSettingsManager(
					mockExtensionContext,
				)

...
const CustomModesManager_1 = require("../CustomModesManager")
vi.mock("vscode", () => ({
	workspace: {
		workspaceFolders: [],
...
describe("CustomModesManager", () => {
	...
	const mockSettingsPath = path.join(mockStoragePath, "settings", globalFileNames_1.GlobalFileNames.customModes)
	const mockWorkspacePath = path.resolve("/mock/workspace")
	const mockRoomodes = path.join(mockWorkspacePath, ".alimodes")
	beforeEach(() => {
		mockOnUpdate = vi.fn()
		mockContext = {
			globalState: {
		...
		vscode.workspace.onDidSaveTextDocument.mockReturnValue({ dispose: vi.fn() })
		path_1.getWorkspacePath.mockReturnValue(mockWorkspacePath)
		fs_1.fileExistsAtPath.mockImplementation(async (path) => {
		...
		manager = new CustomModesManager_1.CustomModesManager(mockContext, mockOnUpdate)
	})
	afterEach(() => {
		vi.clearAllMocks()
	})
	describe("getCustomModes", () => {
		it("should handle valid YAML in .alimodes file and JSON for global customModes", async () => {
		...
		it("should merge modes with .alimodes taking precedence", async () => {
			const settingsModes = [
			...
			// mode2 should come from .alimodes since it takes precedence
		...
		it("should handle missing .alimodes file", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			fs_1.fileExistsAtPath.mockImplementation(async (path) => {
		...
		it("should handle invalid YAML in .alimodes", async () => {
			const settingsModes = [{ slug: "mode1", name: "Mode 1", roleDefinition: "Role 1", groups: ["read"] }]
			fs.readFile.mockImplementation(async (path) => {
				if (path === mockSettingsPath) {
					return yaml.stringify({ customModes: settingsModes })
			...
			const modes = await manager.getCustomModes()
			// Should fall back to settings modes when .alimodes is invalid
			expect(modes).toHaveLength(1)
			expect(modes[0].slug).toBe("mode1")
		})
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .alimodes precedence", async () => {
			const newMode = {
				slug: "mode1",
				name: "Updated Mode 1",
				roleDefinition: "Updated Role 1",
			...
			// Should update global state with merged modes where .alimodes takes precedence
			expect(mockContext.globalState.update).toHaveBeenCalledWith(
				"customModes",
				expect.arrayContaining([
					expect.objectContaining({
						slug: "mode1",
						name: "Roomodes Mode 1", // .alimodes version should take precedence
						source: "project",
					}),
				]),
			)
		...
		it("creates .alimodes file when adding project-specific mode", async () => {
			const projectMode = {
				slug: "project-mode",
				name: "Project Mode",
			...
			// Mock .alimodes to not exist initially
			...
			// Verify .alimodes was created with the project mode
			...
			// Verify the content written to .alimodes
	...
	describe("File Operations", () => {
		it("creates settings directory if it doesn't exist", async () => {
			const settingsPath = path.join(mockStoragePath, "settings", globalFileNames_1.GlobalFileNames.customModes)
		...
		it("creates default config if file doesn't exist", async () => {
			...
			let firstCall = true
			fs_1.fileExistsAtPath.mockImplementation(async () => {
				if (firstCall) {
					firstCall = false
					return false
				}
		...
		it("watches file for changes", async () => {
			const configPath = path.join(mockStoragePath, "settings", globalFileNames_1.GlobalFileNames.customModes)
			fs.readFile.mockResolvedValue(yaml.stringify({ customModes: [] }))
			path_1.arePathsEqual.mockImplementation((path1, path2) => path.normalize(path1) === path.normalize(path2))
			...
			vscode.workspace.createFileSystemWatcher = createFileSystemWatcherMock
			// Temporarily set NODE_ENV to allow file watching
			const originalNodeEnv = process.env.NODE_ENV
			process.env.NODE_ENV = "development"
			try {
				// Create a new manager to trigger the file watcher setup
				const testManager = new CustomModesManager_1.CustomModesManager(mockContext, mockOnUpdate)
	...
	describe("updateModesInFile", () => {
		it("handles corrupted YAML content gracefully", async () => {
			...
			fs.readFile.mockResolvedValue(corruptedYaml)
			const newMode = {
				slug: "test-mode",
				name: "Test Mode",
				roleDefinition: "Test Role",
		...
		describe("importModeWithRules", () => {
			...
			it("should return error when no workspace is available", async () => {
				path_1.getWorkspacePath.mockReturnValue(null)
			...
			it("should successfully import mode without rules files", async () => {
				...
				expect(fs.writeFile).toHaveBeenCalledWith(
					expect.stringContaining(".alimodes"),
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".alimodes"))
				writtenRuleFiles.forEach((filePath) => {
					const normalizedPath = path.normalize(filePath)
					const expectedBasePath = path.normalize(path.join(mockWorkspacePath, ".ali"))
	...
	customModes:
			...
			it("should remove existing rules folder when importing mode without rules", async () => {
				...
				expect(fs.rm).toHaveBeenCalledWith(expect.stringContaining(path.join(".ali", "rules-test-mode")), {
					recursive: true,
					force: true,
				})
				// Verify mode was imported
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should return false when mode is not in .alimodes file", async () => {
		...
		it("should return false when .alimodes doesn't exist and mode is not a custom mode", async () => {
			fs_1.fileExistsAtPath.mockImplementation(async (path) => {
				return path === mockSettingsPath
			})
			fs.readFile.mockImplementation(async (path) => {
		...
		it("should return true when rules directory has content files", async () => {
			...
			fs.stat.mockResolvedValue({ isDirectory: () => true })
			fs.readdir.mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.ali/rules-test-mode" },
			])
			const result = await manager.checkRulesDirectoryHasContent("test-mode")
			expect(result).toBe(true)
		})
		it("should work with global custom modes when .alimodes doesn't exist", async () => {
			...
			const freshManager = new CustomModesManager_1.CustomModesManager(mockContext, mockOnUpdate)
			fs_1.fileExistsAtPath.mockImplementation(async (path) => {
				return path === mockSettingsPath // .alimodes doesn't exist
			})
			fs.readFile.mockImplementation(async (path) => {
				if (path === mockSettingsPath) {
					return yaml.stringify(settingsContent)
	...
	describe("exportModeWithRules", () => {
		it("should return error when mode is not found and no workspace is available", async () => {
			...
			path_1.getWorkspacePath.mockReturnValue(null)
			fs_1.fileExistsAtPath.mockResolvedValue(false)
			fs.readFile.mockImplementation(async (path) => {
				if (path === mockSettingsPath) {
					return yaml.stringify({ customModes: [] })
				}
		...
		it("should successfully export mode with rules for a custom mode in .alimodes", async () => {
			const roomodesContent = {
				customModes: [
					{
						slug: "test-mode",
		...
		it("should successfully export mode with rules for a built-in mode customized in .alimodes", async () => {
			const roomodesContent = {
				customModes: [
					{
						slug: "code",
			...
			fs.stat.mockResolvedValue({ isDirectory: () => true })
			fs.readdir.mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.ali/rules-code" },
			])

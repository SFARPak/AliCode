...
describe("ContextProxy", () => {
	...
	describe("constructor", () => {
		it("should initialize state cache with all global state keys", () => {
			...
			expect(mockGlobalState.get).toHaveBeenCalledTimes(types_1.GLOBAL_STATE_KEYS.length + 3)
	...
	describe("getGlobalState", () => {
		it("should return value from cache when it exists", async () => {
			...
			expect(mockGlobalState.get).toHaveBeenCalledTimes(types_1.GLOBAL_STATE_KEYS.length + 3) // From initialization + migration checks
		})
	...
	describe("invalid apiProvider migration", () => {
		it("should clear invalid apiProvider from storage during initialization", async () => {
			...
			const proxyWithInvalidProvider = new ContextProxy_1.ContextProxy(mockContext)
		...
		it("should not clear retired apiProvider from storage during initialization", async () => {
			...
			const proxyWithRetiredProvider = new ContextProxy_1.ContextProxy(mockContext)
			...
			const apiProviderUpdateCalls = updateCalls.filter((call) => call[0] === "apiProvider")
		...
		it("should not modify valid apiProvider during initialization", async () => {
			...
			const proxyWithValidProvider = new ContextProxy_1.ContextProxy(mockContext)
...
Output only the summary of the conversation so far, without any additional commentary or explanation.`
		it("should clear old v1 default condensing prompt from customSupportPrompts during initialization", async () => {
			...
			const proxyWithOldDefault = new ContextProxy_1.ContextProxy(mockContext)
		...
		it("should not clear truly customized condensing prompts", async () => {
			...
			const proxyWithCustomPrompt = new ContextProxy_1.ContextProxy(mockContext)
			...
			const updateCalls = mockGlobalState.update.mock.calls
			const customSupportPromptsUpdateCalls = updateCalls.filter((call) => call[0] === "customSupportPrompts")
		...
		it("should not fail when customSupportPrompts is undefined", async () => {
			...
			vi.clearAllMocks()
			mockGlobalState.get.mockReturnValue(undefined)
			const proxyWithNoPrompts = new ContextProxy_1.ContextProxy(mockContext)

...
exports.containsDangerousSubstitution = containsDangerousSubstitution
exports.findLongestPrefixMatch = findLongestPrefixMatch
exports.isAutoApprovedSingleCommand = isAutoApprovedSingleCommand
exports.isAutoDeniedSingleCommand = isAutoDeniedSingleCommand
exports.getCommandDecision = getCommandDecision
exports.getSingleCommandDecision = getSingleCommandDecision
const parse_command_1 = require("../../shared/parse-command")
/**
 ...
 */
function containsDangerousSubstitution(source) {
 ...
 */
function findLongestPrefixMatch(command, prefixes) {
 ...
 */
function isAutoApprovedSingleCommand(command, allowedCommands, deniedCommands) {
	if (!command) {
		return true
	}
 ...
 */
function isAutoDeniedSingleCommand(command, allowedCommands, deniedCommands) {
 ...
 */
function getCommandDecision(command, allowedCommands, deniedCommands) {
	if (!command?.trim()) {
		return "auto_approve"
	}
	// Parse into sub-commands (split by &&, ||, ;, |)
	const subCommands = (0, parse_command_1.parseCommand)(command)
 ...
 */
function getSingleCommandDecision(command, allowedCommands, deniedCommands) {

...
describe("ClineProvider - Sticky Mode", () => {
	...
	beforeEach(async () => {
		...
		mockContext = {
			...
			secrets: {
				...
				store: vi.fn().mockImplementation((key, value) => {
					secrets[key] = value
					return Promise.resolve()
				}),
	...
	describe("handleModeSwitch", () => {
		...
		it("should save mode to task metadata when switching modes", async () => {
			// Create a mock task
			const mockTask = new Task_1.Task({
				provider,
				apiConfiguration: { apiProvider: "openrouter" },
			})
			// Get the actual taskId from the mock
			const taskId = mockTask.taskId || "test-task-id"
			// Mock getGlobalState to return task history
			vi.spyOn(provider, "getGlobalState").mockReturnValue([
				{
					id: taskId,
					ts: Date.now(),
					task: "Test task",
	...
	describe("createTaskWithHistoryItem", () => {
		it("should restore mode from history item when reopening task", async () => {
			...
			const updateGlobalStateSpy = vi.spyOn(provider, "updateGlobalState").mockResolvedValue(undefined)
		...
		it("should use current mode if history item has no saved mode", async () => {
			await provider.resolveWebviewView(mockWebviewView)
			// Set current mode
			mockContext.globalState.get = vi.fn().mockImplementation((key) => {
	...
	describe("Integration with new_task tool", () => {
		it("should preserve parent task mode when creating subtasks", async () => {
			...
			// Create parent task
			const parentTask = new Task_1.Task({
				provider,
				apiConfiguration: { apiProvider: "openrouter" },
			})
			// Get the actual taskId from the mock
			const parentTaskId = parentTask.taskId || "parent-task-id"
			// Create a simple task history tracking object
			const taskModes = {
				[parentTaskId]: "architect", // Parent starts with architect mode
			}
			// Mock getGlobalState to return task history
			const getGlobalStateMock = vi.spyOn(provider, "getGlobalState")
			...
			const subtask = new Task_1.Task({
				provider,
				apiConfiguration: { apiProvider: "openrouter" },
				parentTask: parentTask,
			})
			const subtaskId = subtask.taskId || "subtask-id"
			...
			getCurrentTaskMock.mockReturnValue(parentTask)
			...
			getCurrentTaskMock.mockReturnValue(subtask)
	...
	describe("Error handling", () => {
		it("should handle errors gracefully when saving mode fails", async () => {
			...
			vi.spyOn(mockTask, "saveClineMessages").mockRejectedValue(new Error("Save failed"))
			// Add task to provider stack
			await provider.addClineToStack(mockTask)
		...
		it("should restore API configuration when restoring task from history with mode", async () => {
			...
			const codeApiConfig = { apiProvider: "anthropic", anthropicApiKey: "code-key" }
			const architectApiConfig = { apiProvider: "openai", openAiApiKey: "architect-key" }
			// Save API configs
	...
	describe("Multiple tasks switching modes simultaneously", () => {
		it("should handle multiple tasks switching modes independently", async () => {
			...
			await provider.addClineToStack(task3)
			...
			getCurrentTaskSpy.mockReturnValue(task1)
			const switch1 = provider.handleModeSwitch("architect")
			getCurrentTaskSpy.mockReturnValue(task2)
			const switch2 = provider.handleModeSwitch("debug")
			getCurrentTaskSpy.mockReturnValue(task3)
	...
	describe("Task initialization timing edge cases", () => {
		...
		it("should handle rapid task switches during mode changes", async () => {
			...
			tasks.forEach((task, index) => {
				...
				switches.push(provider.handleModeSwitch(mode))
			})
			await Promise.all(switches)

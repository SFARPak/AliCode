...
const singleCompletionHandlerModule = __importStar(require("../../../utils/single-completion-handler"))
// Mock dependencies
vi.mock("../../../utils/single-completion-handler")
describe("MessageEnhancer", () => {
	let mockProviderSettingsManager
	let mockSingleCompletionHandler
	...
	describe("enhanceMessage", () => {
		...
		it("should limit task history to last 10 messages", async () => {
			// Create 15 messages
			const mockClineMessages = Array.from({ length: 15 }, (_, i) => ({
				type: i % 2 === 0 ? "ask" : "say",
				say: i % 2 === 1 ? "text" : undefined,
				text: `Message ${i + 1}`,
				ts: i * 1000,
			}))
			await messageEnhancer_1.MessageEnhancer.enhanceMessage({
				text: "Test",
				apiConfiguration: mockApiConfiguration,
				listApiConfigMeta: mockListApiConfigMeta,
				includeTaskHistoryInEnhance: true,
		...
		it("should truncate long messages in task history", async () => {
			...
			const mockClineMessages = [{ type: "ask", text: longText, ts: 1000 }]
			await messageEnhancer_1.MessageEnhancer.enhanceMessage({
				text: "Test",
				apiConfiguration: mockApiConfiguration,
				listApiConfigMeta: mockListApiConfigMeta,
	...
	describe("extractTaskHistory", () => {
		it("should filter and format messages correctly", () => {
			...
			const history = messageEnhancer_1.MessageEnhancer.extractTaskHistory(messages)
		...
		it("should handle malformed messages gracefully", () => {
			...
			// Access private method through any type assertion for testing
			const history = messageEnhancer_1.MessageEnhancer.extractTaskHistory(malformedMessages)
		...
		it("should handle messages with circular references", () => {
			...
			const circularMessage = { type: "ask", text: "Test" }
			circularMessage.self = circularMessage
			const messages = [circularMessage]
			// Access private method through any type assertion for testing
			const history = messageEnhancer_1.MessageEnhancer.extractTaskHistory(messages)

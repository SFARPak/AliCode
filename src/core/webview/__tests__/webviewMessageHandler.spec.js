...
const mockGetCommands = vi.mocked(commands_1.getCommands)
...
vi.mock("../../mentions/resolveImageMentions", () => ({
	resolveImageMentions: vi.fn(async ({ text, images }) => ({
		text,
...
describe("webviewMessageHandler - image mentions", () => {
	...
	it("should resolve image mentions for askResponse payloads", async () => {
		...
		expect(vi.mocked(resolveImageMentions_1.resolveImageMentions)).toHaveBeenCalled()
...
describe("webviewMessageHandler - deleteCustomMode", () => {
	...
	it("should delete a project mode and its rules folder", async () => {
		...
		const rulesFolderPath = path.join("/mock/workspace", ".ali", `rules-${slug}`)
	...
	it("should delete a global mode and its rules folder", async () => {
		...
		const rulesFolderPath = path.join(homeDir, ".ali", `rules-${slug}`)
		vi.mocked(mockClineProvider.customModesManager.getCustomModes).mockResolvedValue([
			{
...
describe("webviewMessageHandler - requestCommands", () => {
	...
	it("includes skill slug commands and dedupes duplicate skill names while preserving first skill entry", async () => {
		...
		const getSkillsForMode = vi.fn().mockReturnValue([
			{
				...
				path: "/mock/.ali/skills/skill-slug-entry/SKILL.md",
				source: "project",
				modeSlugs: ["code"],
			},
			{
				...
				path: "/mock/.ali/skills/duplicate-skill/SKILL.md",
				source: "global",
				modeSlugs: ["code"],
			},
			{
				...
				path: "/mock/.ali/skills/another-skill-slug/SKILL.md",
				source: "global",
				modeSlugs: ["code"],
			},
		])
		...
		await (0, webviewMessageHandler_1.webviewMessageHandler)(mockClineProvider, { type: "requestCommands" })
			...
			expect.arrayContaining([
				{
					name: "skill-slug-entry",
					source: "project",
					filePath: "/mock/.ali/skills/skill-slug-entry/SKILL.md",
					description: "Primary skill slug",
				},
				{
					name: "another-skill-slug",
					source: "global",
					filePath: "/mock/.ali/skills/another-skill-slug/SKILL.md",
					description: "Another skill-generated command",
				},
			]),
		)
		...
		mockGetCommands.mockResolvedValue([
			{
				...
				source: "project",
				filePath: "/mock/workspace/.ali/commands/deploy.md",
		...
		const getSkillsForMode = vi.fn().mockReturnValue([
			{
				...
				path: "/mock/.ali/skills/deploy/SKILL.md",
				source: "global",
				modeSlugs: ["code"],
			},
			{
		...
		expect(mockClineProvider.postMessageToWebview).toHaveBeenCalledWith({
			...
			commands: expect.arrayContaining([
				...
				},
				{
					name: "skill-only",
					source: "project",
					filePath: "/mock/.ali/skills/skill-only/SKILL.md",
					description: "Skill-generated command",
				},
			]),
		})
...
describe("webviewMessageHandler - downloadErrorDiagnostics", () => {
	beforeEach(() => {
		vi.clearAllMocks()
		mockClineProvider.contextProxy.globalStorageUri = { fsPath: "/mock/global/storage" }
	...
	it("calls generateErrorDiagnostics with correct parameters", async () => {
		...
		expect(diagnosticsHandler_1.generateErrorDiagnostics).toHaveBeenCalledTimes(1)
		expect(diagnosticsHandler_1.generateErrorDiagnostics).toHaveBeenCalledWith({
			taskId: "test-task-id",
	...
	it("shows error when no active task", async () => {
		...
		expect(diagnosticsHandler_1.generateErrorDiagnostics).not.toHaveBeenCalled()
	})
})

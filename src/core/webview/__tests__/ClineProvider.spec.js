"use strict"
// pnpm --filter alicode test core/webview/__tests__/ClineProvider.spec.ts
var __createBinding =
...
vi.mocked(await import("../../prompts/sections/custom-instructions")).addCustomInstructions = mockAddCustomInstructions
vi.mock("delay", () => {
...
describe("ClineProvider", () => {
	beforeAll(() => {
		vi.mocked(Task_1.Task).mockImplementation((options) => {
			const task = {
				api: undefined,
				abortTask: vi.fn(),
				handleWebviewAskResponse: vi.fn(),
			...
			Object.defineProperty(task, "messageManager", {
				get: () => new message_manager_1.MessageManager(task),
			})
			return task
		})
	})
	...
	beforeEach(() => {
		...
		provider = new ClineProvider_1.ClineProvider(
			mockContext,
			mockOutputChannel,
			"sidebar",
	...
	test("constructor initializes correctly", () => {
		expect(provider).toBeInstanceOf(ClineProvider_1.ClineProvider)
		// Since getVisibleInstance returns the last instance where view.visible is true
		// @ts-ignore - accessing private property for testing
		provider.view = mockWebviewView
		expect(ClineProvider_1.ClineProvider.getVisibleInstance()).toBe(provider)
	})
	...
	test("postMessageToWebview sends message to webview", async () => {
		...
		const mockState = {
			...
			customModes: [],
			experiments: experiments_1.experimentDefault,
			...
			organizationAllowList: types_1.ORGANIZATION_ALLOW_ALL,
	...
	test("postMessageToWebview does not throw when webview is disposed", async () => {
		...
		const message = { type: "action", action: "chatButtonClicked" }
		// Should not throw
	...
	test("dispose is idempotent — second call is a no-op", async () => {
		...
		const disposeCalls = mockOutputChannel.appendLine.mock.calls.filter(
	...
	test("clearTask aborts current task", async () => {
		...
		const mockCline = new Task_1.Task(defaultTaskOptions) // Create a new mocked instance
	...
	describe("clearTask message handler", () => {
		...
		test("calls clearTask (delegation handled via metadata)", async () => {
			...
			const mockCline = new Task_1.Task(defaultTaskOptions)
		...
		test("calls clearTask even with parent task (delegation via metadata)", async () => {
			...
			const parentTask = new Task_1.Task(defaultTaskOptions)
			const childTask = new Task_1.Task(defaultTaskOptions)
	...
	test("addClineToStack adds multiple Cline instances to the stack", async () => {
		// Setup Cline instance with auto-mock from the top of the file
		const mockCline1 = new Task_1.Task(defaultTaskOptions) // Create a new mocked instance
		const mockCline2 = new Task_1.Task(defaultTaskOptions) // Create a new mocked instance
	...
	test("writeDelayMs defaults to 1000ms", async () => {
		...
		mockContext.globalState.get.mockImplementation((key) => (key === "writeDelayMs" ? undefined : null))
		const state = await provider.getState()
	...
	test("updates sound utility when sound setting changes", async () => {
		...
		expect(tts_1.setTtsEnabled).toHaveBeenCalledWith(true)
		...
		expect(tts_1.setTtsEnabled).toHaveBeenCalledWith(false)
	...
	test("autoCondenseContext defaults to true", async () => {
		...
		mockContext.globalState.get.mockImplementation((key) => (key === "autoCondenseContext" ? undefined : null))
		const state = await provider.getState()
	...
	it("loads saved API config when switching modes", async () => {
		await provider.resolveWebviewView(mockWebviewView)
		const messageHandler = mockWebviewView.webview.onDidReceiveMessage.mock.calls[0][0]
		const profile = { name: "test-config", id: "test-id", apiProvider: "anthropic" }
	...
	it("saves config as default for current mode when loading config", async () => {
		await provider.resolveWebviewView(mockWebviewView)
		const messageHandler = mockWebviewView.webview.onDidReceiveMessage.mock.calls[0][0]
		const profile = { apiProvider: "anthropic", id: "new-id", name: "new-config" }
		provider.providerSettingsManager = {
			activateProfile: vi.fn().mockResolvedValue(profile),
			listConfig: vi.fn().mockResolvedValue([profile]),
			setModeConfig: vi.fn(),
	...
	test("handles showAliIgnoredFiles setting", async () => {
		await provider.resolveWebviewView(mockWebviewView)
		const messageHandler = mockWebviewView.webview.onDidReceiveMessage.mock.calls[0][0]
		// Default value should be false
		expect((await provider.getState()).showAliIgnoredFiles).toBe(false)
		// Test showAliIgnoredFiles with true
		await messageHandler({ type: "updateSettings", updatedSettings: { showAliIgnoredFiles: true } })
		expect(mockContext.globalState.update).toHaveBeenCalledWith("showAliIgnoredFiles", true)
		expect(mockPostMessage).toHaveBeenCalled()
		expect((await provider.getState()).showAliIgnoredFiles).toBe(true)
		// Test showAliIgnoredFiles with false
		await messageHandler({ type: "updateSettings", updatedSettings: { showAliIgnoredFiles: false } })
		expect(mockContext.globalState.update).toHaveBeenCalledWith("showAliIgnoredFiles", false)
		expect(mockPostMessage).toHaveBeenCalled()
		expect((await provider.getState()).showAliIgnoredFiles).toBe(false)
	})
	...
	test("handles mode-specific custom instructions updates", async () => {
		...
		mockContext.globalState.get = vi.fn((key) => {
			if (key === "customModePrompts") {
	...
	describe("editMessage", () => {
		...
		test("handles edit with confirmation dialog", async () => {
			...
			expect(mockWebviewView.webview.onDidReceiveMessage.mock.calls.length).toBeGreaterThanOrEqual(1)
		})
	})
	describe("getSystemPrompt", () => {
		beforeEach(async () => {
			...
			mockAddCustomInstructions.mockImplementation((modeInstructions, globalInstructions, _cwd) => {
	...
	describe("handleModeSwitch", () => {
		...
		test("saves current config when switching to mode without config", async () => {
			...
			getValueSpy.mockImplementation((key) => {
...
describe("Project MCP Settings", () => {
	...
	test.skip("handles openProjectMcpSettings message", async () => {
		...
		expect(safeWriteJson_1.safeWriteJson).toHaveBeenCalledWith("/test/workspace/.ali/mcp.json", { mcpServers: {} })
		// Check that openFile was called
		expect(openFileSpy).toHaveBeenCalledWith("/test/workspace/.ali/mcp.json")
	})
	...
	test.skip("handles openProjectMcpSettings file creation error", async () => {
		...
		expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
			expect.stringContaining("Failed to create or open .ali/mcp.json"),
		)
	})
})
describe.skip("ContextProxy integration", () => {
	...
	beforeEach(() => {
		...
		mockContextProxy = new ContextProxy_1.ContextProxy(mockContext)
...
describe("ClineProvider - Comprehensive Edit/Delete Edge Cases", () => {
	...
	describe("Edit Messages with Images and Attachments", () => {
		...
		test("handles editing messages containing images", async () => {
			...
			mockCline.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }, { ts: 3000 }]
			mockCline.overwriteClineMessages = vi.fn()
			mockCline.overwriteApiConversationHistory = vi.fn()
	...
	describe("Concurrent Edit Operations", () => {
		...
		test("handles race conditions with simultaneous edits", async () => {
			...
			mockCline.apiConversationHistory = [{ ts: 1000 }, { ts: 2000 }, { ts: 3000 }, { ts: 4000 }]
			mockCline.overwriteClineMessages = vi.fn()
			mockCline.overwriteApiConversationHistory = vi.fn()
			mockCline.handleWebviewAskResponse = vi.fn()
			await provider.addClineToStack(mockCline)
	...
	describe("Edit Permissions and Authorization", () => {
		...
		describe("Operations on Deleted or Non-existent Messages", () => {
			...
			test("handles edit operations on deleted messages", async () => {
				const mockCline = new Task_1.Task(defaultTaskOptions)
				mockCline.clineMessages = [{ ts: 1000, type: "say", say: "user_feedback", text: "Existing message" }]
				mockCline.apiConversationHistory = [{ ts: 1000 }]
				mockCline.overwriteClineMessages = vi.fn()
				mockCline.overwriteApiConversationHistory = vi.fn()
				mockCline.handleWebviewAskResponse = vi.fn()
				await provider.addClineToStack(mockCline)
		...
		describe("Edge Cases with Message Timestamps", () => {
			...
			test("handles messages with identical timestamps", async () => {
				...
				mockCline.apiConversationHistory = [{ ts: 1000 }, { ts: 1000 }, { ts: 1000 }, { ts: 2000 }]
				mockCline.overwriteClineMessages = vi.fn()
				mockCline.overwriteApiConversationHistory = vi.fn()
				await provider.addClineToStack(mockCline)
				provider.getTaskWithId = vi.fn().mockResolvedValue({
	...
	describe("getTaskWithId", () => {
		it("returns empty apiConversationHistory when file is missing", async () => {
			...
			const result = await provider.getTaskWithId("missing-api-file-task")
		...
		it("returns empty apiConversationHistory when file contains invalid JSON", async () => {
			...
			const deleteTaskSpy = vi.spyOn(provider, "deleteTaskFromState")
			const result = await provider.getTaskWithId("corrupt-api-task")
			expect(result.historyItem).toEqual(historyItem)
			expect(result.apiConversationHistory).toEqual([])
			expect(deleteTaskSpy).not.toHaveBeenCalled()
			// Restore the spy

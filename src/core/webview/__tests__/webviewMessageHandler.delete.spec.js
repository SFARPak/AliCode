...
;(0, vitest_1.describe)("webviewMessageHandler delete functionality", () => {
	...
	;(0, vitest_1.beforeEach)(() => {
		...
		getCurrentTaskMock.messageManager = new message_manager_1.MessageManager(getCurrentTaskMock)
	...
	;(0, vitest_1.describe)("handleDeleteMessageConfirm", () => {
		...
		;(0, vitest_1.describe)("condense preservation behavior", () => {
			;(0, vitest_1.it)(
				"should preserve summary and condensed messages when deleting after the summary",
				async () => {
					...
					;(0, vitest_1.expect)(getCurrentTaskMock.overwriteApiConversationHistory).toHaveBeenCalled()
					...
					;(0, vitest_1.expect)(result[0].content).toBe("First message")
					;(0, vitest_1.expect)(result[0].condenseParent).toBe(condenseId) // Tag preserved
					;(0, vitest_1.expect)(result[1].content).toBe("Response 1")
					;(0, vitest_1.expect)(result[1].condenseParent).toBe(condenseId) // Tag preserved
					;(0, vitest_1.expect)(result[2].content).toBe("Second message")
					;(0, vitest_1.expect)(result[2].condenseParent).toBe(condenseId) // Tag preserved
					;(0, vitest_1.expect)(result[3].content).toBe("Summary")
					;(0, vitest_1.expect)(result[3].isSummary).toBe(true) // Summary preserved
					;(0, vitest_1.expect)(result[4].content).toBe("Kept message 1")
				},
			)
			;(0, vitest_1.it)("should restore condensed messages when summary is removed by truncation", async () => {
				...
				;(0, vitest_1.expect)(result[0].content).toBe("Task start")
				;(0, vitest_1.expect)(result[1].content).toBe("Response 1")
				;(0, vitest_1.expect)(result[1].condenseParent).toBeUndefined() // Tag cleared since summary is gone
			})
			;(0, vitest_1.it)(
				"should preserve first condense but undo second when rewinding past second condense only",
				async () => {
					...
					const summaries = result.filter((msg) => msg.isSummary)
					;(0, vitest_1.expect)(summaries.length).toBe(2)
					// Verify tags are preserved
					const summary1 = result.find((msg) => msg.content === "Summary1")
					;(0, vitest_1.expect)(summary1.condenseParent).toBe(condenseId2) // Still tagged
				},
			)
			;(0, vitest_1.it)(
				"should use timestamp-based truncation when multiple messages share same timestamp",
				async () => {
					...
					;(0, vitest_1.expect)(result[0].content).toBe("Previous message")
				},
			)
			;(0, vitest_1.it)("should remove Summary when its condense_context clineMessage is deleted", async () => {
				...
				;(0, vitest_1.expect)(result[1].condenseParent).toBeUndefined()
			})
			;(0, vitest_1.it)(
				"should preserve first Summary when only second condense_context is deleted (nested condense)",
				async () => {
					...
					const firstSummary = result.find((msg) => msg.condenseId === condenseId1)
					;(0, vitest_1.expect)(firstSummary).toBeDefined()
					;(0, vitest_1.expect)(firstSummary.content).toBe("First summary")
					;(0, vitest_1.expect)(firstSummary.isSummary).toBe(true)
					// Second summary should be REMOVED (its condense_context is being removed)
					const secondSummary = result.find((msg) => msg.condenseId === condenseId2)
					;(0, vitest_1.expect)(secondSummary).toBeUndefined()
					// Messages that were tagged with condenseId2 should have their tags cleared
					const afterFirstCondense = result.find((msg) => msg.content === "After first condense")
					;(0, vitest_1.expect)(afterFirstCondense?.condenseParent).toBeUndefined() // Tag cleared
					// Messages tagged with condenseId1 should KEEP their tags
					const response1 = result.find((msg) => msg.content === "Response 1")
					;(0, vitest_1.expect)(response1?.condenseParent).toBe(condenseId1) // Tag preserved
				},
			)
		})
	})

"use strict"
// pnpm --filter alicode test core/webview/__tests__/ClineProvider.taskHistory.spec.ts
...
describe("ClineProvider Task History Synchronization", () => {
	...
	const findCallsByType = (calls, type) => {
	...
	describe("updateTaskHistory", () => {
		...
		it("invalidates recentTasksCache on updateTaskHistory (UTH-04)", async () => {
			...
			expect(provider.recentTasksCache).toEqual(initialRecent)
			await provider.updateTaskHistory(
				createHistoryItem({
					id: "cache-new",
					task: "Cache new",
		...
		it("updates existing task in history", async () => {
			...
			const matchingItems = storeHistory.filter((item) => item.id === "task-update")
	...
	describe("broadcastTaskHistoryUpdate", () => {
		it("sends taskHistoryUpdated message with sorted history", async () => {
			...
			const sentHistory = call?.[0]?.taskHistory
	...
	describe("task history includes all workspaces", () => {
		it("getStateToPostToWebview returns tasks from all workspaces", async () => {
			...
			expect(state.taskHistory.some((item) => item.workspace === "/path/to/workspace1")).toBe(true)
			expect(state.taskHistory.some((item) => item.workspace === "/path/to/workspace2")).toBe(true)
			expect(state.taskHistory.some((item) => item.workspace === "/different/workspace")).toBe(true)
		})
	})
	describe("taskHistory write lock (mutex)", () => {
		...
		it("serializes concurrent updates to the same item preserving the last write", async () => {
			...
			const history = provider.taskHistoryStore.getAll()
			const item = history.find((h) => h.id === "race-item")

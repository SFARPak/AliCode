...
const mcp_server_1 = require("../mcp_server")
// Helper to get the function property from a tool
const getFunction = (tool) => tool.function
describe("getMcpServerTools", () => {
	const createMockTool = (name, description = "Test tool") => ({
		name,
		description,
		inputSchema: {
			type: "object",
	...
	const createMockServer = (name, tools, source = "global") => ({
		name,
	...
	it("should skip servers without tools", () => {
		...
		const serverWithUndefinedTools = {

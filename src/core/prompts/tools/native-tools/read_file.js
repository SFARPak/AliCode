 ...
 */
function getReadFileSupportsNote(supportsImages) {
	if (supportsImages) {
...
function createReadFileTool(options = {}) {
	...
	const limitNote = ` By default, returns up to ${exports.DEFAULT_LINE_LIMIT} lines per file. Lines longer than ${exports.MAX_LINE_LENGTH} characters are truncated.`
	const description =
		descriptionIntro +
		modeDescription +
		limitNote +
	...
	const properties = {
		...
		limit: {
			type: "integer",
			description: `Maximum number of lines to return (slice mode, default: ${exports.DEFAULT_LINE_LIMIT})`,
		},
		indentation: {
			type: "object",
			description:
 ...
 */
exports.read_file = createReadFileTool()

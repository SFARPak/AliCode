...
async function resolveSymLink(symlinkPath, fileInfo, depth) {
	...
	try {
		...
		if (stats.isFile()) {
			...
			const anotherEntries = await promises_1.default.readdir(resolvedTarget, {
				withFileTypes: true,
				recursive: true,
			})
...
function formatDirectoryContent(files, cwd) {
	...
	return files
		.map((file) => {
			...
			const displayPath = path_1.default.relative(cwd, file.filename)
 ...
 */
async function loadRuleFiles(cwd, enableSubfolderRules = false) {
	const rules = []
	// Use recursive discovery only if enableSubfolderRules is true
	const rooDirectories = enableSubfolderRules
		? await (0, ali_config_1.getAllAliDirectoriesForCwd)(cwd)
		: (0, ali_config_1.getAliDirectoriesForCwd)(cwd)
	...
	for (const aliDir of rooDirectories) {
		const rulesDir = path_1.default.join(aliDir, "rules")
 ...
 */
async function readAgentRulesFile(filePath) {
	let resolvedPath = filePath
	// Check if file exists and handle symlinks
	try {
		const stats = await promises_1.default.lstat(filePath)
 ...
 */
async function loadAgentRulesFileFromDirectory(directory, showPath = false, cwd) {
	// Try both filenames - AGENTS.md (standard) first, then AGENT.md (alternative)
	const filenames = ["AGENTS.md", "AGENT.md"]
	const results = []
	const displayPath = cwd ? path_1.default.relative(cwd, directory) : directory
	for (const filename of filenames) {
		try {
			const agentPath = path_1.default.join(directory, filename)
	...
	try {
		...
		const localPath = path_1.default.join(directory, localFilename)
 ...
 */
async function loadAllAgentRulesFiles(cwd, enableSubfolderRules = false) {
	const agentRules = []
	...
	const directories = await (0, ali_config_1.getAgentsDirectoriesForCwd)(cwd)
...
async function addCustomInstructions(modeCustomInstructions, globalCustomInstructions, cwd, mode, options = {}) {
	...
	if (mode) {
		...
		for (const aliDir of rooDirectories) {
			const modeRulesDir = path_1.default.join(aliDir, `rules-${mode}`)
	...
	if (options.language) {
		const languageName = (0, types_1.isLanguage)(options.language)
			? language_1.LANGUAGES[options.language]
			: options.language
		sections.push(
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path_1.default.join(".ali", `rules-${mode}`))) {

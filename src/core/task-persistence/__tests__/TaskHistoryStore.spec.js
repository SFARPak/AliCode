"use strict"
// pnpm --filter alicode test core/task-persistence/__tests__/TaskHistoryStore.spec.ts
var __createBinding =
...
const TaskHistoryStore_1 = require("../TaskHistoryStore")
const globalFileNames_1 = require("../../../shared/globalFileNames")
vi.mock("../../../utils/storage", () => ({
	getStorageBasePath: vi.fn().mockImplementation((defaultPath) => defaultPath),
}))
// Mock safeWriteJson to use plain fs writes in tests (avoids proper-lockfile issues)
vi.mock("../../../utils/safeWriteJson", () => ({
	safeWriteJson: vi.fn().mockImplementation(async (filePath, data) => {
...
describe("TaskHistoryStore", () => {
	...
	beforeEach(async () => {
		...
		store = new TaskHistoryStore_1.TaskHistoryStore(tmpDir)
	})
	afterEach(async () => {
		store.dispose()
	...
	describe("initialize()", () => {
		...
		it("initializes from existing index file", async () => {
			...
			await fs.writeFile(
				path.join(tasksDir, "task-1", globalFileNames_1.GlobalFileNames.historyItem),
				JSON.stringify(item1),
			)
			await fs.writeFile(
				path.join(tasksDir, "task-2", globalFileNames_1.GlobalFileNames.historyItem),
			...
			await fs.writeFile(
				path.join(tasksDir, globalFileNames_1.GlobalFileNames.historyIndex),
	...
	describe("upsert()", () => {
		it("writes per-task file and updates cache", async () => {
			...
			const filePath = path.join(tmpDir, "tasks", "upsert-task", globalFileNames_1.GlobalFileNames.historyItem)
		...
		it("preserves existing metadata on partial updates (delegation fields)", async () => {
			...
			const partialUpdate = makeHistoryItem({
				id: "delegate-task",
				tokensIn: 500,
				tokensOut: 200,
			})
	...
	describe("reconcile()", () => {
		it("detects tasks on disk missing from index", async () => {
			...
			await fs.writeFile(path.join(taskDir, globalFileNames_1.GlobalFileNames.historyItem), JSON.stringify(item))
	...
	describe("migrateFromGlobalState()", () => {
		...
		it("does not overwrite existing per-task files", async () => {
			...
			await fs.writeFile(
				path.join(taskDir, globalFileNames_1.GlobalFileNames.historyItem),
			...
			const raw = await fs.readFile(path.join(taskDir, globalFileNames_1.GlobalFileNames.historyItem), "utf8")
	...
	describe("flushIndex()", () => {
		it("writes index to disk on flush", async () => {
			...
			const indexPath = path.join(tmpDir, "tasks", globalFileNames_1.GlobalFileNames.historyIndex)
	...
	describe("invalidate()", () => {
		it("re-reads a task from disk", async () => {
			...
			const filePath = path.join(
				...
				"tasks",
				"invalidate-task",
				globalFileNames_1.GlobalFileNames.historyItem,
			)
		...
		it("removes item from cache if file no longer exists", async () => {
			...
			await store.upsert(item)
			// Delete the file
			const filePath = path.join(tmpDir, "tasks", "gone-task", globalFileNames_1.GlobalFileNames.historyItem)

 ...
 */
exports.TOKEN_BUFFER_PERCENTAGE = 0.1
/**
 ...
 */
async function estimateTokenCount(content, apiHandler) {
 ...
 */
function truncateConversation(messages, fracToRemove, taskId) {
	const truncationId = crypto_1.default.randomUUID()
...
function willManageContext({
	...
	if (!autoCondenseContext) {
		...
		const reservedTokens = maxTokens || types_1.ANTHROPIC_DEFAULT_MAX_TOKENS
		const prevContextTokens = totalTokens + lastMessageTokens
		const allowedTokens = contextWindow * (1 - exports.TOKEN_BUFFER_PERCENTAGE) - reservedTokens
	...
	if (profileThreshold !== undefined) {
		if (profileThreshold === -1) {
			...
			profileThreshold >= condense_1.MIN_CONDENSE_THRESHOLD &&
			profileThreshold <= condense_1.MAX_CONDENSE_THRESHOLD
		) {
...
async function manageContext({
	...
	if (autoCondenseContext) {
		...
		if (contextPercent >= effectiveThreshold || prevContextTokens > allowedTokens) {
			...
			const result = await (0, condense_1.summarizeConversation)({
				messages,
				apiHandler,
				systemPrompt,
				taskId,

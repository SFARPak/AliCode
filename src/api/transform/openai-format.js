...
exports.consolidateReasoningDetails = consolidateReasoningDetails
exports.sanitizeGeminiMessages = sanitizeGeminiMessages
 ...
 * @see https://github.com/cline/cline/issues/8214
 */
function sanitizeGeminiMessages(messages, modelId) {
		...
		if (msg.role === "assistant") {
			...
			if (Array.isArray(toolCalls) && toolCalls.length > 0) {
				...
				if (!hasReasoningDetails) {
					...
					if (anyMsg.content) {
						sanitized.push({ role: "assistant", content: anyMsg.content })
					}
					continue
				}
...
function convertToOpenAiMessages(anthropicMessages, options) {
	const openAiMessages = []
	const mapReasoningDetails = (details) => {
	...
	const normalizeId = options?.normalizeToolCallId ?? ((id) => id)
	for (const anthropicMessage of anthropicMessages) {
		if (typeof anthropicMessage.content === "string") {
			...
			const messageWithDetails = anthropicMessage
			const baseMessage = {
			...
			if (anthropicMessage.role === "user") {
				...
				if (filteredNonToolMessages.length > 0) {
					...
					if (shouldMergeIntoToolMessage) {
						...
						const lastToolMessage = openAiMessages[openAiMessages.length - 1]
						if (lastToolMessage?.role === "tool") {
							const additionalText = filteredNonToolMessages.map((part) => part.text).join("\n")
				...
				// the order expected by providers like Ali. Property order matters

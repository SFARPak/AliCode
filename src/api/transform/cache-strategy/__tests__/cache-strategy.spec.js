...
const multi_point_strategy_1 = require("../multi-point-strategy")
...
const createMessageWithTokens = (role, tokenCount) => ({
	role,
	content: "x".repeat(tokenCount * 4), // Approximate 4 chars per token
})
const hasCachePoint = (block) => {
	return (
		"cachePoint" in block &&
...
describe("Cache Strategy", () => {
	...
	describe("Strategy Implementation", () => {
		describe("Strategy Selection", () => {
			it("should use MultiPointStrategy when caching is not supported", () => {
				...
				const strategy = new multi_point_strategy_1.MultiPointStrategy(config)
				expect(strategy).toBeInstanceOf(multi_point_strategy_1.MultiPointStrategy)
			})
	...
	describe("AwsBedrockHandler Integration", () => {
		...
		beforeEach(() => {
			...
			vitest.spyOn(handler, "convertToBedrockConverseMessages").mockImplementation(function (...args) {
				...
				strategy = new multi_point_strategy_1.MultiPointStrategy(config)
				// Store the result
				const result = strategy.determineOptimalCachePoints()
		...
		it("should select MultiPointStrategy when conditions are met", async () => {
			...
			if (convertToBedrockConverseMessagesMock.lastConfig) {
				const strategy = new multi_point_strategy_1.MultiPointStrategy(
					convertToBedrockConverseMessagesMock.lastConfig,
				)
				expect(strategy).toBeInstanceOf(multi_point_strategy_1.MultiPointStrategy)
			}
		})
		...
		it("should include cachePoint nodes in API request when using MultiPointStrategy", async () => {
			...
			handler.convertToBedrockConverseMessages.mockReturnValueOnce({
	...
	describe("Multi-Point Strategy Cache Point Placement", () => {
		...
		const createMessage = (role, content, tokenCount) => {
		...
		const logPlacements = (placements) => {
			console.log(
		...
		describe("Cache Point Optimization", () => {
			...
			it.skip("documentation example 5 verification", () => {
				...
				const _previousCachePointPlacements = [
					{
						index: 2, // After the second user message
						type: "message",
						tokensCovered: 240,

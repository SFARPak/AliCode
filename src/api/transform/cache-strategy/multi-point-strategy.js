...
exports.MultiPointStrategy = void 0
 ...
 */
class MultiPointStrategy extends base_strategy_1.CacheStrategy {
	/**
	...
	determineOptimalCachePoints() {
		...
		let remainingCachePoints = this.config.modelInfo.maxCachePoints
		...
		if (this.config.systemPrompt) {
			systemBlocks = [{ text: this.config.systemPrompt }]
			if (useSystemCache) {
				systemBlocks.push(this.createCachePoint())
	 ...
	 */
	determineMessageCachePoints(minTokensPerPoint, remainingCachePoints) {
		...
		if (newMessagesTokens >= minTokensPerPoint) {
			...
			if (remainingCachePoints > previousPlacements.length) {
				...
				if (newMessagesTokens >= requiredTokenThreshold) {
					...
					logging_1.logger.info("Combining cache points is beneficial", {
						ctx: "cache-strategy",
						newMessagesTokens,
						smallestGap,
					...
					logging_1.logger.info("Combining cache points is not beneficial", {
						ctx: "cache-strategy",
						newMessagesTokens,
						smallestGap,
	 ...
	 */
	findOptimalPlacementForRange(startIndex, endIndex, minTokensPerPoint) {
	 ...
	 */
	formatWithoutCachePoints() {
		const systemBlocks = this.config.systemPrompt ? [{ text: this.config.systemPrompt }] : []
		return this.formatResult(systemBlocks, this.messagesToContentBlocks(this.config.messages))
	}
}
exports.MultiPointStrategy = MultiPointStrategy

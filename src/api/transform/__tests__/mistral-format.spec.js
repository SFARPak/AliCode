...
const mistral_format_1 = require("../mistral-format")
describe("normalizeMistralToolCallId", () => {
	it("should strip non-alphanumeric characters and truncate to 9 characters", () => {
		...
		expect((0, mistral_format_1.normalizeMistralToolCallId)("call_5019f900a247472bacde0b82")).toBe("call5019f")
	})
	it("should handle Anthropic-style tool call IDs", () => {
		// Anthropic-style tool call ID
		expect((0, mistral_format_1.normalizeMistralToolCallId)("toolu_01234567890abcdef")).toBe("toolu0123")
	})
	it("should pad short IDs to 9 characters", () => {
		expect((0, mistral_format_1.normalizeMistralToolCallId)("abc")).toBe("abc000000")
		expect((0, mistral_format_1.normalizeMistralToolCallId)("tool-1")).toBe("tool10000")
	})
	it("should handle IDs that are exactly 9 alphanumeric characters", () => {
		expect((0, mistral_format_1.normalizeMistralToolCallId)("abcd12345")).toBe("abcd12345")
	})
	it("should return consistent results for the same input", () => {
		const id = "call_5019f900a247472bacde0b82"
		expect((0, mistral_format_1.normalizeMistralToolCallId)(id)).toBe(
			(0, mistral_format_1.normalizeMistralToolCallId)(id),
		)
	})
	it("should handle edge cases", () => {
		// Empty string
		expect((0, mistral_format_1.normalizeMistralToolCallId)("")).toBe("000000000")
		// Only non-alphanumeric characters
		expect((0, mistral_format_1.normalizeMistralToolCallId)("---___---")).toBe("000000000")
		// Mixed special characters
		expect((0, mistral_format_1.normalizeMistralToolCallId)("a-b_c.d@e")).toBe("abcde0000")
	})
})
describe("convertToMistralMessages", () => {
	it("should convert simple text messages for user and assistant roles", () => {
		...
		const mistralMessages = (0, mistral_format_1.convertToMistralMessages)(anthropicMessages)
	...
	it("should handle user messages with image content", () => {
		...
		const content = mistralMessages[0].content
	...
	it("should handle user messages with only tool results", () => {
		...
		expect(mistralMessages[0].toolCallId).toBe((0, mistral_format_1.normalizeMistralToolCallId)("weather-123"))
	...
	it("should handle a conversation with mixed message types", () => {
		...
		expect(mistralMessages[0].role).toBe("user")
		const userContent = mistralMessages[0].content
		...
		expect(mistralMessages[2].toolCallId).toBe((0, mistral_format_1.normalizeMistralToolCallId)("search-123"))

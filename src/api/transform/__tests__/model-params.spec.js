...
const model_params_1 = require("../model-params")
...
describe("getModelParams", () => {
	...
	describe("Basic functionality", () => {
		it("should return default values when no custom values are provided", () => {
			const result = (0, model_params_1.getModelParams)({
				...anthropicParams,
				settings: {},
				model: baseModel,
		...
		it("should use model maxTokens when available", () => {
			...
			expect((0, model_params_1.getModelParams)({ ...anthropicParams, settings: {}, model })).toEqual({
				format: anthropicParams.format,
				maxTokens: 2000,
				temperature: 0,
				reasoningEffort: undefined,
		...
		it("should handle null maxTokens in model", () => {
			...
			const result = (0, model_params_1.getModelParams)({ ...anthropicParams, settings: {}, model })
			expect(result.maxTokens).toBe(types_1.ANTHROPIC_DEFAULT_MAX_TOKENS)
		})
	})
	...
	describe("Reasoning Budget (Hybrid reasoning models)", () => {
		it("should handle requiredReasoningBudget models correctly", () => {
			...
			expect(
				(0, model_params_1.getModelParams)({ ...anthropicParams, settings: { modelMaxTokens: 2000 }, model }),
			).toEqual({
				format: anthropicParams.format,
				maxTokens: 2000,
		...
		it("should honor customMaxTokens for reasoning budget models", () => {
			...
			expect(
				(0, model_params_1.getModelParams)({ ...anthropicParams, settings: { modelMaxTokens: 3000 }, model }),
			).toEqual({
				format: anthropicParams.format,
				maxTokens: 3000,
				temperature: 1.0,
		...
		it("should honor customMaxThinkingTokens for reasoning budget models", () => {
			...
			expect(
				(0, model_params_1.getModelParams)({
					...anthropicParams,
		...
		it("should use DEFAULT_HYBRID_REASONING_MODEL_MAX_TOKENS when no maxTokens is provided for reasoning budget models", () => {
			...
			expect((0, model_params_1.getModelParams)({ ...anthropicParams, settings: {}, model })).toEqual({
				format: anthropicParams.format,
				maxTokens: api_1.DEFAULT_HYBRID_REASONING_MODEL_MAX_TOKENS,
				temperature: 1.0,
				reasoningEffort: undefined,
				reasoningBudget: api_1.DEFAULT_HYBRID_REASONING_MODEL_THINKING_TOKENS,
				reasoning: {
					type: "enabled",
					budget_tokens: api_1.DEFAULT_HYBRID_REASONING_MODEL_THINKING_TOKENS,
				},
			})
		})
	...
	describe("Reasoning Effort (Traditional reasoning models)", () => {
		...
		it("should include 'minimal' and 'none' for openrouter format", () => {
			...
			const minimalRes = (0, model_params_1.getModelParams)({
				...openrouterParams,
				settings: { reasoningEffort: "minimal" },
				model,
			})
			...
			const noneRes = (0, model_params_1.getModelParams)({
				...openrouterParams,
				settings: { reasoningEffort: "none" },
				model,
			})
	...
	describe("Edge cases and combinations", () => {
		...
		it("should handle zero maxTokens", () => {
			...
			expect(result.maxTokens).toBe(types_1.ANTHROPIC_DEFAULT_MAX_TOKENS) // Should fallback for anthropic
		})

	...
	describe("toolUseId sanitization for Bedrock 64-char limit", () => {
		it("truncates toolUseId longer than 64 characters in tool_use blocks", () => {
			...
			if ("toolUse" in toolBlock && toolBlock.toolUse && toolBlock.toolUse.toolUseId) {
				expect(toolBlock.toolUse.toolUseId.length).toBeLessThanOrEqual(tool_id_1.OPENAI_CALL_ID_MAX_LENGTH)
				expect(toolBlock.toolUse.toolUseId.length).toBe(tool_id_1.OPENAI_CALL_ID_MAX_LENGTH)
		...
		it("truncates toolUseId longer than 64 characters in tool_result blocks with string content", () => {
			...
			if ("toolResult" in resultBlock && resultBlock.toolResult && resultBlock.toolResult.toolUseId) {
				expect(resultBlock.toolResult.toolUseId.length).toBeLessThanOrEqual(tool_id_1.OPENAI_CALL_ID_MAX_LENGTH)
				expect(resultBlock.toolResult.toolUseId.length).toBe(tool_id_1.OPENAI_CALL_ID_MAX_LENGTH)
		...
		it("produces consistent truncated IDs for the same input", () => {
			...
			const result1 = (0, bedrock_converse_format_1.convertToBedrockConverseMessages)(messages)
			const result2 = (0, bedrock_converse_format_1.convertToBedrockConverseMessages)(messages)
			const toolBlock1 = result1[0]?.content?.[0]
			const toolBlock2 = result2[0]?.content?.[0]
		...
		it("produces different truncated IDs for different long inputs", () => {
			...
			const result1 = (0, bedrock_converse_format_1.convertToBedrockConverseMessages)(messages1)
			const result2 = (0, bedrock_converse_format_1.convertToBedrockConverseMessages)(messages2)
			const toolBlock1 = result1[0]?.content?.[0]
			const toolBlock2 = result2[0]?.content?.[0]
			if ("toolUse" in toolBlock1 && toolBlock1.toolUse && "toolUse" in toolBlock2 && toolBlock2.toolUse) {
		...
		it("matching tool_use and tool_result IDs are both truncated consistently", () => {
			...
			const result = (0, bedrock_converse_format_1.convertToBedrockConverseMessages)(messages)
			const toolUseBlock = result[0]?.content?.[0]
			const toolResultBlock = result[1]?.content?.[0]
			if (
				...
				expect(toolUseBlock.toolUse.toolUseId.length).toBeLessThanOrEqual(tool_id_1.OPENAI_CALL_ID_MAX_LENGTH)
			} else {

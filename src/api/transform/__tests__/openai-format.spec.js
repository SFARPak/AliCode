...
const openai_format_1 = require("../openai-format")
...
describe("convertToOpenAiMessages", () => {
	it("should convert simple text messages", () => {
		...
		const openAiMessages = (0, openai_format_1.convertToOpenAiMessages)(anthropicMessages)
	...
	it("should normalize tool call IDs when normalizeToolCallId function is provided", () => {
		...
		const openAiMessages = (0, openai_format_1.convertToOpenAiMessages)(anthropicMessages, {
			normalizeToolCallId: mistral_format_1.normalizeMistralToolCallId,
		})
		const assistantMessage = openAiMessages[0]
		expect(assistantMessage.tool_calls[0].id).toBe(
			(0, mistral_format_1.normalizeMistralToolCallId)("call_5019f900a247472bacde0b82"),
		)
		const toolMessage = openAiMessages[1]
		expect(toolMessage.tool_call_id).toBe(
			(0, mistral_format_1.normalizeMistralToolCallId)("call_5019f900a247472bacde0b82"),
		)
	})
	...
	it("should use custom normalization function when provided", () => {
		...
		const customNormalizer = (id) => `custom_${id}`
		const openAiMessages = (0, openai_format_1.convertToOpenAiMessages)(anthropicMessages, {
			normalizeToolCallId: customNormalizer,
		})
		const assistantMessage = openAiMessages[0]
	...
	describe("mergeToolResultText option", () => {
		...
		it("should merge text into last tool message when multiple tool results exist", () => {
			...
			expect(openAiMessages).toHaveLength(2)
			expect(openAiMessages[0].content).toBe("First result")
			expect(openAiMessages[1].content).toBe(
		...
		it("should create separate user message when mergeToolResultText is false", () => {
			...
			expect(openAiMessages).toHaveLength(2)
			expect(openAiMessages[0].role).toBe("tool")
			expect(openAiMessages[0].content).toBe("Tool result content")
			expect(openAiMessages[1].role).toBe("user")
		})
	...
	describe("reasoning_details transformation", () => {
		...
		it("should pass through all reasoning_details without extracting to top-level reasoning", () => {
			// This simulates the stored format after receiving from xAI/Ali API
...
describe("consolidateReasoningDetails", () => {
	it("should return empty array for empty input", () => {
		expect((0, openai_format_1.consolidateReasoningDetails)([])).toEqual([])
	})
	it("should return empty array for undefined input", () => {
		expect((0, openai_format_1.consolidateReasoningDetails)(undefined)).toEqual([])
	})
	it("should filter out corrupted encrypted blocks (missing data field)", () => {
		...
		const result = (0, openai_format_1.consolidateReasoningDetails)(details)
...
describe("sanitizeGeminiMessages", () => {
	it("should return messages unchanged for non-Gemini models", () => {
		...
		const result = (0, openai_format_1.sanitizeGeminiMessages)(messages, "anthropic/claude-3-5-sonnet")
	...
	it("should drop tool calls without reasoning_details for Gemini models", () => {
		...
		const result = (0, openai_format_1.sanitizeGeminiMessages)(messages, "google/gemini-3-flash-preview")

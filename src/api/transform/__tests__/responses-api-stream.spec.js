...
const responses_api_stream_1 = require("../responses-api-stream")
...
describe("processResponsesApiStream", () => {
	describe("text deltas", () => {
		it("should yield text chunk for response.output_text.delta", async () => {
			...
			const chunks = await collectChunks((0, responses_api_stream_1.processResponsesApiStream)(stream, noopUsage))
	...
	describe("completion and usage", () => {
		it("should yield usage from response.completed", async () => {
			...
			const chunks = await collectChunks(
				(0, responses_api_stream_1.processResponsesApiStream)(stream, mockNormalize),
			)
...
describe("createUsageNormalizer", () => {
	it("should return undefined for null/undefined usage", () => {
		const normalize = (0, responses_api_stream_1.createUsageNormalizer)()
	...
	it("should compute totalCost when calculateCost is provided", () => {
		const calculateCost = (input, output, cached) => 0.42
		const normalize = (0, responses_api_stream_1.createUsageNormalizer)(calculateCost)
		const result = normalize({ input_tokens: 100, output_tokens: 50 })

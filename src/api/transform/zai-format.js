...
exports.convertToZAiFormat = convertToZAiFormat
/**
 ...
 */
function convertToZAiFormat(messages, options) {
	const result = []
		...
		if (message.role === "user") {
			...
			if (Array.isArray(message.content)) {
				...
				if (textParts.length > 0 || imageParts.length > 0) {
					...
					if (shouldMergeIntoToolMessage) {
						...
						const lastToolMessage = result[result.length - 1]
			...
			if (Array.isArray(message.content)) {
				...
				if (lastMessage?.role === "assistant" && !toolCalls.length && !lastMessage.tool_calls) {
					...
					if (finalReasoning) {
						lastMessage.reasoning_content = finalReasoning
					}
				} else {

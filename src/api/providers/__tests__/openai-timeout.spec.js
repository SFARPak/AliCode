...
describe("OpenAiHandler timeout configuration", () => {
	...
	it("should use default timeout for standard OpenAI", () => {
		...
		new openai_1.OpenAiHandler(options)
	...
	it("should use custom timeout for OpenAI-compatible providers", () => {
		timeout_config_1.getApiRequestTimeout.mockReturnValue(1800000) // 30 minutes
		const options = {
	...
	it("should use timeout for Azure OpenAI", () => {
		timeout_config_1.getApiRequestTimeout.mockReturnValue(900000) // 15 minutes
		const options = {
			apiModelId: "gpt-4",
			openAiModelId: "gpt-4",

...
const GEMINI_MODEL_NAME = types_1.geminiDefaultModelId
describe("GeminiHandler", () => {
	...
	beforeEach(() => {
		...
		handler = new gemini_1.GeminiHandler({
			apiKey: "test-key",
	...
	describe("createMessage", () => {
		...
		it("should handle text messages correctly", async () => {
			...
			handler["client"].models.generateContentStream.mockResolvedValue({
		...
		it("should handle API errors", async () => {
			...
			handler["client"].models.generateContentStream.mockRejectedValue(mockError)
			const stream = handler.createMessage(systemPrompt, mockMessages)
			await expect(async () => {
	...
	describe("completePrompt", () => {
		it("should complete prompt successfully", async () => {
			...
			handler["client"].models.generateContent.mockResolvedValue({
				text: "Test response",
			})
		...
		it("should handle API errors", async () => {
			const mockError = new Error("Gemini API error")
			handler["client"].models.generateContent.mockRejectedValue(mockError)
			await expect(handler.completePrompt("Test prompt")).rejects.toThrow(
				(0, i18next_1.t)("common:errors.gemini.generate_complete_prompt", { error: "Gemini API error" }),
			)
		})
	...
	describe("getModel", () => {
		...
		it("should return default model if invalid model specified", () => {
			const invalidHandler = new gemini_1.GeminiHandler({
				apiModelId: "invalid-model",
				geminiApiKey: "test-key",
			})
			const modelInfo = invalidHandler.getModel()
			expect(modelInfo.id).toBe(types_1.geminiDefaultModelId) // Default model
		})
		...
		it("should not duplicate tool entries if already present", () => {
			const modelInfo = handler.getModel()
			const excludedCount = modelInfo.info.excludedTools.filter((t) => t === "apply_diff").length
			const includedCount = modelInfo.info.includedTools.filter((t) => t === "edit").length
	...
	describe("calculateCost", () => {
		...
		it("should return undefined if pricing info is missing", () => {
			...
			const incompleteInfo = { ...mockInfo, outputPrice: undefined }

...
const ollama_1 = require("../fetchers/ollama")
...
const mockGetOllamaModels = vitest.mocked(ollama_1.getOllamaModels)
describe("NativeOllamaHandler", () => {
	...
	describe("tool calling", () => {
		it("should include tools when tools are provided", async () => {
			...
			const stream = handler.createMessage("System", [{ role: "user", content: "What's the weather?" }], {
				taskId: "test",
				tools,
			})
			// Consume the stream
		...
		it("should yield tool_call_partial when model returns tool calls", async () => {
			...
			const stream = handler.createMessage("System", [{ role: "user", content: "What's the weather in SF?" }], {
				taskId: "test",
				tools,
			})
			const results = []

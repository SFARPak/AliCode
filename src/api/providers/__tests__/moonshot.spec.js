...
const moonshot_1 = require("../moonshot")
describe("MoonshotHandler", () => {
	...
	beforeEach(() => {
		...
		handler = new moonshot_1.MoonshotHandler(mockOptions)
		vi.clearAllMocks()
	})
	describe("constructor", () => {
		it("should initialize with provided options", () => {
			expect(handler).toBeInstanceOf(moonshot_1.MoonshotHandler)
			expect(handler.getModel().id).toBe(mockOptions.apiModelId)
		})
		it("should use default model ID if not provided", () => {
			const handlerWithoutModel = new moonshot_1.MoonshotHandler({
				...mockOptions,
				apiModelId: undefined,
			})
			expect(handlerWithoutModel.getModel().id).toBe(types_1.moonshotDefaultModelId)
		})
		it("should use default base URL if not provided", () => {
			const handlerWithoutBaseUrl = new moonshot_1.MoonshotHandler({
				...mockOptions,
				moonshotBaseUrl: undefined,
			})
			expect(handlerWithoutBaseUrl).toBeInstanceOf(moonshot_1.MoonshotHandler)
		})
		it("should use chinese base URL if provided", () => {
			const customBaseUrl = "https://api.moonshot.cn/v1"
			const handlerWithCustomUrl = new moonshot_1.MoonshotHandler({
				...mockOptions,
				moonshotBaseUrl: customBaseUrl,
			})
			expect(handlerWithCustomUrl).toBeInstanceOf(moonshot_1.MoonshotHandler)
		})
	})
	describe("getModel", () => {
		...
		it("should return provided model ID with default model info if model does not exist", () => {
			const handlerWithInvalidModel = new moonshot_1.MoonshotHandler({
				...mockOptions,
		...
		it("should return default model if no model ID is provided", () => {
			...
			expect(model.id).toBe(types_1.moonshotDefaultModelId)
			expect(model.info).toBeDefined()
			expect(model.info.supportsPromptCache).toBe(true)
		})
	...
	describe("processUsageMetrics", () => {
		it("should correctly process usage metrics including cache information", () => {
			...
			class TestMoonshotHandler extends moonshot_1.MoonshotHandler {

...
describe("OpenAiNativeHandler", () => {
	...
	beforeEach(() => {
		...
		handler = new openai_native_1.OpenAiNativeHandler(mockOptions)
	...
	describe("constructor", () => {
		it("should initialize with provided options", () => {
			expect(handler).toBeInstanceOf(openai_native_1.OpenAiNativeHandler)
			expect(handler.getModel().id).toBe(mockOptions.apiModelId)
		})
		it("should initialize with empty API key", () => {
			const handlerWithoutKey = new openai_native_1.OpenAiNativeHandler({
				apiModelId: "gpt-4.1",
				openAiNativeApiKey: "",
			})
			expect(handlerWithoutKey).toBeInstanceOf(openai_native_1.OpenAiNativeHandler)
		})
		it("should pass undefined baseURL when openAiNativeBaseUrl is empty string", () => {
			openai_1.default.mockClear()
			new openai_native_1.OpenAiNativeHandler({
				apiModelId: "gpt-4.1",
				openAiNativeApiKey: "test-key",
				openAiNativeBaseUrl: "",
			})
			expect(openai_1.default).toHaveBeenCalledWith(expect.objectContaining({ baseURL: undefined }))
		})
	...
	describe("getModel", () => {
		...
		it("should return GPT-5.3 Codex model info when selected", () => {
			const codexHandler = new openai_native_1.OpenAiNativeHandler({
				...mockOptions,
		...
		it("should return GPT-5.4 model info when selected", () => {
			const gpt54Handler = new openai_native_1.OpenAiNativeHandler({
				...mockOptions,
				apiModelId: "gpt-5.4",
			})
		...
		it("should return GPT-5.4 Mini model info when selected", () => {
			const gpt54MiniHandler = new openai_native_1.OpenAiNativeHandler({
				...mockOptions,
		...
		it("should return GPT-5.4 Nano model info when selected", () => {
			const gpt54NanoHandler = new openai_native_1.OpenAiNativeHandler({
				...mockOptions,
		...
		it("should return GPT-5.3 Chat model info when selected", () => {
			const chatHandler = new openai_native_1.OpenAiNativeHandler({
				...mockOptions,
		...
		it("should handle undefined model ID", () => {
			const handlerWithoutModel = new openai_native_1.OpenAiNativeHandler({
				openAiNativeApiKey: "test-api-key",
			})
	...
	describe("GPT-5 models", () => {
		...
		it("should support minimal reasoning effort for GPT-5", async () => {
			...
			handler = new openai_native_1.OpenAiNativeHandler({
				...mockOptions,
				apiModelId: "gpt-5.1",
				reasoningEffort: "minimal", // GPT-5 supports minimal
			})
			const stream = handler.createMessage(systemPrompt, messages)
			const chunks = []
			for await (const chunk of stream) {
		...
		it("should omit reasoning when selection is 'disable'", async () => {
			...
			// Mock SDK to fail
			mockResponsesCreate.mockRejectedValue(new Error("SDK not available"))
			const handler = new openai_native_1.OpenAiNativeHandler({
				...mockOptions,
				apiModelId: "gpt-5.1",
		...
		it("should support low reasoning effort for GPT-5", async () => {
			...
			const body2 = mockFetch.mock.calls[0][1].body
		...
		it("should support both verbosity and reasoning effort together for GPT-5", async () => {
			...
			const body3 = mockFetch.mock.calls[0][1].body
		...
		it("should format full conversation correctly", async () => {
			...
			global.fetch = mockFetch
			mockResponsesCreate.mockRejectedValue(new Error("SDK not available"))
			const gpt5Handler = new openai_native_1.OpenAiNativeHandler({
				...mockOptions,
				apiModelId: "gpt-5.1",
			})
...
describe("GPT-5 streaming event coverage (additional)", () => {
	...
	it("should handle refusal delta events for GPT-5 and prefix output", async () => {
		...
		const systemPrompt = "You are a helpful assistant."
		const messages = [{ role: "user", content: "Do something disallowed" }]
		const stream = handler.createMessage(systemPrompt, messages)
		const chunks = []
		for await (const chunk of stream) {
			chunks.push(chunk)
	...
	it("should ignore malformed JSON lines in SSE stream", async () => {
		...
		const textChunks = chunks.filter((c) => c.type === "text")
		expect(textChunks.map((c) => c.text)).toEqual(["Before", "After"])
	})
	describe("Codex Mini Model", () => {
		...
		it("should handle codex-mini-latest streaming response", async () => {
			...
			const messages = [{ role: "user", content: "Write a hello world function" }]
			const stream = handler.createMessage(systemPrompt, messages)
			const chunks = []
			for await (const chunk of stream) {
				chunks.push(chunk)

...
describe("OpenAiHandler", () => {
	...
	beforeEach(() => {
		...
		handler = new openai_1.OpenAiHandler(mockOptions)
		mockCreate.mockClear()
	})
	describe("constructor", () => {
		it("should initialize with provided options", () => {
			expect(handler).toBeInstanceOf(openai_1.OpenAiHandler)
			expect(handler.getModel().id).toBe(mockOptions.openAiModelId)
		})
		it("should use custom base URL if provided", () => {
			const customBaseUrl = "https://custom.openai.com/v1"
			const handlerWithCustomUrl = new openai_1.OpenAiHandler({
				...mockOptions,
				openAiBaseUrl: customBaseUrl,
			})
			expect(handlerWithCustomUrl).toBeInstanceOf(openai_1.OpenAiHandler)
		})
		it("should set default headers correctly", () => {
			// Check that the OpenAI constructor was called with correct parameters
			expect(vi.mocked(openai_2.default)).toHaveBeenCalledWith({
	...
	describe("createMessage", () => {
		...
		it("should handle non-streaming mode", async () => {
			const handler = new openai_1.OpenAiHandler({
				...mockOptions,
		...
		it("should include reasoning_effort when reasoning effort is enabled", async () => {
			...
			const reasoningHandler = new openai_1.OpenAiHandler(reasoningOptions)
		...
		it("should not include reasoning_effort when reasoning effort is disabled", async () => {
			...
			const noReasoningHandler = new openai_1.OpenAiHandler(noReasoningOptions)
		...
		it("should include max_tokens when includeMaxTokens is true", async () => {
			...
			const handlerWithMaxTokens = new openai_1.OpenAiHandler(optionsWithMaxTokens)
		...
		it("should not include max_tokens when includeMaxTokens is false", async () => {
			...
			const handlerWithoutMaxTokens = new openai_1.OpenAiHandler(optionsWithoutMaxTokens)
		...
		it("should not include max_tokens when includeMaxTokens is undefined", async () => {
			const optionsWithUndefinedMaxTokens = {
				...mockOptions,
			...
			const handlerWithDefaultMaxTokens = new openai_1.OpenAiHandler(optionsWithUndefinedMaxTokens)
		...
		it("should use user-configured modelMaxTokens instead of model default maxTokens", async () => {
			const optionsWithUserMaxTokens = {
				...mockOptions,
				includeMaxTokens: true,
			...
			const handlerWithUserMaxTokens = new openai_1.OpenAiHandler(optionsWithUserMaxTokens)
		...
		it("should fallback to model default maxTokens when user modelMaxTokens is not set", async () => {
			const optionsWithoutUserMaxTokens = {
				...mockOptions,
				includeMaxTokens: true,
			...
			const handlerWithoutUserMaxTokens = new openai_1.OpenAiHandler(optionsWithoutUserMaxTokens)
	...
	describe("getModel", () => {
		...
		it("should handle undefined model ID", () => {
			const handlerWithoutModel = new openai_1.OpenAiHandler({
				...mockOptions,
	...
	describe("Azure AI Inference Service", () => {
		...
		it("should initialize with Azure AI Inference Service configuration", () => {
			const azureHandler = new openai_1.OpenAiHandler(azureOptions)
			expect(azureHandler).toBeInstanceOf(openai_1.OpenAiHandler)
		...
		it("should handle non-streaming responses with Azure AI Inference Service", async () => {
			const azureHandler = new openai_1.OpenAiHandler({
				...azureOptions,
				openAiStreamingEnabled: false,
			})
			const systemPrompt = "You are a helpful assistant."
	...
	describe("Grok xAI Provider", () => {
		...
		it("should initialize with Grok xAI configuration", () => {
			const grokHandler = new openai_1.OpenAiHandler(grokOptions)
			expect(grokHandler).toBeInstanceOf(openai_1.OpenAiHandler)
	...
	describe("O3 Family Models", () => {
		...
		it("should handle O3 model with streaming and include max_completion_tokens when includeMaxTokens is true", async () => {
			const o3Handler = new openai_1.OpenAiHandler({
				...o3Options,
				includeMaxTokens: true,
		...
		it("should handle tool calls with O3 model in streaming mode", async () => {
			const o3Handler = new openai_1.OpenAiHandler(o3Options)
			mockCreate.mockImplementation(async (options) => {
				return {
					[Symbol.asyncIterator]: async function* () {
						yield {
		...
		it("should handle O3 model with Azure AI Inference Service respecting includeMaxTokens", async () => {
			const o3AzureHandler = new openai_1.OpenAiHandler({
				...o3Options,
				openAiBaseUrl: "https://test.services.ai.azure.com",
...
describe("getOpenAiModels", () => {
	...
	it("should trim whitespace from baseUrl", async () => {
		...
		vi.mocked(axios_1.default.get).mockResolvedValueOnce(mockResponse)
		const result = await (0, openai_1.getOpenAiModels)("  https://api.openai.com/v1  ", "test-key")
		expect(axios_1.default.get).toHaveBeenCalledWith("https://api.openai.com/v1/models", expect.any(Object))
	...
	it("should handle baseUrl with trailing spaces", async () => {
		...
		vi.mocked(axios_1.default.get).mockResolvedValueOnce(mockResponse)
		const result = await (0, openai_1.getOpenAiModels)("https://api.example.com/v1 ", "test-key")
		expect(axios_1.default.get).toHaveBeenCalledWith("https://api.example.com/v1/models", expect.any(Object))
	...
	it("should return empty array for invalid URL after trimming", async () => {
		const result = await (0, openai_1.getOpenAiModels)("   not-a-valid-url   ", "test-key")
		expect(result).toEqual([])
		expect(axios_1.default.get).not.toHaveBeenCalled()
	})
	it("should include authorization header when apiKey is provided", async () => {
		...
		vi.mocked(axios_1.default.get).mockResolvedValueOnce(mockResponse)
		await (0, openai_1.getOpenAiModels)("https://api.example.com/v1", "test-api-key")
	...
	it("should include custom headers when provided", async () => {
		...
		await (0, openai_1.getOpenAiModels)("https://api.example.com/v1", "test-key", customHeaders)
		expect(axios_1.default.get).toHaveBeenCalledWith(
			"https://api.example.com/v1/models",
			expect.objectContaining({
				headers: expect.objectContaining({
	...
	it("should handle API errors gracefully", async () => {
		vi.mocked(axios_1.default.get).mockRejectedValueOnce(new Error("Network error"))
		const result = await (0, openai_1.getOpenAiModels)("https://api.example.com/v1", "test-key")
		expect(result).toEqual([])
	})
	it("should handle malformed response data", async () => {
		vi.mocked(axios_1.default.get).mockResolvedValueOnce({ data: null })
		const result = await (0, openai_1.getOpenAiModels)("https://api.example.com/v1", "test-key")
		expect(result).toEqual([])
	})

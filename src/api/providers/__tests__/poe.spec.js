...
vitest.mock("ai-sdk-provider-poe", () => ({
	createPoe: (...args) => mockCreatePoe(...args),
}))
vitest.mock("ai-sdk-provider-poe/code", () => ({
	mapToolChoice: vitest.fn((value) => value),
	extractUsageMetrics: vitest.fn((usage) => ({
...
describe("PoeHandler", () => {
	...
	describe("constructor", () => {
		it("creates poe provider with api key and default base URL", () => {
			new poe_1.PoeHandler({ poeApiKey: "test-key" })
		...
		it("creates poe provider with custom base URL", () => {
			new poe_1.PoeHandler({ poeApiKey: "key", poeBaseUrl: "https://custom.poe.com/v1" })
			expect(mockCreatePoe).toHaveBeenCalledWith({
				apiKey: "key",
	...
	describe("getModel", () => {
		it("returns model info from cache", () => {
			const handler = new poe_1.PoeHandler({ poeApiKey: "key", apiModelId: "anthropic/claude-sonnet-4" })
		...
		it("returns default model when no model ID specified", () => {
			const handler = new poe_1.PoeHandler({ poeApiKey: "key" })
			const result = handler.getModel()
			expect(result.id).toBe(types_1.poeDefaultModelId)
		})
		it("falls back to default model info when model not in cache", () => {
			const handler = new poe_1.PoeHandler({ poeApiKey: "key", apiModelId: "unknown/model" })
			const result = handler.getModel()
	...
	describe("reasoning", () => {
		it("passes anthropic thinking config for budget models", async () => {
			const handler = new poe_1.PoeHandler({
				poeApiKey: "key",
		...
		it("passes openai reasoning effort for effort models", async () => {
			...
			for await (const _ of handler.createMessage("system", [{ role: "user", content: "reason" }])) {
				/* drain */
			}
	...
	describe("completePrompt", () => {
		it("returns generated text", async () => {
			const handler = new poe_1.PoeHandler({ poeApiKey: "key", apiModelId: "openai/gpt-4o" })

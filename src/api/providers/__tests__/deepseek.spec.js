...
describe("DeepSeekHandler", () => {
	...
	beforeEach(() => {
		...
		handler = new deepseek_1.DeepSeekHandler(mockOptions)
		vi.clearAllMocks()
	})
	describe("constructor", () => {
		it("should initialize with provided options", () => {
			expect(handler).toBeInstanceOf(deepseek_1.DeepSeekHandler)
			expect(handler.getModel().id).toBe(mockOptions.apiModelId)
		})
		it.skip("should throw error if API key is missing", () => {
			expect(() => {
				new deepseek_1.DeepSeekHandler({
					...mockOptions,
		...
		it("should use default model ID if not provided", () => {
			const handlerWithoutModel = new deepseek_1.DeepSeekHandler({
				...mockOptions,
				apiModelId: undefined,
			})
			expect(handlerWithoutModel.getModel().id).toBe(types_1.deepSeekDefaultModelId)
		})
		it("should use default base URL if not provided", () => {
			const handlerWithoutBaseUrl = new deepseek_1.DeepSeekHandler({
				...mockOptions,
				deepSeekBaseUrl: undefined,
			})
			expect(handlerWithoutBaseUrl).toBeInstanceOf(deepseek_1.DeepSeekHandler)
		...
		it("should use custom base URL if provided", () => {
			...
			const handlerWithCustomUrl = new deepseek_1.DeepSeekHandler({
				...mockOptions,
				deepSeekBaseUrl: customBaseUrl,
			})
			expect(handlerWithCustomUrl).toBeInstanceOf(deepseek_1.DeepSeekHandler)
		...
		it("should set includeMaxTokens to true", () => {
			...
			const _handler = new deepseek_1.DeepSeekHandler(mockOptions)
			expect(openai_1.default).toHaveBeenCalledWith(
	...
	describe("getModel", () => {
		...
		it("should return correct model info for deepseek-reasoner", () => {
			const handlerWithReasoner = new deepseek_1.DeepSeekHandler({
				...mockOptions,
		...
		it("should return provided model ID with default model info if model does not exist", () => {
			const handlerWithInvalidModel = new deepseek_1.DeepSeekHandler({
				...mockOptions,
		...
		it("should return default model if no model ID is provided", () => {
			...
			expect(model.id).toBe(types_1.deepSeekDefaultModelId)
			expect(model.info).toBeDefined()
			expect(model.info.supportsPromptCache).toBe(true)
		})
		...
		it("should use DEEP_SEEK_DEFAULT_TEMPERATURE as the default temperature", () => {
			const model = handler.getModel()
			expect(model.temperature).toBe(types_1.DEEP_SEEK_DEFAULT_TEMPERATURE)
		})
		it("should respect user-provided temperature over DEEP_SEEK_DEFAULT_TEMPERATURE", () => {
			const handlerWithTemp = new deepseek_1.DeepSeekHandler({
				...mockOptions,
	...
	describe("processUsageMetrics", () => {
		it("should correctly process usage metrics including cache information", () => {
			...
			class TestDeepSeekHandler extends deepseek_1.DeepSeekHandler {
				testProcessUsageMetrics(usage) {
	...
	describe("interleaved thinking mode", () => {
		...
		it("should handle reasoning_content in streaming responses for deepseek-reasoner", async () => {
			const reasonerHandler = new deepseek_1.DeepSeekHandler({
				...mockOptions,
				apiModelId: "deepseek-reasoner",
			})
		...
		it("should NOT pass thinking parameter for deepseek-chat model", async () => {
			const chatHandler = new deepseek_1.DeepSeekHandler({
				...mockOptions,
				apiModelId: "deepseek-chat",
			})

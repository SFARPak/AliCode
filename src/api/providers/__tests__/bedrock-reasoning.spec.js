...
describe("AwsBedrockHandler - Extended Thinking", () => {
	...
	beforeEach(() => {
		...
		mockSend = vi.fn()
		client_bedrock_runtime_1.ConverseStreamCommand.mockImplementation((payload) => {
		...
		logging_1.logger.info.mockImplementation(() => {})
		logging_1.logger.error.mockImplementation(() => {})
	})
	afterEach(() => {
		vi.clearAllMocks()
	})
	describe("Extended Thinking Support", () => {
		...
		it("should log when extended thinking is enabled", async () => {
			...
			// Verify logging
			expect(logging_1.logger.info).toHaveBeenCalledWith(
		...
		it("should support API key authentication", async () => {
			...
			expect(client_bedrock_runtime_1.BedrockRuntimeClient).toHaveBeenCalledWith(
				expect.objectContaining({
					region: "us-east-1",

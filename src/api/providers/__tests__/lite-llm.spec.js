...
const lite_llm_1 = require("../lite-llm")
...
vi.mock("../fetchers/modelCache", () => ({
	getModels: vi.fn().mockImplementation(() => {
		return Promise.resolve({
			[types_1.litellmDefaultModelId]: types_1.litellmDefaultModelInfo,
			"gpt-5": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			gpt5: { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			"GPT-5": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			"gpt-5-turbo": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			"gpt5-preview": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			"gpt-5o": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			"gpt-5.1": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			"gpt-5-mini": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			"gpt-4": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			"claude-3-opus": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			"llama-3": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			"gpt-4-turbo": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			// Gemini models for thought signature injection tests
			"gemini-3-pro": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			"gemini-3-flash": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			"gemini-2.5-pro": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			"google/gemini-3-pro": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
			"vertex_ai/gemini-3-pro": { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
		})
	}),
...
describe("LiteLLMHandler", () => {
	...
	beforeEach(() => {
		...
		mockOptions = {
			...
			litellmModelId: types_1.litellmDefaultModelId,
		}
		handler = new lite_llm_1.LiteLLMHandler(mockOptions)
	})
	describe("prompt caching", () => {
		it("should add cache control headers when litellmUsePromptCache is enabled", async () => {
			...
			handler = new lite_llm_1.LiteLLMHandler(optionsWithCache)
	...
	describe("GPT-5 model handling", () => {
		it("should use max_completion_tokens instead of max_tokens for GPT-5 models", async () => {
			const optionsWithGPT5 = {
				...mockOptions,
				litellmModelId: "gpt-5",
			}
			handler = new lite_llm_1.LiteLLMHandler(optionsWithGPT5)
			const systemPrompt = "You are a helpful assistant"
		...
		it("should still use max_tokens for non-GPT-5 models", async () => {
			...
			for (const modelId of nonGPT5Models) {
				...
				handler = new lite_llm_1.LiteLLMHandler(options)
				const systemPrompt = "You are a helpful assistant"
				const messages = [{ role: "user", content: "Test" }]
				// Mock the stream response
				const mockStream = {
		...
		it("should not set any max token fields when maxTokens is undefined (GPT-5 streaming)", async () => {
			...
			vi.spyOn(handler, "fetchModel").mockResolvedValue({
				id: "gpt-5",
				info: { ...types_1.litellmDefaultModelInfo, maxTokens: undefined },
			})
			// Mock the stream response
			const mockStream = {
				async *[Symbol.asyncIterator]() {
	...
	describe("Gemini thought signature injection", () => {
		describe("isGeminiModel detection", () => {
			it("should detect Gemini 3 models", () => {
				const handler = new lite_llm_1.LiteLLMHandler(mockOptions)
				const isGeminiModel = handler.isGeminiModel.bind(handler)
		...
		describe("injectThoughtSignatureForGemini", () => {
			...
			it("should inject provider_specific_fields.thought_signature for assistant messages with tool_calls", () => {
				const handler = new lite_llm_1.LiteLLMHandler(mockOptions)
				const injectThoughtSignature = handler.injectThoughtSignatureForGemini.bind(handler)
				const messages = [
					{ role: "user", content: "Hello" },
					{
						role: "assistant",
		...
		describe("createMessage integration with Gemini models", () => {
			...
			it("should inject thought signatures for Gemini 3 models with native tools", async () => {
				...
				handler = new lite_llm_1.LiteLLMHandler(optionsWithGemini)
				// Mock fetchModel to return a Gemini model
				vi.spyOn(handler, "fetchModel").mockResolvedValue({
					id: "gemini-3-pro",
					info: { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
				})
				const systemPrompt = "You are a helpful assistant"
			...
			it("should not inject thought signatures for non-Gemini models", async () => {
				const optionsWithGPT4 = {
					...mockOptions,
					litellmModelId: "gpt-4",
				}
				handler = new lite_llm_1.LiteLLMHandler(optionsWithGPT4)
				vi.spyOn(handler, "fetchModel").mockResolvedValue({
					id: "gpt-4",
					info: { ...types_1.litellmDefaultModelInfo, maxTokens: 8192 },
				})
	...
	describe("tool ID normalization", () => {
		it("should truncate tool IDs longer than 64 characters", async () => {
			const optionsWithBedrock = {
				...mockOptions,
				litellmModelId: "bedrock/anthropic.claude-3-sonnet",
			}
			handler = new lite_llm_1.LiteLLMHandler(optionsWithBedrock)
			vi.spyOn(handler, "fetchModel").mockResolvedValue({
			...
			const toolMessage = createCall.messages.find((msg) => msg.role === "tool")
			expect(assistantMessage).toBeDefined()

...
class NativeOllamaHandler extends base_provider_1.BaseProvider {
	...
	ensureClient() {
		if (!this.client) {
			try {
				...
				this.client = new ollama_1.Ollama(clientOptions)
			} catch (error) {
	...
	async fetchModel() {
		this.models = await (0, ollama_2.getOllamaModels)(this.options.ollamaBaseUrl, this.options.ollamaApiKey)
	...
	getModel() {
		...
		return {
			id: modelId,
			info: this.models[modelId] || types_1.openAiModelInfoSaneDefaults,
		}
	}

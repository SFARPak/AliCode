...
exports.parseLMStudioModel = exports.forceFullModelDetailsLoad = exports.hasLoadedFullDetails = void 0
...
const modelsWithLoadedDetails = new Set()
const hasLoadedFullDetails = (modelId) => modelsWithLoadedDetails.has(modelId)
exports.hasLoadedFullDetails = hasLoadedFullDetails
const forceFullModelDetailsLoad = async (baseUrl, modelId) => {
	try {
		// Test the connection to LM Studio first
		// Crrors will be caught further down.
		await axios_1.default.get(`${baseUrl}/v1/models`)
		const lmsUrl = baseUrl.replace(/^http:\/\//, "ws://").replace(/^https:\/\//, "wss://")
		const client = new sdk_1.LMStudioClient({ baseUrl: lmsUrl })
		await client.llm.model(modelId)
		// Flush and refresh cache to get updated model details
		await (0, modelCache_1.flushModels)({ provider: "lmstudio", baseUrl }, true)
...
exports.forceFullModelDetailsLoad = forceFullModelDetailsLoad
const parseLMStudioModel = (rawModel) => {
	...
	const modelInfo = Object.assign({}, types_1.lMStudioDefaultModelInfo, {
...
exports.parseLMStudioModel = parseLMStudioModel
	...
	try {
		...
		try {
			...
			for (const model of downloadedModels) {
				...
				models[model.path] = (0, exports.parseLMStudioModel)(model)
			}
		} catch (error) {
		...
		const loadedModels = await client.llm.listLoaded().then((models) => {
		...
		for (const lmstudioModel of loadedModels) {
			...
			models[lmstudioModel.modelKey] = (0, exports.parseLMStudioModel)(lmstudioModel)

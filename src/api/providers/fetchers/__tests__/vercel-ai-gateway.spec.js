...
describe("Vercel AI Gateway Fetchers", () => {
	...
	describe("getVercelAiGatewayModels", () => {
		...
		it("fetches and parses models correctly", async () => {
			...
			const models = await (0, vercel_ai_gateway_1.getVercelAiGatewayModels)()
	...
	describe("parseVercelAiGatewayModel", () => {
		...
		it("parses basic model info correctly", () => {
			const result = (0, vercel_ai_gateway_1.parseVercelAiGatewayModel)({
				id: "test/model",
				model: baseModel,
			})
		...
		it("detects vision-only models", () => {
			...
			expect(result.supportsImages).toBe(
				types_1.VERCEL_AI_GATEWAY_VISION_ONLY_MODELS.has("anthropic/claude-3.5-haiku"),
			)
		})
		it("detects vision and tools models", () => {
			...
			expect(result.supportsImages).toBe(
				types_1.VERCEL_AI_GATEWAY_VISION_AND_TOOLS_MODELS.has("anthropic/claude-sonnet-4"),
			)
		})
		...
		it("validates all vision model categories", () => {
			...
			visionOnlyModels.forEach((modelId) => {
				if (types_1.VERCEL_AI_GATEWAY_VISION_ONLY_MODELS.has(modelId)) {
					const result = (0, vercel_ai_gateway_1.parseVercelAiGatewayModel)({
						id: modelId,
			...
			visionAndToolsModels.forEach((modelId) => {
				if (types_1.VERCEL_AI_GATEWAY_VISION_AND_TOOLS_MODELS.has(modelId)) {
					const result = (0, vercel_ai_gateway_1.parseVercelAiGatewayModel)({
						id: modelId,
						model: { ...baseModel, id: modelId },
					})

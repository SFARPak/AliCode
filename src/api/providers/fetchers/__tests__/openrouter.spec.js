...
nock_1.back.setMode("lockdown")
describe("OpenRouter API", () => {
	describe("getOpenRouterModels", () => {
		it("fetches models and validates schema", async () => {
			const { nockDone } = await (0, nock_1.back)("openrouter-models.json")
			const models = await (0, openrouter_1.getOpenRouterModels)()
	...
	describe("getOpenRouterModelEndpoints", () => {
		it("fetches model endpoints and validates schema", async () => {
			...
			const endpoints = await (0, openrouter_1.getOpenRouterModelEndpoints)("google/gemini-2.5-pro-preview")
		...
		it("copies model-level capabilities from parent model to endpoint models", async () => {
			...
			const axios = await import("axios")
			const getSpy = vi.spyOn(axios.default, "get").mockResolvedValue(mockEndpointsResponse)
			const endpoints = await (0, openrouter_1.getOpenRouterModelEndpoints)("anthropic/claude-sonnet-4")
			// Simulate what modelEndpointCache does - copy capabilities from parent
	...
	describe("parseOpenRouterModel", () => {
		it("sets claude-sonnet-4.6 model to Anthropic max tokens", () => {
			...
			const result = (0, openrouter_1.parseOpenRouterModel)({
		...
		it("filters out image generation models", () => {
			...
			const textResult = (0, openrouter_1.parseOpenRouterModel)({
				id: "test/text-model",
				model: mockTextModel,
				inputModality: ["text"],
				outputModality: ["text"],
			...
			const imageResult = (0, openrouter_1.parseOpenRouterModel)({
				id: "test/image-model",
		...
		it("treats supportedParameters containing tools as allowed", () => {
			...
			const resultWithTools = (0, openrouter_1.parseOpenRouterModel)({
				id: "test/tools-model",
				model: mockModel,
				inputModality: ["text"],
				outputModality: ["text"],
		...
		it("treats supportedParameters without tools as allowed", () => {
			...
			const resultWithoutTools = (0, openrouter_1.parseOpenRouterModel)({

...
describe("getLiteLLMModels", () => {
	...
	it("handles base URLs with trailing slashes correctly", async () => {
		...
		await (0, litellm_1.getLiteLLMModels)("test-api-key", "http://localhost:4000/")
	...
	it("handles base URLs with a path correctly", async () => {
		...
		mockedAxios.get.mockResolvedValue(mockResponse)
		await (0, litellm_1.getLiteLLMModels)("test-api-key", "http://localhost:4000/litellm")
	...
	it("handles base URLs with query parameters correctly", async () => {
		...
		mockedAxios.get.mockResolvedValue(mockResponse)
		await (0, litellm_1.getLiteLLMModels)("test-api-key", "http://localhost:4000/litellm?key=value")
	...
	it("handles base URLs with fragments correctly", async () => {
		...
		mockedAxios.get.mockResolvedValue(mockResponse)
		await (0, litellm_1.getLiteLLMModels)("test-api-key", "http://localhost:4000/litellm#section")
	...
	it("successfully fetches and formats LiteLLM models", async () => {
		...
		mockedAxios.get.mockResolvedValue(mockResponse)
		const result = await (0, litellm_1.getLiteLLMModels)("test-api-key", "http://localhost:4000")
		expect(mockedAxios.get).toHaveBeenCalledWith("http://localhost:4000/v1/model/info", {
			headers: {
				Authorization: "Bearer test-api-key",
				"Content-Type": "application/json",
	...
	it("makes request without authorization header when no API key provided", async () => {
		...
		mockedAxios.get.mockResolvedValue(mockResponse)
		await (0, litellm_1.getLiteLLMModels)("", "http://localhost:4000")
		expect(mockedAxios.get).toHaveBeenCalledWith("http://localhost:4000/v1/model/info", {
			headers: {
				"Content-Type": "application/json",
				...constants_1.DEFAULT_HEADERS,
	...
	it("throws error for unexpected response format", async () => {
		...
		mockedAxios.get.mockResolvedValue(mockResponse)
		await expect((0, litellm_1.getLiteLLMModels)("test-api-key", "http://localhost:4000")).rejects.toThrow(
	...
	it("throws detailed error for HTTP error responses", async () => {
		...
		await expect((0, litellm_1.getLiteLLMModels)(DUMMY_INVALID_KEY, "http://localhost:4000")).rejects.toThrow(
	...
	it("throws network error for request failures", async () => {
		...
		mockedAxios.isAxiosError.mockReturnValue(true)
		mockedAxios.get.mockRejectedValue(axiosError)
		await expect((0, litellm_1.getLiteLLMModels)("test-api-key", "http://invalid-url")).rejects.toThrow(

...
exports.generateImageWithImagesApi = generateImageWithImagesApi
const i18n_1 = require("../../../i18n")
/**
 * Shared image generation implementation for OpenAI-compatible image providers.
 */
async function generateImageWithProvider(options) {
	...
	try {
		...
		if (!response.ok) {
			...
			let errorMessage = (0, i18n_1.t)("tools:generateImage.failedWithStatus", {
			...
			try {
				...
				if (errorJson.error?.message) {
					errorMessage = (0, i18n_1.t)("tools:generateImage.failedWithMessage", {
		...
		if (result.error) {
			return {
				success: false,
				error: (0, i18n_1.t)("tools:generateImage.failedWithMessage", {
		...
		if (!images || images.length === 0) {
			return {
				success: false,
				error: (0, i18n_1.t)("tools:generateImage.noImageGenerated"),
			}
		}
		...
		if (!imageData) {
			return {
				success: false,
				error: (0, i18n_1.t)("tools:generateImage.invalidImageData"),
			}
		}
		...
		if (!base64Match) {
			return {
				success: false,
				error: (0, i18n_1.t)("tools:generateImage.invalidImageFormat"),
			}
		}
		return {
			success: true,
		...
		return {
			success: false,
			error: error instanceof Error ? error.message : (0, i18n_1.t)("tools:generateImage.unknownError"),
		}
	}
}
/**
 ...
 */
async function generateImageWithImagesApi(options) {

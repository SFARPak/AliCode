...
describe("Settings Migration", () => {
	...
	// New file names
	const newCustomModesYaml = path.join(mockSettingsDir, globalFileNames_1.GlobalFileNames.customModes)
	const newMcpSettingsPath = path.join(mockSettingsDir, globalFileNames_1.GlobalFileNames.mcpSettings)
	beforeEach(() => {
	...
	it("should migrate custom modes file if old file exists and new file doesn't", async () => {
		...
		vitest.mocked(fs_1.fileExistsAtPath).mockImplementation(async (path) => {
		...
		// Run the migration
		await (0, migrateSettings_1.migrateSettings)(mockContext, mockOutputChannel)
	...
	it("should handle errors gracefully", async () => {
		...
		vitest.mocked(fs_1.fileExistsAtPath).mockRejectedValue(new Error("Test error"))
		await (0, migrateSettings_1.migrateSettings)(mockContext, mockOutputChannel)
	...
	it("should convert custom_modes.json to YAML format", async () => {
		...
		vitest.mocked(fs.readFile).mockImplementation(async (path) => {

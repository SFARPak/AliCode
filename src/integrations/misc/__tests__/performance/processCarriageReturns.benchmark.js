...
function generateTestData(size, complexity = "medium") {
...
function generateLongLineTestData(lineLengthKB, updateCount) {
...
function generateHighDensityCRData(size) {
	let result = ""
...
function runPerformanceTest(name, fn, input, iterations, args = []) {
	console.log(`\nTesting ${name}...`)
	// Pre-warm
...
function runBaselineTest(input, iterations) {
	...
	for (let i = 0; i < runs; i++) {
		results.push(runPerformanceTest(`Run ${i + 1}`, extract_text_1.processCarriageReturns, input, iterations))
	}
...
function runBenchmark() {
	...
	for (let i = 0; i < 50; i++) {
		;(0, extract_text_1.processCarriageReturns)(warmupData)
		;(0, extract_text_1.applyRunLengthEncoding)(warmupData)
		;(0, extract_text_1.truncateOutput)(warmupData, 500)
	}
	...
	for (const [size, complexity] of standardTestConfigs) {
		...
		runPerformanceTest("processCarriageReturns", extract_text_1.processCarriageReturns, testData, iterations)
		console.log("\n--- Function 2: applyRunLengthEncoding ---")
		runPerformanceTest("applyRunLengthEncoding", extract_text_1.applyRunLengthEncoding, testData, iterations)
		console.log("\n--- Function 3: truncateOutput ---")
		runPerformanceTest("truncateOutput", extract_text_1.truncateOutput, testData, iterations, [lineLimit])
		...
		runPerformanceTest(
			"Full Pipeline",
			(input) =>
				(0, extract_text_1.truncateOutput)(
					(0, extract_text_1.applyRunLengthEncoding)((0, extract_text_1.processCarriageReturns)(input)),
					lineLimit,
				),
			testData,
	...
	for (const [lineLength, updateCount] of longLineTestConfigs) {
		...
		runPerformanceTest(
			"processCarriageReturns (long line)",
			extract_text_1.processCarriageReturns,
			testData,
			iterations,
		)
	}
	...
	for (const size of highDensityCRConfigs) {
		...
		runPerformanceTest(
			"processCarriageReturns (high-density CR)",
			extract_text_1.processCarriageReturns,
			testData,
			iterations,
		)

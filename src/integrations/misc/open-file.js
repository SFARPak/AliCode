	...
	try {
			...
			for (const group of vscode.window.tabGroups.all) {
				const existingTab = group.tabs.find(
					(tab) =>
						...
						(0, path_1.arePathsEqual)(tab.input.uri.fsPath, uriToProcess.fsPath),
				)
				if (existingTab) {
		...
		if (error instanceof Error) {
			vscode.window.showErrorMessage(
				(0, i18n_1.t)("common:errors.could_not_open_file", { errorMessage: error.message }),
			)
		} else {
			vscode.window.showErrorMessage((0, i18n_1.t)("common:errors.could_not_open_file_generic"))
		}
	}
}

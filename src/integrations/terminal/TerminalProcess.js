...
const BaseTerminalProcess_1 = require("./BaseTerminalProcess")
const Terminal_1 = require("./Terminal")
class TerminalProcess extends BaseTerminalProcess_1.BaseTerminalProcess {
	terminalRef
	...
	async run(command) {
		...
		const streamAvailable = new Promise((resolve, reject) => {
			const timeoutId = setTimeout(() => {
				...
				this.emit(
					"no_shell_integration",
					`VSCE shell integration stream did not start within ${Terminal_1.Terminal.getShellIntegrationTimeout() / 1000} seconds. Terminal problem?`,
				)
				// Reject with descriptive error
				reject(
					new Error(
						`VSCE shell integration stream did not start within ${Terminal_1.Terminal.getShellIntegrationTimeout() / 1000} seconds.`,
					),
				)
			}, Terminal_1.Terminal.getShellIntegrationTimeout())
			// Clean up timeout if stream becomes available
			this.once("stream_available", (stream) => {
		...
		const shellExecutionComplete = new Promise((resolve) => {
			this.once("shell_execution_complete", (details) => resolve(details))
		})
		// Execute command
		...
		const isPowerShell =
			...
			(defaultWindowsShellProfile === null || defaultWindowsShellProfile?.toLowerCase().includes("powershell"))
		if (isPowerShell) {
			let commandToExecute = command
			// Only add the PowerShell counter workaround if enabled
			if (Terminal_1.Terminal.getPowershellCounter()) {
				commandToExecute += ` ; "(Ali/PS Workaround: ${this.terminal.cmdCounter++})" > $null`
			}
			// Only add the sleep command if the command delay is greater than 0
			if (Terminal_1.Terminal.getCommandDelay() > 0) {
				commandToExecute += ` ; start-sleep -milliseconds ${Terminal_1.Terminal.getCommandDelay()}`
			}
		...
		if (commandOutputStarted) {
			...
			const inspectPreOutput = (0, util_1.inspect)(preOutput, { colors: false, breakLength: Infinity })
	...
	stringIndexMatch(data, prefix, suffix, bell = "\x07") {
		let startIndex
		let endIndex
		let prefixLength
		if (prefix === undefined) {
	 ...
	 */
	removeVSCodeShellIntegration(text) {
	 ...
	 */
	matchAfterVsceStartMarkers(data) {
	 ...
	 */
	matchBeforeVsceEndMarkers(data) {
	 ...
	 */
	matchVsceMarkers(data, prefix633, prefix133, suffix633, suffix133) {

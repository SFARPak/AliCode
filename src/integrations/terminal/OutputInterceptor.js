...
exports.OutputInterceptor = void 0
...
class OutputInterceptor {
	...
	constructor(options) {
		...
		this.previewBytes = types_1.TERMINAL_PREVIEW_BYTES[options.previewSize]
	 ...
	 * @private
	 */
	addToTailBuffer(chunk, chunkBytes) {
	 ...
	 * @private
	 */
	sliceByBytesFromEnd(str, maxBytes) {
		let bytes = 0
		let i = str.length - 1
	 ...
	 * ```
	 */
	static async cleanup(storageDir) {
		try {
	 ...
	 * ```
	 */
	static async cleanupByIds(storageDir, executionIds) {
		try {
			const files = await fs.promises.readdir(storageDir)
			for (const file of files) {
...
exports.OutputInterceptor = OutputInterceptor

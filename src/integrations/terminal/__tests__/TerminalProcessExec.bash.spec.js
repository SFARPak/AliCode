...
function createRealCommandStream(command) {
	let realOutput
	let exitCode
	try {
		...
		realOutput = (0, child_process_1.execSync)(command + stderrRedirect, {
			encoding: "utf8",
 ...
 */
async function testTerminalCommand(command, expectedOutput) {
...
describe("TerminalProcess with Bash Command Output", () => {
	...
	it(TerminalProcessExec_common_1.TEST_PURPOSES.LARGE_OUTPUT, async () => {
		...
		if (process.platform === "win32") {
			...
			command = `for /l %i in (1,1,${lines}) do @echo ${TerminalProcessExec_common_1.TEST_TEXT.LARGE_PREFIX}%i`
			expectedOutput =
				Array.from(
					{ length: lines },
			...
			// Unix command
			command = `for i in $(seq 1 ${lines}); do echo "${TerminalProcessExec_common_1.TEST_TEXT.LARGE_PREFIX}$i"; done`
			expectedOutput =
				Array.from(
					{ length: lines },
					(_, i) => `${TerminalProcessExec_common_1.TEST_TEXT.LARGE_PREFIX}${i + 1}`,

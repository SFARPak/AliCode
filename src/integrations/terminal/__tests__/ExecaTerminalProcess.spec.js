...
vitest.mock("ps-tree", () => ({
	default: vitest.fn((_, cb) => cb(null, [])),
}))
...
describe("ExecaTerminalProcess", () => {
	...
	beforeEach(() => {
		...
		BaseTerminal_1.BaseTerminal.setExecaShellPath(undefined)
		mockTerminal = {
			provider: "execa",
			id: 1,
			busy: false,
		...
		terminalProcess = new ExecaTerminalProcess_1.ExecaTerminalProcess(mockTerminal)
	})
	afterEach(() => {
	...
	describe("UTF-8 encoding fix", () => {
		it("should set LANG and LC_ALL to en_US.UTF-8", async () => {
			...
			const execaMock = vitest.mocked(execa_1.execa)
		...
		it("should preserve existing environment variables", async () => {
			...
			await terminalProcess.run("echo test")
			const execaMock = vitest.mocked(execa_1.execa)
			const calledOptions = execaMock.mock.calls[0][0]
		...
		it("should use execaShellPath when set", async () => {
			BaseTerminal_1.BaseTerminal.setExecaShellPath("/bin/bash")
			await terminalProcess.run("echo test")
			const execaMock = vitest.mocked(execa_1.execa)
			expect(execaMock).toHaveBeenCalledWith(
				expect.objectContaining({
	...
	describe("basic functionality", () => {
		it("should create instance with terminal reference", () => {
			expect(terminalProcess).toBeInstanceOf(ExecaTerminalProcess_1.ExecaTerminalProcess)
	...
	describe("trimRetrievedOutput", () => {
		it("clears buffer when all output has been retrieved", () => {
			...
			terminalProcess.trimRetrievedOutput()

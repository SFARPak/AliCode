...
class TestTerminalProcess extends TerminalProcess_1.TerminalProcess {
	callTrimRetrievedOutput() {
...
describe("TerminalProcess", () => {
	...
	beforeEach(() => {
		...
		mockTerminalInfo = new Terminal_1.Terminal(1, mockTerminal, "./")
		// Create a process for testing
		terminalProcess = new TestTerminalProcess(mockTerminalInfo)
		TerminalRegistry_1.TerminalRegistry["terminals"].push(mockTerminalInfo)
	...
	describe("run", () => {
		...
		it("handles terminals without shell integration", async () => {
			...
			const noShellTerminalInfo = new Terminal_1.Terminal(2, noShellTerminal, "./")
			// Create new process with the no-shell terminal
			const noShellProcess = new TerminalProcess_1.TerminalProcess(noShellTerminalInfo)
			// Set up event listeners to verify events are emitted
			const eventPromises = Promise.all([
				new Promise((resolve) => noShellProcess.once("no_shell_integration", (_message) => resolve())),
				new Promise((resolve) => noShellProcess.once("completed", (_output) => resolve())),
				new Promise((resolve) => noShellProcess.once("continue", resolve)),
			])
	...
	describe("interpretExitCode", () => {
		...
		it("handles normal exit codes (0-128)", () => {
			...
			const result2 = TerminalProcess_1.TerminalProcess.interpretExitCode(1)
			expect(result2).toEqual({ exitCode: 1 })
			const result3 = TerminalProcess_1.TerminalProcess.interpretExitCode(128)
			expect(result3).toEqual({ exitCode: 128 })
		})
		it("interprets signal exit codes (>128)", () => {
			// SIGTERM (15) -> 128 + 15 = 143
			const result = TerminalProcess_1.TerminalProcess.interpretExitCode(143)
			...
			const result2 = TerminalProcess_1.TerminalProcess.interpretExitCode(139)
		...
		it("handles unknown signals", () => {
			const result = TerminalProcess_1.TerminalProcess.interpretExitCode(255)
			expect(result).toEqual({
				exitCode: 255,
				signal: 127,
	...
	describe("mergePromise", () => {
		it("merges promise methods with terminal process", async () => {
			const process = new TerminalProcess_1.TerminalProcess(mockTerminalInfo)
			const promise = Promise.resolve()
			const merged = (0, mergePromise_1.mergePromise)(process, promise)
			expect(merged).toHaveProperty("then")
			expect(merged).toHaveProperty("catch")
			expect(merged).toHaveProperty("finally")
			expect(merged instanceof TerminalProcess_1.TerminalProcess).toBe(true)

...
const pwshStream_1 = require("./streamUtils/pwshStream")
const streamUtils_1 = require("./streamUtils")
// Skip this test if PowerShell Core is not available
const hasPwsh = (0, streamUtils_1.isPowerShellCoreAvailable)()
 ...
 */
async function testPowerShellCommand(command, expectedOutput, useMock = false, skipVerification = false) {
	...
	const mockTerminalInfo = new Terminal_1.Terminal(1, mockTerminal, "/test/path")
	...
	try {
		...
		if (useMock) {
			...
			;({ stream, exitCode } = (0, streamUtils_1.createPowerShellMockStream)(expectedOutput))
		} else {
			// Set up the real command stream
			;({ stream, exitCode } = (0, pwshStream_1.createPowerShellStream)(command))
		}
...
describePlatform("TerminalProcess with PowerShell Command Output", () => {
	...
	it(TerminalProcessExec_common_1.TEST_PURPOSES.LARGE_OUTPUT, async () => {
		...
		const command = `foreach ($i in 1..${lines}) { Write-Output "${TerminalProcessExec_common_1.TEST_TEXT.LARGE_PREFIX}$i" }`
		...
		if (process.platform === "linux") {
			...
			for (let i = 1; i <= lines; i++) {
				expect(capturedOutput).toContain(`${TerminalProcessExec_common_1.TEST_TEXT.LARGE_PREFIX}${i}`)
			}
		} else {

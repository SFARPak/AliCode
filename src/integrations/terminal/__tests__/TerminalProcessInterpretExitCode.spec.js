...
describe("TerminalProcess.interpretExitCode", () => {
	it("should handle undefined exit code", () => {
		const result = TerminalProcess_1.TerminalProcess.interpretExitCode(undefined)
		expect(result).toEqual({ exitCode: undefined })
	})
	it("should handle normal exit codes (0-127)", () => {
		// Test success exit code (0)
		let result = TerminalProcess_1.TerminalProcess.interpretExitCode(0)
		expect(result).toEqual({ exitCode: 0 })
		// Test error exit code (1)
		result = TerminalProcess_1.TerminalProcess.interpretExitCode(1)
		expect(result).toEqual({ exitCode: 1 })
		// Test arbitrary exit code within normal range
		result = TerminalProcess_1.TerminalProcess.interpretExitCode(42)
		expect(result).toEqual({ exitCode: 42 })
		// Test boundary exit code
		result = TerminalProcess_1.TerminalProcess.interpretExitCode(127)
		expect(result).toEqual({ exitCode: 127 })
	})
	it("should handle signal exit codes (128+)", () => {
		// Test SIGINT (Ctrl+C) - 128 + 2 = 130
		const result = TerminalProcess_1.TerminalProcess.interpretExitCode(130)
		...
		const resultTerm = TerminalProcess_1.TerminalProcess.interpretExitCode(143)
		...
		const resultSegv = TerminalProcess_1.TerminalProcess.interpretExitCode(139)
	...
	it("should identify signals that can produce core dumps", () => {
		...
		for (const signal of coreDumpSignals) {
			...
			const result = TerminalProcess_1.TerminalProcess.interpretExitCode(exitCode)
			expect(result.coreDumpPossible).toBe(true)
		}
		// Test a non-core-dump signal
		const nonCoreDumpResult = TerminalProcess_1.TerminalProcess.interpretExitCode(128 + 1) // SIGHUP
	...
	it("should handle unknown signals", () => {
		...
		const result = TerminalProcess_1.TerminalProcess.interpretExitCode(128 + 99)
		expect(result).toEqual({
			exitCode: 128 + 99,
			signal: 99,
...
describe("TerminalProcess.interpretExitCode with real commands", () => {
	it("should correctly interpret exit code 0 from successful command", () => {
		try {
			...
			;(0, child_process_1.execSync)("echo test", { stdio: "ignore" })
			// If we get here, the command succeeded with exit code 0
			const result = TerminalProcess_1.TerminalProcess.interpretExitCode(0)
			expect(result).toEqual({ exitCode: 0 })
		} catch (error) {
	...
	it("should correctly interpret exit code 1 from failed command", () => {
		try {
			...
			;(0, child_process_1.execSync)("ls /nonexistent_directory", { stdio: "ignore" })
			...
			const result = TerminalProcess_1.TerminalProcess.interpretExitCode(error.status)
	...
	it("should correctly interpret exit code from command with custom exit code", () => {
		try {
			...
			;(0, child_process_1.execSync)("exit 42", { stdio: "ignore" })
	...
	it("should correctly interpret signal termination codes", () => {
		...
		const sigtermResult = TerminalProcess_1.TerminalProcess.interpretExitCode(sigtermExitCode)
		...
		const sigsegvResult = TerminalProcess_1.TerminalProcess.interpretExitCode(sigsegvExitCode)
		...
		const sigintResult = TerminalProcess_1.TerminalProcess.interpretExitCode(sigintExitCode)

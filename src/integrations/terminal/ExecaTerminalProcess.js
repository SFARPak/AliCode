...
exports.ExecaTerminalProcess = void 0
...
class ExecaTerminalProcess extends BaseTerminalProcess_1.BaseTerminalProcess {
	terminalRef
	aborted = false
	pid
	subprocess
	...
	async run(command) {
		...
		try {
			this.isHot = true
			this.subprocess = (0, execa_1.execa)({
				shell: BaseTerminal_1.BaseTerminal.getExecaShellPath() || true,
			...
			if (this.pid) {
				this.pidUpdatePromise = new Promise((resolve) => {
					setTimeout(() => {
						;(0, ps_tree_1.default)(this.pid, (err, children) => {
	...
	abort() {
		...
		const performKill = () => {
			...
			if (this.pid) {
				try {
					process_1.default.kill(this.pid, "SIGKILL")
				} catch (e) {
					console.warn(
		...
		if (this.pid) {
			...
			;(0, ps_tree_1.default)(this.pid, async (err, children) => {
				if (!err) {
...
exports.ExecaTerminalProcess = ExecaTerminalProcess

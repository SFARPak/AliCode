...
readme.md text
`
			vi.mocked(promises_1.default.readFile).mockResolvedValue(gitAttributesContent)
			// Expected LFS patterns
			const expectedLfsPatterns = ["*.psd", "*.zip", "*.mp4"]
			// Get exclude patterns
			const excludePatterns = await (0, excludes_1.getExcludePatterns)(testWorkspacePath)
			// Verify .gitattributes was checked at the correct path
			expect(fs_1.fileExistsAtPath).toHaveBeenCalledWith((0, path_1.join)(testWorkspacePath, ".gitattributes"))
			// Verify file was read
			expect(promises_1.default.readFile).toHaveBeenCalledWith(
				(0, path_1.join)(testWorkspacePath, ".gitattributes"),
				"utf8",
			)
...
`
		...
		it("should handle errors when reading .gitattributes", async () => {
			...
			vi.mocked(promises_1.default.readFile).mockRejectedValue(new Error("File read error"))
			// Get exclude patterns
			const excludePatterns = await (0, excludes_1.getExcludePatterns)(testWorkspacePath)
			// Verify .gitattributes was checked
			expect(fs_1.fileExistsAtPath).toHaveBeenCalledWith((0, path_1.join)(testWorkspacePath, ".gitattributes"))

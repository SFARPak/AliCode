...
vi.mock("fs/promises")
vi.mock("../ali-config", () => ({
	getGlobalAliDirectory: vi.fn(() => "/mock/global/.roo"),
	getProjectAliDirectoryForCwd: vi.fn(() => "/mock/project/.roo"),
}))
...
const mockFs = vi.mocked(promises_1.default)
...
\`\`\``
			...
			const result = await (0, commands_1.getCommand)("/test/cwd", "setup")
			expect(result).toEqual({
				name: "setup",
				content: "# Setup Command\n\nRun the following commands:\n```bash\nnpm install\nnpm run build\n```",
				source: "project",
				filePath: path.join("/test/cwd", ".ali", "commands", "setup.md"),
				description: "Sets up the development environment",
...
Global setup instructions.`
			...
			expect(result).toEqual({
				...
				source: "global",
				filePath: expect.stringContaining(path.join(".ali", "commands", "setup.md")),
				description: "Global setup command",
				argumentHint: undefined,
				mode: undefined,
			})
	...
	describe("argument-hint functionality", () => {
		it("should load command with argument-hint from frontmatter", async () => {
			const commandContent = `---
description: Create a new release of the AliCode extension
...
Create a new release.`
			mockFs.stat = vi.fn().mockResolvedValue({ isDirectory: () => true })
			mockFs.readFile = vi.fn().mockResolvedValue(commandContent)
			const result = await (0, commands_1.getCommand)("/test/cwd", "release")
			expect(result).toEqual({
				name: "release",
				content: "# Release Command\n\nCreate a new release.",
				source: "project",
				filePath: path.join("/test/cwd", ".ali", "commands", "release.md"),
				description: "Create a new release of the AliCode extension",
...
Deploy the application.`
			mockFs.stat = vi.fn().mockResolvedValue({ isDirectory: () => true })
			mockFs.readFile = vi.fn().mockResolvedValue(commandContent)
			const result = await (0, commands_1.getCommand)("/test/cwd", "deploy")
			expect(result).toEqual({
				name: "deploy",
				content: "# Deploy Command\n\nDeploy the application.",
				source: "project",
				filePath: path.join("/test/cwd", ".ali", "commands", "deploy.md"),
				description: "Deploy application to environment",
...
Test content.`
			mockFs.stat = vi.fn().mockResolvedValue({ isDirectory: () => true })
			mockFs.readFile = vi.fn().mockResolvedValue(commandContent)
			const result = await (0, commands_1.getCommand)("/test/cwd", "test")
...
Start debugging.`
			mockFs.stat = vi.fn().mockResolvedValue({ isDirectory: () => true })
			mockFs.readFile = vi.fn().mockResolvedValue(commandContent)
			const result = await (0, commands_1.getCommand)("/test/cwd", "debug-app")
			expect(result).toEqual({
				name: "debug-app",
				content: "# Debug Command\n\nStart debugging.",
				source: "project",
				filePath: path.join("/test/cwd", ".ali", "commands", "debug-app.md"),
				description: "Debug the application",
				argumentHint: undefined,
				mode: "debug",
			})
...
Build instructions without frontmatter.`
			...
			const result = await (0, commands_1.getCommands)("/test/cwd")

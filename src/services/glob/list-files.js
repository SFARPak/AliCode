 ...
 */
async function getFirstLevelDirectories(dirPath, ignoreInstance) {
 ...
 */
function ensureFirstLevelDirectoriesIncluded(results, firstLevelDirs, limit) {
 ...
 */
async function handleSpecialDirectories(dirPath) {
	const absolutePath = path.resolve(dirPath)
...
async function getRipgrepPath() {
	...
	const rgPath = await (0, ripgrep_1.getBinPath)(vscodeAppRoot)
	if (!rgPath) {
 ...
 */
async function listFilesWithRipgrep(rgPath, dirPath, recursive, limit) {
 ...
 */
function buildRipgrepArgs(dirPath, recursive) {
 ...
 */
function buildRecursiveArgs(dirPath) {
	const args = []
	...
	const isTargetInIgnoreList = constants_1.DIRS_TO_IGNORE.includes(targetDirName)
	...
	for (const dir of constants_1.DIRS_TO_IGNORE) {
 ...
 */
function buildNonRecursiveArgs() {
	const args = []
 ...
 */
async function createIgnoreInstance(dirPath) {
 ...
 */
async function findGitignoreFiles(startPath) {
 ...
 */
async function listFilteredDirectories(dirPath, recursive, ignoreInstance, limit) {
	const absolutePath = path.resolve(dirPath)
	const directories = []
	let dirCount = 0
	...
	async function scanDirectory(currentPath, context) {
			...
			for (const entry of entries) {
				...
				if (entry.isDirectory() && !entry.isSymbolicLink()) {
					...
					const shouldRecurse =
						...
						!(
							isHiddenDir &&
							constants_1.DIRS_TO_IGNORE.includes(".*") &&
							!context.isTargetDir &&
 ...
 */
function isIgnoredByGitignore(fullDirPath, basePath, ignoreInstance) {
 ...
 */
function shouldIncludeTargetDirectory(dirName) {
	// Only apply non-hidden-directory ignore rules to target directories
	const nonHiddenIgnorePatterns = constants_1.DIRS_TO_IGNORE.filter((pattern) => pattern !== ".*")
 ...
 */
function shouldIncludeInsideHiddenTarget(dirName, fullDirPath, context) {
	// Only apply the most critical ignore patterns when inside explicit hidden target
 ...
 */
function shouldIncludeRegularDirectory(dirName, fullDirPath, context) {
 ...
 */
function shouldIncludeDirectory(dirName, fullDirPath, context) {
 ...
 */
function isDirectoryExplicitlyIgnored(dirName) {
	for (const pattern of constants_1.DIRS_TO_IGNORE) {
		// Exact name matching
		if (pattern === dirName) {
			return true
		}
 ...
 */
function formatAndCombineResults(files, directories, limit) {
 ...
 */
async function execRipgrep(rgPath, args, limit) {

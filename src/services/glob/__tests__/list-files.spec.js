...
describe("listFiles", () => {
	it("should return empty array immediately when limit is 0", async () => {
		const result = await (0, list_files_1.listFiles)("/test/path", true, 0)
...
describe("list-files symlink support", () => {
	...
	it("should include --follow flag in ripgrep arguments", async () => {
		...
		await (0, list_files_1.listFiles)(testDir, false, 100)
	...
	it("should include --follow flag for recursive listings too", async () => {
		...
		await (0, list_files_1.listFiles)(testDir, true, 100)
		// Verify that spawn was called with --follow flag (the critical fix)
		const [rgPath, args] = mockSpawn.mock.calls[0]
		expect(rgPath).toBe("/mock/path/to/rg")
		expect(args).toContain("--files")
	...
	it("should ensure first-level directories are included when limit is reached", async () => {
		...
		mockReaddir.mockResolvedValueOnce([
			{ name: "a_dir", isDirectory: () => true, isSymbolicLink: () => false, isFile: () => false },
			{ name: "b_dir", isDirectory: () => true, isSymbolicLink: () => false, isFile: () => false },
			{ name: "c_dir", isDirectory: () => true, isSymbolicLink: () => false, isFile: () => false },
			{ name: "file1.txt", isDirectory: () => false, isSymbolicLink: () => false, isFile: () => true },
			{ name: "file2.txt", isDirectory: () => false, isSymbolicLink: () => false, isFile: () => true },
		])
		...
		const [results, limitReached] = await (0, list_files_1.listFiles)("/test/dir", true, 10)
...
describe("hidden directory exclusion", () => {
	...
	it("should exclude .git subdirectories from recursive directory listing", async () => {
		...
		mockSpawn.mockReturnValue(mockProcess)
		// Call listFiles with recursive=true
		const [result] = await (0, list_files_1.listFiles)("/test", true, 100)
	...
	it("should allow explicit targeting of hidden directories", async () => {
		...
		const [result] = await (0, list_files_1.listFiles)("/test/.roo-memory", true, 100)
	...
	it("should include top-level files when recursively listing a hidden directory that's also in DIRS_TO_IGNORE", async () => {
		// This test specifically addresses the bug where files at the root level of .ali/temp
		...
		const mockProcess = {
			stdout: {
				on: vi.fn((event, callback) => {
					if (event === "data") {
						// Simulate files that should be found in .ali/temp
						// Note: ripgrep returns relative paths
						setTimeout(() => {
							callback("teste1.md\n")
		...
		mockSpawn.mockReturnValue(mockProcess)
		// Mock directory listing for .ali/temp
		const mockReaddir = vi.fn()
		vi.mocked(fs.promises).readdir = mockReaddir
		mockReaddir.mockResolvedValueOnce([{ name: "22", isDirectory: () => true, isSymbolicLink: () => false }])
		// Call listFiles targeting .ali/temp (which is both hidden and in DIRS_TO_IGNORE)
		const [files] = await (0, list_files_1.listFiles)("/test/.ali/temp", true, 100)
...
describe("buildRecursiveArgs edge cases", () => {
	...
	it("should correctly detect hidden directories with trailing slashes", async () => {
		...
		await (0, list_files_1.listFiles)("/test/.hidden/", true, 100)
		const [rgPath, args] = mockSpawn.mock.calls[0]
	...
	it("should correctly detect nested hidden directories with mixed separators", async () => {
		...
		await (0, list_files_1.listFiles)("/test//normal/.hidden//subdir/", true, 100)
		const [rgPath, args] = mockSpawn.mock.calls[0]
	...
	it("should not detect hidden directories when path only has dots in filenames", async () => {
		...
		await (0, list_files_1.listFiles)("/test/file.with.dots/normal", true, 100)
		const [rgPath, args] = mockSpawn.mock.calls[0]

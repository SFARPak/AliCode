...
describe("listFiles limit handling for large projects", () => {
	...
	it("should prevent stack overflow when scanning large projects", async () => {
		...
		try {
			...
			const [res, didHitLimit] = await (0, list_files_1.listFiles)("/test/large-project", true, limit)
	...
	it("should terminate early when directory limit is reached", async () => {
		...
		const limit = 10
		const [results, limitReached] = await (0, list_files_1.listFiles)("/test/project", true, limit)
	...
	it("should handle zero limit gracefully", async () => {
		...
		const [results, limitReached] = await (0, list_files_1.listFiles)("/test/path", true, 0)
	...
	it("should correctly distribute limit between files and directories", async () => {
		...
		const [results, limitReached] = await (0, list_files_1.listFiles)("/test/project", true, 10)

...
vi.mock("../../ali-config", () => ({
	...
	getProjectAgentsDirectoryForCwd: (cwd) => p(cwd, ".agents"),
...
describe("SkillsManager", () => {
	...
	beforeEach(() => {
		...
		skillsManager = new SkillsManager_1.SkillsManager(mockProvider)
	})
	afterEach(async () => {
	...
	describe("discoverSkills", () => {
		it("should discover skills from global directory", async () => {
			...
			// Setup mocks
			mockDirectoryExists.mockImplementation(async (dir) => {
				return dir === globalSkillsDir
			})
			mockRealpath.mockImplementation(async (pathArg) => pathArg)
			...
			mockStat.mockImplementation(async (pathArg) => {
				if (pathArg === pdfSkillDir) {
					return { isDirectory: () => true }
				}
				throw new Error("Not found")
			})
			mockFileExists.mockImplementation(async (file) => {
...
# Mismatched name`
		...
		it("should skip skills with invalid name formats (spec compliance)", async () => {
			...
			mockDirectoryExists.mockImplementation(async (dir) => dir === globalSkillsDir)
			mockRealpath.mockImplementation(async (pathArg) => pathArg)
			mockReaddir.mockImplementation(async (dir) => (dir === globalSkillsDir ? invalidNames : []))
			mockStat.mockImplementation(async (pathArg) => {
...
# Invalid Skill`
		...
		it("should skip skills with name longer than 64 characters (spec compliance)", async () => {
			...
			mockDirectoryExists.mockImplementation(async (dir) => dir === globalSkillsDir)
			mockRealpath.mockImplementation(async (pathArg) => pathArg)
			mockReaddir.mockImplementation(async (dir) => (dir === globalSkillsDir ? [longName] : []))
			mockStat.mockImplementation(async (pathArg) => {
				if (pathArg === longDir) return { isDirectory: () => true }
				throw new Error("Not found")
			})
			mockFileExists.mockImplementation(async (file) => file === longMd)
...
# Long Name Skill`)
		...
		it("should skip skills with empty/whitespace-only description (spec compliance)", async () => {
			...
			mockDirectoryExists.mockImplementation(async (dir) => dir === globalSkillsDir)
			mockRealpath.mockImplementation(async (pathArg) => pathArg)
			mockReaddir.mockImplementation(async (dir) => (dir === globalSkillsDir ? ["valid-name"] : []))
			mockStat.mockImplementation(async (pathArg) => {
				if (pathArg === skillDir) return { isDirectory: () => true }
				throw new Error("Not found")
			})
			mockFileExists.mockImplementation(async (file) => file === skillMd)
			mockReadFile.mockResolvedValue(`---
name: valid-name
description: "   "
---
...
# Too Long Description`)
		...
		it("should handle symlinked skills directory", async () => {
			...
			mockRealpath.mockImplementation(async (pathArg) => {

...
`
		const captures = (0, markdownParser_1.parseMarkdown)(content)
...
`
	...
	it("should return empty array for empty content", () => {
		expect((0, markdownParser_1.parseMarkdown)("")).toEqual([])
		expect((0, markdownParser_1.parseMarkdown)("   ")).toEqual([])
		expect((0, markdownParser_1.parseMarkdown)(null)).toEqual([])
	})
	it("should handle content with no headers", () => {
		const content = `This is just some text.
No headers here.
Just plain text.`
		expect((0, markdownParser_1.parseMarkdown)(content)).toEqual([])
	})
...
Short`
		...
		const formatted1 = (0, markdownParser_1.formatMarkdownCaptures)(captures)
		...
		const formatted2 = (0, markdownParser_1.formatMarkdownCaptures)(captures, 2)
		...
		const formatted3 = (0, markdownParser_1.formatMarkdownCaptures)(captures, 1)
...
Line 5`
		...
		for (let minLines = 1; minLines <= 6; minLines++) {
			const formatted = (0, markdownParser_1.formatMarkdownCaptures)(captures, minLines)
...
[Apache 2.0 © 2025 AliCode, Inc.](./LICENSE)

---

**Enjoy AliCode!** Whether you keep it on a short leash or let it roam autonomously, we can't wait to see what you build.`
		const captures = (0, markdownParser_1.parseMarkdown)(content)
		expect(captures).toBeDefined()
		// Format with default minSectionLines = 4
		const formatted = (0, markdownParser_1.formatMarkdownCaptures)(captures)
		expect(formatted).toBeDefined()
...
function processCaptures(captures, lines, minComponentLines = 4) {

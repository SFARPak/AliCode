...
class McpHub {
		...
		try {
			...
			if (!result.success) {
				...
				vscode.window.showErrorMessage(
					(0, i18n_1.t)("mcp:errors.invalid_settings_validation", { errorMessages }),
				)
				return
			}
	...
	async watchProjectMcpFile() {
		...
		const projectMcpPattern = new vscode.RelativePattern(workspaceFolder, ".ali/mcp.json")
	...
	async getMcpSettingsFilePath() {
		...
		const mcpSettingsFilePath = path.join(
			...
			globalFileNames_1.GlobalFileNames.mcpSettings,
		)
  ...
  "mcpServers": {
	...
	async getProjectMcpPath() {
		...
		const projectMcpDir = path.join(workspacePath, ".ali")
	...
	async connectToServer(name, config, source = "global") {
		...
		const sanitizedName = (0, mcp_name_1.sanitizeMcpName)(name)
	...
	findServerNameBySanitizedName(sanitizedServerName) {
		...
		const fuzzyMatch = this.connections.find((conn) =>
			(0, mcp_name_1.toolNamesMatch)(conn.server.name, sanitizedServerName),
		)
		if (fuzzyMatch) {
	...
	async updateServerConfig(serverName, configUpdate, source = "global") {
		...
		try {
			await (0, safeWriteJson_1.safeWriteJson)(configPath, updatedConfig, { prettyPrint: true })
		} finally {
	...
	async updateServerToolList(serverName, source, toolName, listName, addTool) {
		...
		this.isProgrammaticUpdate = true
		try {
			await (0, safeWriteJson_1.safeWriteJson)(normalizedPath, config, { prettyPrint: true })
		} finally {
			// Reset flag after watcher debounce period (non-blocking)
			this.flagResetTimer = setTimeout(() => {
				this.isProgrammaticUpdate = false
		...
		if (!enabled) {
			...
			if (disconnectionErrors.length > 0) {
				...
				vscode.window.showWarningMessage(
					(0, i18n_1.t)("mcp:errors.disconnect_servers_partial", {

...
describe("McpHub", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(promises_1.default.readFile).mockResolvedValue(
			JSON.stringify({
				mcpServers: {
					"test-server": {
						type: "stdio",
		...
		mcpHub = new McpHub_1.McpHub(mockProvider)
	})
	afterEach(() => {
	...
	describe("Discriminated union type handling", () => {
		it("should create connected connections with proper type", async () => {
			...
			const mcpHub = new McpHub_1.McpHub(mockProvider)
	...
	describe("File watcher cleanup", () => {
		...
		it("should clean up all file watchers when server is deleted", async () => {
			...
			vi.mocked(chokidar.watch).mockImplementation(() => {
				return watcherIndex++ === 0 ? mockWatcher1 : mockWatcher2
			})
			// Mock StdioClientTransport
			const stdioModule = await import("@modelcontextprotocol/sdk/client/stdio.js")
			const StdioClientTransport = stdioModule.StdioClientTransport
	...
	describe("toggleToolAlwaysAllow", () => {
		it("should add tool to always allow list when enabling", async () => {
			...
			vi.mocked(promises_1.default.readFile).mockResolvedValueOnce(JSON.stringify(mockConfig))
			...
			const writeCalls = vi.mocked(promises_1.default.writeFile).mock.calls
		...
		it("should mark all tools as always allowed when wildcard is present", async () => {
			...
			vi.mocked(promises_1.default.readFile).mockResolvedValue(JSON.stringify(mockConfig))
	...
	describe("toggleToolEnabledForPrompt", () => {
		it("should add tool to disabledTools list when enabling", async () => {
			...
			mcpHub.connections = [mockConnection]
			promises_1.default.readFile.mockResolvedValueOnce(JSON.stringify(mockConfig))
			await mcpHub.toggleToolEnabledForPrompt("test-server", "global", "new-tool", false)
			// Verify the config was updated correctly
			const writeCalls = promises_1.default.writeFile.mock.calls
			expect(writeCalls.length).toBeGreaterThan(0)
			// Find the write call
			const callToUse = writeCalls[writeCalls.length - 1]
			expect(callToUse).toBeTruthy()
	...
	describe("callTool", () => {
		...
		describe("timeout configuration", () => {
			it("should validate timeout values", () => {
				...
				expect(() => McpHub_1.ServerConfigSchema.parse(validConfig)).not.toThrow()
				...
				invalidConfigs.forEach((config) => {
					expect(() => McpHub_1.ServerConfigSchema.parse(config)).toThrow()
				})
			})
	...
	describe("MCP global enable/disable", () => {
		...
		it("should not connect to servers when MCP is globally disabled", async () => {
			...
			const mcpHub = new McpHub_1.McpHub(disabledMockProvider)
			// Wait for initialization
			await new Promise((resolve) => setTimeout(resolve, 100))
		...
		it("should connect to servers when MCP is globally enabled", async () => {
			...
			const mcpHub = new McpHub_1.McpHub(enabledMockProvider)
			// Wait for initialization
			await new Promise((resolve) => setTimeout(resolve, 100))
	...
	describe("Windows command wrapping", () => {
		...
		it("should wrap commands with cmd.exe on Windows", async () => {
			...
			StdioClientTransport.mockImplementation((config) => {

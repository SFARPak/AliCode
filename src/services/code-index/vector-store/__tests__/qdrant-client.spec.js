...
describe("QdrantVectorStore", () => {
	...
	beforeEach(() => {
		...
		js_client_rest_1.QdrantClient.mockImplementation(() => mockQdrantClientInstance)
		crypto_1.createHash.mockReturnValue(mockCreateHashInstance)
		...
		path_1.getWorkspacePath.mockReturnValue(mockWorkspacePath)
		vectorStore = new qdrant_client_1.QdrantVectorStore(
			mockWorkspacePath,
			mockQdrantUrl,
			mockVectorSize,
			mockApiKey,
	...
	it("should correctly initialize QdrantClient and collectionName in constructor", () => {
		expect(js_client_rest_1.QdrantClient).toHaveBeenCalledTimes(1)
		expect(js_client_rest_1.QdrantClient).toHaveBeenCalledWith({
			host: "mock-qdrant",
			https: false,
			port: 6333,
			apiKey: mockApiKey,
		...
		expect(crypto_1.createHash).toHaveBeenCalledWith("sha256")
		...
		expect(vectorStore.collectionName).toBe(expectedCollectionName)
		expect(vectorStore.vectorSize).toBe(mockVectorSize)
	})
	it("should handle constructor with default URL when none provided", () => {
		const vectorStoreWithDefaults = new qdrant_client_1.QdrantVectorStore(
			mockWorkspacePath,
			undefined,
			mockVectorSize,
		)
		expect(js_client_rest_1.QdrantClient).toHaveBeenLastCalledWith({
			host: "localhost",
			https: false,
			port: 6333,
			apiKey: undefined,
	...
	it("should handle constructor without API key", () => {
		const vectorStoreWithoutKey = new qdrant_client_1.QdrantVectorStore(
			mockWorkspacePath,
			mockQdrantUrl,
			mockVectorSize,
		)
	...
	describe("URL Parsing and Explicit Port Handling", () => {
		describe("HTTPS URL handling", () => {
			it("should use explicit port 443 for HTTPS URLs without port (fixes the main bug)", () => {
				const vectorStore = new qdrant_client_1.QdrantVectorStore(
					mockWorkspacePath,
				...
				expect(vectorStore.qdrantUrl).toBe("https://qdrant.ashbyfam.com")
			})
			it("should use explicit port for HTTPS URLs with explicit port", () => {
				...
				expect(vectorStore.qdrantUrl).toBe("https://example.com:9000")
			})
			it("should use port 443 for HTTPS URLs with paths and query parameters", () => {
				...
				expect(vectorStore.qdrantUrl).toBe("https://example.com/api/v1?key=value")
			})
		})
		describe("HTTP URL handling", () => {
			it("should use explicit port 80 for HTTP URLs without port", () => {
				...
				expect(vectorStore.qdrantUrl).toBe("http://example.com")
			})
			it("should use explicit port for HTTP URLs with explicit port", () => {
				...
				expect(vectorStore.qdrantUrl).toBe("http://localhost:8080")
			})
			it("should use port 80 for HTTP URLs while preserving paths and query parameters", () => {
				...
				expect(vectorStore.qdrantUrl).toBe("http://example.com/api/v1?key=value")
			})
		})
		describe("Hostname handling", () => {
			it("should convert hostname to http with port 80", () => {
				...
				expect(vectorStore.qdrantUrl).toBe("http://qdrant.example.com")
			})
			it("should handle hostname:port format with explicit port", () => {
				...
				expect(vectorStore.qdrantUrl).toBe("http://localhost:6333")
			})
			it("should handle explicit HTTP URLs correctly", () => {
				...
				expect(vectorStore.qdrantUrl).toBe("http://localhost:9000")
			})
		})
		describe("IP address handling", () => {
			it("should convert IP address to http with port 80", () => {
				...
				expect(vectorStore.qdrantUrl).toBe("http://192.168.1.100")
			})
			it("should handle IP:port format with explicit port", () => {
				...
				expect(vectorStore.qdrantUrl).toBe("http://192.168.1.100:6333")
			})
		})
		describe("Edge cases", () => {
			it("should handle undefined URL with host-based config", () => {
				const vectorStore = new qdrant_client_1.QdrantVectorStore(mockWorkspacePath, undefined, mockVectorSize)
				expect(js_client_rest_1.QdrantClient).toHaveBeenLastCalledWith({
					host: "localhost",
					https: false,
					port: 6333,
			...
			it("should handle empty string URL with host-based config", () => {
				const vectorStore = new qdrant_client_1.QdrantVectorStore(mockWorkspacePath, "", mockVectorSize)
				expect(js_client_rest_1.QdrantClient).toHaveBeenLastCalledWith({
					host: "localhost",
					https: false,
					port: 6333,
		...
		describe("Invalid URL fallback", () => {
			it("should treat invalid URLs as hostnames with port 80", () => {
				...
				expect(vectorStore.qdrantUrl).toBe("http://invalid-url-format")
			})
		})
	})
	describe("URL Prefix Handling", () => {
		it("should pass the URL pathname as prefix to QdrantClient if not root", () => {
			const vectorStoreWithPrefix = new qdrant_client_1.QdrantVectorStore(
				mockWorkspacePath,
			...
			expect(vectorStoreWithPrefix.qdrantUrl).toBe("http://localhost:6333/some/path")
		})
		it("should not pass prefix if the URL pathname is root ('/')", () => {
			const vectorStoreWithoutPrefix = new qdrant_client_1.QdrantVectorStore(
				mockWorkspacePath,
				"http://localhost:6333/",
				mockVectorSize,
			)
			...
			expect(vectorStoreWithoutPrefix.qdrantUrl).toBe("http://localhost:6333/")
		})
		it("should handle HTTPS URL with path as prefix", () => {
			const vectorStoreWithHttpsPrefix = new qdrant_client_1.QdrantVectorStore(
				mockWorkspacePath,
			...
			expect(vectorStoreWithHttpsPrefix.qdrantUrl).toBe("https://qdrant.ashbyfam.com/api")
		})
		it("should normalize URL pathname by removing trailing slash for prefix", () => {
			const vectorStoreWithTrailingSlash = new qdrant_client_1.QdrantVectorStore(
				mockWorkspacePath,
			...
			expect(vectorStoreWithTrailingSlash.qdrantUrl).toBe("http://localhost:6333/api/")
		})
		it("should normalize URL pathname by removing multiple trailing slashes for prefix", () => {
			const vectorStoreWithMultipleTrailingSlashes = new qdrant_client_1.QdrantVectorStore(
				mockWorkspacePath,
				"http://localhost:6333/api///",
				mockVectorSize,
			)
			...
			expect(vectorStoreWithMultipleTrailingSlashes.qdrantUrl).toBe("http://localhost:6333/api///")
		})
		it("should handle multiple path segments correctly for prefix", () => {
			const vectorStoreWithMultiSegment = new qdrant_client_1.QdrantVectorStore(
				mockWorkspacePath,
			...
			expect(vectorStoreWithMultiSegment.qdrantUrl).toBe("http://localhost:6333/api/v1/qdrant")
		})
		it("should handle complex URL with multiple segments, multiple trailing slashes, query params, and fragment", () => {
			...
			const vectorStoreComplex = new qdrant_client_1.QdrantVectorStore(
				mockWorkspacePath,
				complexUrl,
				mockVectorSize,
			)
			...
			expect(vectorStoreComplex.qdrantUrl).toBe(complexUrl)
		})
		it("should ignore query parameters and fragments when determining prefix", () => {
			const vectorStoreWithQueryParams = new qdrant_client_1.QdrantVectorStore(
				mockWorkspacePath,
			...
			expect(vectorStoreWithQueryParams.qdrantUrl).toBe("http://localhost:6333/api/path?key=value#fragment")
		})
	})
	describe("initialize", () => {
		it("should create a new collection if none exists and return true", async () => {
			...
			mockQdrantClientInstance.createCollection.mockResolvedValue(true) // Cast to any to satisfy QdrantClient types if strict
			mockQdrantClientInstance.createPayloadIndex.mockResolvedValue({}) // Mock successful index creation
		...
		it("should not create a new collection if one exists with matching vectorSize and return false", async () => {
			...
			}) // Cast to any to satisfy QdrantClient types
			mockQdrantClientInstance.createPayloadIndex.mockResolvedValue({})
			const result = await vectorStore.initialize()
		...
		it("should recreate collection if it exists but vectorSize mismatches and return true", async () => {
			...
			mockQdrantClientInstance.deleteCollection.mockResolvedValue(true)
			mockQdrantClientInstance.createCollection.mockResolvedValue(true)
			mockQdrantClientInstance.createPayloadIndex.mockResolvedValue({})
			...
			expect(mockQdrantClientInstance.createPayloadIndex).toHaveBeenCalledTimes(6)
			console.warn.mockRestore() // Restore console.warn
		})
		...
		it("should handle dimension mismatch scenario from 2048 to 768 dimensions", async () => {
			...
			const newVectorStore = new qdrant_client_1.QdrantVectorStore(
				mockWorkspacePath,
				mockQdrantUrl,
				newVectorSize,
				mockApiKey,
	...
	describe("upsertPoints", () => {
		it("should correctly call qdrantClient.upsert with processed points", async () => {
			...
			mockQdrantClientInstance.upsert.mockResolvedValue({})
	...
	describe("search", () => {
		it("should correctly call qdrantClient.query and transform results", async () => {
			...
			expect(callArgs).toMatchObject({
				query: queryVector,
				score_threshold: constants_1.DEFAULT_SEARCH_MIN_SCORE,
				limit: constants_1.DEFAULT_MAX_SEARCH_RESULTS,
				params: {
					hnsw_ef: 128,
					exact: false,
				},
		...
		it("should use constants DEFAULT_MAX_SEARCH_RESULTS and DEFAULT_SEARCH_MIN_SCORE correctly", async () => {
			...
			const callArgs = mockQdrantClientInstance.query.mock.calls[0][1]
			expect(callArgs.limit).toBe(constants_1.DEFAULT_MAX_SEARCH_RESULTS)
			expect(callArgs.score_threshold).toBe(constants_1.DEFAULT_SEARCH_MIN_SCORE)
		})

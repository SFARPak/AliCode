...
class QdrantVectorStore {
	 ...
	 */
	constructor(workspacePath, url, vectorSize, apiKey) {
		...
		const hash = (0, crypto_1.createHash)("sha256").update(workspacePath).digest("hex")
	...
	async initialize() {
		...
		try {
			...
			throw new Error(
				(0, i18n_1.t)("embeddings:vectorStore.qdrantConnectionFailed", {
	...
	async _recreateCollectionWithNewDimension(existingVectorSize) {
		...
		try {
			...
			const dimensionMismatchError = new Error(
				(0, i18n_1.t)("embeddings:vectorStore.vectorDimensionMismatch", {
	...
	async search(queryVector, directoryPrefix, minScore, maxResults) {
		try {
			...
			const searchRequest = {
				query: queryVector,
				filter: mergedFilter,
				score_threshold: minScore ?? constants_1.DEFAULT_SEARCH_MIN_SCORE,
				limit: maxResults ?? constants_1.DEFAULT_MAX_SEARCH_RESULTS,
				params: {
					hnsw_ef: 128,
					exact: false,
				},
	...
	async hasIndexedData() {
		try {
			...
			const metadataId = (0, uuid_1.v5)("__indexing_metadata__", constants_1.QDRANT_CODE_BLOCK_NAMESPACE)

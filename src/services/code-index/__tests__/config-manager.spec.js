...
const mockedGetDefaultModelId = vi.mocked(embeddingModels_1.getDefaultModelId)
const mockedGetModelDimension = vi.mocked(embeddingModels_1.getModelDimension)
const mockedGetModelScoreThreshold = vi.mocked(embeddingModels_1.getModelScoreThreshold)
describe("CodeIndexConfigManager", () => {
	...
	beforeEach(() => {
		...
		configManager = new config_manager_1.CodeIndexConfigManager(mockContextProxy)
	})
	// Helper function to setup secret mocking
	const setupSecretMocks = (secrets) => {
		// Mock sync secret access
		mockContextProxy.getSecret.mockImplementation((key) => {
	...
	describe("loadConfiguration", () => {
		...
		it("should load OpenAI Compatible configuration from globalState and secrets", async () => {
			...
			mockContextProxy.getGlobalState.mockImplementation((key) => {
		...
		describe("simplified restart detection", () => {
			...
			describe("currentSearchMinScore priority system", () => {
				...
				it("should handle priority correctly: user > model > default", async () => {
					...
					const newManager = new config_manager_1.CodeIndexConfigManager(mockContextProxy)
					...
					const anotherManager = new config_manager_1.CodeIndexConfigManager(mockContextProxy)
			...
			describe("currentSearchMaxResults", () => {
				it("should return user setting when provided, otherwise default", async () => {
					...
					const minManager = new config_manager_1.CodeIndexConfigManager(mockContextProxy)
					...
					const maxManager = new config_manager_1.CodeIndexConfigManager(mockContextProxy)
	...
	describe("initialization and restart prevention", () => {
		...
		it("should properly initialize with current config to prevent false restarts", async () => {
			...
			const newConfigManager = new config_manager_1.CodeIndexConfigManager(mockContextProxy)
		...
		describe("currentModelDimension", () => {
			...
			describe("OpenRouter provider dimension handling", () => {
				it("should correctly handle OpenRouter mistral model dimensions across restarts", async () => {
					...
					const restartConfigManager = new config_manager_1.CodeIndexConfigManager(mockContextProxy)

...
function mockUri(fsPath, scheme = "file") {
	return {
		fsPath,
		scheme,
		authority: "",
		path: fsPath,
		toString: (skipEncoding) => `${scheme}://${fsPath}`,
	}
}
// Mock vscode module
vi.mock("vscode", () => {
			...
			workspaceFolders: [
				{
					uri: {
						...
						toString: (_skipEncoding) => `file://${testWorkspacePath}`,
					},
					name: "test",
					index: 0,
				},
...
// Mock fs/promises for AliIgnoreController
vi.mock("fs/promises", () => ({
	default: {
		readFile: vi.fn().mockRejectedValue(new Error("File not found")),
	},
}))
// Mock file utils for AliIgnoreController
vi.mock("../../../utils/fs", () => ({
...
const MockedCodeIndexServiceFactory = service_factory_1.CodeIndexServiceFactory
describe("CodeIndexManager - handleSettingsChange regression", () => {
	...
	beforeEach(() => {
		...
		manager_1.CodeIndexManager.disposeAll()
		const workspaceStateStore = {}
		const globalStateStore = {}
		mockContext = {
			subscriptions: [],
			workspaceState: {
				get: vi.fn((key, defaultValue) => workspaceStateStore[key] ?? defaultValue),
				update: vi.fn(async (key, value) => {
					workspaceStateStore[key] = value
				}),
			},
			globalState: {
				get: vi.fn((key, defaultValue) => globalStateStore[key] ?? defaultValue),
				update: vi.fn(async (key, value) => {
		...
		manager = manager_1.CodeIndexManager.getInstance(mockContext)
	})
	afterEach(() => {
		manager_1.CodeIndexManager.disposeAll()
	})
	describe("handleSettingsChange", () => {
		it("should not throw when called on uninitialized manager (regression test)", async () => {
			...
			manager._configManager = mockConfigManager
			// Mock cache manager
			const mockCacheManager = {
				initialize: vi.fn(),
				clearCacheFile: vi.fn(),
			}
			manager._cacheManager = mockCacheManager
		...
		it("should work normally when manager is initialized", async () => {
			...
			manager._cacheManager = mockCacheManager
			manager._orchestrator = { stopWatcher: vi.fn(), stopIndexing: vi.fn() }
			manager._searchService = {}
			...
			const recreateServicesSpy = vi.spyOn(manager, "_recreateServices")
	...
	describe("embedder validation integration", () => {
		let mockServiceFactoryInstance
		...
		beforeEach(() => {
			...
			mockStateManager = manager._stateManager
		...
		it("should set error state when embedder validation fails", async () => {
			...
			// Act & Assert
			await expect(manager._recreateServices()).rejects.toThrow("embeddings:validation.authenticationFailed")
		...
		it("should set generic error state when embedder validation throws", async () => {
			...
			// Act & Assert
			await expect(manager._recreateServices()).rejects.toThrow("embeddings:validation.configurationError")
			// Assert other expectations
			expect(mockServiceFactoryInstance.createServices).toHaveBeenCalled()
			const createdEmbedder = mockServiceFactoryInstance.createServices.mock.results[0].value.embedder
			expect(mockServiceFactoryInstance.validateEmbedder).toHaveBeenCalledWith(createdEmbedder)
		...
		it("should handle embedder creation failure", async () => {
			...
			await expect(manager._recreateServices()).rejects.toThrow("Invalid configuration")
	...
	describe("recoverFromError", () => {
		...
		beforeEach(() => {
			...
			manager._orchestrator = { stopWatcher: vi.fn(), stopIndexing: vi.fn(), state: "Error" }
			manager._searchService = {}
		...
		it("should reset internal service instances", async () => {
			...
			expect(manager._serviceFactory).toBeDefined()
			expect(manager._orchestrator).toBeDefined()
			expect(manager._searchService).toBeDefined()
			// Act
			await manager.recoverFromError()
			// Assert - all service instances should be undefined
			expect(manager._configManager).toBeUndefined()
			expect(manager._serviceFactory).toBeUndefined()
			expect(manager._orchestrator).toBeUndefined()
			expect(manager._searchService).toBeUndefined()
		})
	...
	describe("workspace-enabled gating", () => {
		...
		it("should store enablement per folder URI, not per window", async () => {
			...
			const sharedContext = {
				...mockContext,
				workspaceState: {
					get: vi.fn((key, defaultValue) => sharedStore[key] ?? defaultValue),
					update: vi.fn(async (key, value) => {
						sharedStore[key] = value
					}),
				},
				globalState: {
					get: vi.fn((_key, _defaultValue) => false),
					update: vi.fn(),
				},
			}
			...
			const managerA = manager_1.CodeIndexManager.getInstance(sharedContext, folderAPath)
			const managerB = manager_1.CodeIndexManager.getInstance(sharedContext, folderBPath)
	...
	describe("stopIndexing", () => {
		it("should delegate to orchestrator.stopIndexing()", () => {
			...
			manager._orchestrator = mockOrchestrator

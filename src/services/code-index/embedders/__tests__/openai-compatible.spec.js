...
describe("OpenAICompatibleEmbedder", () => {
	...
	beforeEach(() => {
		...
		const tempEmbedder = new openai_compatible_1.OpenAICompatibleEmbedder(testBaseUrl, testApiKey, testModelId)
		tempEmbedder.constructor.globalRateLimitState = {
			isRateLimited: false,
			rateLimitResetTime: 0,
			consecutiveRateLimitErrors: 0,
			lastRateLimitError: 0,
			mutex: tempEmbedder.constructor.globalRateLimitState.mutex,
		}
	})
	afterEach(() => {
	...
	describe("constructor", () => {
		it("should create embedder with valid configuration", () => {
			embedder = new openai_compatible_1.OpenAICompatibleEmbedder(testBaseUrl, testApiKey, testModelId)
		...
		it("should use default model when modelId is not provided", () => {
			embedder = new openai_compatible_1.OpenAICompatibleEmbedder(testBaseUrl, testApiKey)
			expect(MockedOpenAI).toHaveBeenCalledWith({
				baseURL: testBaseUrl,
				apiKey: testApiKey,
			})
		...
		it("should throw error when baseUrl is missing", () => {
			expect(() => new openai_compatible_1.OpenAICompatibleEmbedder("", testApiKey, testModelId)).toThrow(
				"embeddings:validation.baseUrlRequired",
			)
		})
		it("should throw error when apiKey is missing", () => {
			expect(() => new openai_compatible_1.OpenAICompatibleEmbedder(testBaseUrl, "", testModelId)).toThrow(
				"embeddings:validation.apiKeyRequired",
			)
		})
		it("should throw error when both baseUrl and apiKey are missing", () => {
			expect(() => new openai_compatible_1.OpenAICompatibleEmbedder("", "", testModelId)).toThrow(
				"embeddings:validation.baseUrlRequired",
			)
		})
		it("should handle API key with invalid characters (ByteString conversion error)", () => {
			...
			expect(
				() => new openai_compatible_1.OpenAICompatibleEmbedder(testBaseUrl, invalidApiKey, testModelId),
	...
	describe("createEmbeddings", () => {
		...
		describe("batching logic", () => {
			...
			it("should skip texts that exceed MAX_ITEM_TOKENS", async () => {
				...
				const oversizedText = "a".repeat(constants_1.MAX_ITEM_TOKENS * 5) // Exceeds MAX_ITEM_TOKENS
		...
		describe("OpenAI package base64 behavior verification", () => {
			it("should return unparsed base64 when encoding_format is base64", async () => {
				...
				const realOpenAI = new (await vi.importActual("openai")).OpenAI({
					baseURL: testBaseUrl,
					apiKey: testApiKey,
				})
				...
				realOpenAI.post = vi.fn().mockImplementation((path, opts) => {
				...
				const returnedBuffer = Buffer.from(response.data[0].embedding, "base64")
		...
		describe("Azure OpenAI compatibility", () => {
			...
			const createBase64Embedding = (values) => {
			...
			const expectEmbeddingValues = (actual, expected) => {
			...
			describe("URL detection", () => {
				it.each([
					...
					const embedder = new openai_compatible_1.OpenAICompatibleEmbedder(url, testApiKey, testModelId)
					const isFullUrl = embedder.isFullEndpointUrl(url)
			...
			describe("direct HTTP requests", () => {
				it("should use direct fetch for Azure URLs and SDK for base URLs", async () => {
					...
					const azureEmbedder = new openai_compatible_1.OpenAICompatibleEmbedder(
						azureUrl,
						testApiKey,
						testModelId,
					)
					...
					vitest.clearAllMocks()
					const baseEmbedder = new openai_compatible_1.OpenAICompatibleEmbedder(
						baseUrl,
						testApiKey,
						testModelId,
					)
				...
				it.each([
					...
					const embedder = new openai_compatible_1.OpenAICompatibleEmbedder(azureUrl, testApiKey, testModelId)
				...
				it("should handle rate limiting with retries", async () => {
					...
					const base64String = createBase64Embedding([0.1, 0.2, 0.3])
					global.fetch
						.mockResolvedValueOnce(createMockResponse({}, 429, false))
						.mockResolvedValueOnce(createMockResponse({}, 429, false))
						.mockResolvedValueOnce(
							createMockResponse({
								data: [{ embedding: base64String }],
	...
	describe("URL detection", () => {
		it("should detect Azure deployment URLs as full endpoints", async () => {
			const embedder = new openai_compatible_1.OpenAICompatibleEmbedder(
		...
		it("should treat base URLs without endpoint patterns as SDK URLs", async () => {
			const embedder = new openai_compatible_1.OpenAICompatibleEmbedder("https://api.openai.com/v1", "test-key")
	...
	describe("validateConfiguration", () => {
		...
		it("should validate successfully with full endpoint URL", async () => {
			...
			embedder = new openai_compatible_1.OpenAICompatibleEmbedder(fullUrl, testApiKey, testModelId)

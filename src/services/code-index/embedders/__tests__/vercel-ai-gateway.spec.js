...
const MockedOpenAICompatibleEmbedder = vi.mocked(openai_compatible_1.OpenAICompatibleEmbedder)
describe("VercelAiGatewayEmbedder", () => {
	let embedder
	let mockOpenAICompatibleEmbedder
	beforeEach(() => {
		vi.clearAllMocks()
	...
	describe("constructor", () => {
		it("should create VercelAiGatewayEmbedder with default model", () => {
			...
			// Act
			embedder = new vercel_ai_gateway_1.VercelAiGatewayEmbedder(apiKey)
			// Assert
		...
		it("should create VercelAiGatewayEmbedder with custom model", () => {
			...
			// Act
			embedder = new vercel_ai_gateway_1.VercelAiGatewayEmbedder(apiKey, modelId)
			// Assert
			expect(MockedOpenAICompatibleEmbedder).toHaveBeenCalledWith(
				"https://ai-gateway.vercel.sh/v1",
				apiKey,
		...
		it("should throw error when API key is missing", () => {
			// Act & Assert
			expect(() => new vercel_ai_gateway_1.VercelAiGatewayEmbedder("")).toThrow("validation.apiKeyRequired")
		})
	})
	describe("createEmbeddings", () => {
		beforeEach(() => {
			embedder = new vercel_ai_gateway_1.VercelAiGatewayEmbedder("test-api-key")
		})

...
describe("BedrockEmbedder", () => {
	...
	beforeEach(() => {
		...
		const MockedBedrockRuntimeClient = client_bedrock_runtime_1.BedrockRuntimeClient
		MockedBedrockRuntimeClient.mockImplementation(() => ({
			send: mockSend,
		}))
		embedder = new bedrock_1.BedrockEmbedder("us-east-1", "test-profile", "amazon.titan-embed-text-v2:0")
	})
	afterEach(() => {
		vitest.clearAllMocks()
	})
	describe("constructor", () => {
		...
		it("should require region", () => {
			expect(() => new bedrock_1.BedrockEmbedder("", "profile", "model")).toThrow(
				"Region is required for AWS Bedrock embedder",
			)
		})
		it("should use profile for credentials", () => {
			const profileEmbedder = new bedrock_1.BedrockEmbedder("us-west-2", "dev-profile")
	...
	describe("createEmbeddings", () => {
		...
		it("should handle Cohere model format", async () => {
			const cohereEmbedder = new bedrock_1.BedrockEmbedder("us-east-1", "test-profile", "cohere.embed-english-v3")
			const testTexts = ["Hello world"]
			const mockResponse = {
				body: new TextEncoder().encode(
					JSON.stringify({
		...
		it("should create embeddings with Nova multimodal model", async () => {
			const novaMultimodalEmbedder = new bedrock_1.BedrockEmbedder(
				"us-east-1",
				"test-profile",
		...
		describe("batching logic", () => {
			it("should warn and skip texts exceeding maximum token limit", async () => {
				...
				const oversizedText = "a".repeat(constants_1.MAX_ITEM_TOKENS * 4 + 100)
		...
		describe("retry logic", () => {
			...
			it("should retry on throttling errors with exponential backoff", async () => {
				...
				await vitest.advanceTimersByTimeAsync(constants_1.INITIAL_RETRY_DELAY_MS) // First retry delay
				await vitest.advanceTimersByTimeAsync(constants_1.INITIAL_RETRY_DELAY_MS * 2) // Second retry delay

...
describe("OpenAICompatibleEmbedder - Global Rate Limiting", () => {
	...
	beforeEach(() => {
		...
		const embedder = new openai_compatible_1.OpenAICompatibleEmbedder(testBaseUrl, testApiKey, testModelId)
		embedder.constructor.globalRateLimitState = {
			isRateLimited: false,
			rateLimitResetTime: 0,
			consecutiveRateLimitErrors: 0,
			lastRateLimitError: 0,
			mutex: embedder.constructor.globalRateLimitState.mutex,
		}
	})
	afterEach(() => {
		vi.useRealTimers()
	...
	it("should apply global rate limiting across multiple batch requests", async () => {
		const embedder1 = new openai_compatible_1.OpenAICompatibleEmbedder(testBaseUrl, testApiKey, testModelId)
		const embedder2 = new openai_compatible_1.OpenAICompatibleEmbedder(testBaseUrl, testApiKey, testModelId)
		...
		const state = embedder1.constructor.globalRateLimitState
	...
	it("should track consecutive rate limit errors", async () => {
		const embedder = new openai_compatible_1.OpenAICompatibleEmbedder(testBaseUrl, testApiKey, testModelId)
		const state = embedder.constructor.globalRateLimitState
		const rateLimitError = new Error("Rate limit exceeded")
		rateLimitError.status = 429
	...
	it("should reset consecutive error count after time passes", async () => {
		...
		await embedder.updateGlobalRateLimitState(rateLimitError)

...
exports.codeParser = exports.CodeParser = void 0
const promises_1 = require("fs/promises")
const crypto_1 = require("crypto")
const path = __importStar(require("path"))
const languageParser_1 = require("../../tree-sitter/languageParser")
const markdownParser_1 = require("../../tree-sitter/markdownParser")
...
class CodeParser {
	...
	isSupportedLanguage(extension) {
		return supported_extensions_1.scannerExtensions.includes(extension)
	}
	/**
	 ...
	 */
	async parseContent(filePath, content, fileHash) {
		...
		if ((0, supported_extensions_1.shouldUseFallbackChunking)(`.${ext}`)) {
		...
		if (!this.loadedParsers[ext]) {
			...
			if (pendingLoad) {
				...
				const loadPromise = (0, languageParser_1.loadRequiredLanguageParsers)([filePath])
		...
		if (captures.length === 0) {
			if (content.length >= constants_1.MIN_BLOCK_CHARS) {
		...
		const queue = Array.from(captures).map((capture) => capture.node)
		while (queue.length > 0) {
			...
			if (currentNode.text.length >= constants_1.MIN_BLOCK_CHARS) {
				// If it also exceeds the maximum character limit, try to break it down
				if (currentNode.text.length > constants_1.MAX_BLOCK_CHARS * constants_1.MAX_CHARS_TOLERANCE_FACTOR) {
					...
					const segmentHash = (0, crypto_1.createHash)("sha256")
	 ...
	 */
	_chunkTextByLines(lines, filePath, fileHash, chunkType, seenSegmentHashes, baseStartLine = 1) {
		const chunks = []
		let currentChunkLines = []
		let currentChunkLength = 0
		let chunkStartLineIndex = 0 // 0-based index within the `lines` array
		const effectiveMaxChars = constants_1.MAX_BLOCK_CHARS * constants_1.MAX_CHARS_TOLERANCE_FACTOR
		const finalizeChunk = (endLineIndex) => {
			if (currentChunkLength >= constants_1.MIN_BLOCK_CHARS && currentChunkLines.length > 0) {
		...
		for (let i = 0; i < lines.length; i++) {
			...
			if (lineLength > effectiveMaxChars) {
				...
				while (remainingLineContent.length > 0) {
					const segment = remainingLineContent.substring(0, constants_1.MAX_BLOCK_CHARS)
					remainingLineContent = remainingLineContent.substring(constants_1.MAX_BLOCK_CHARS)
					createSegmentBlock(segment, originalLineNumber, currentSegmentStartChar)
					currentSegmentStartChar += constants_1.MAX_BLOCK_CHARS
				}
			...
			if (currentChunkLength > 0 && currentChunkLength + lineLength > effectiveMaxChars) {
				...
				if (
					currentChunkLength >= constants_1.MIN_BLOCK_CHARS &&
					remainderLength < constants_1.MIN_CHUNK_REMAINDER_CHARS &&
					...
					for (let k = i - 2; k >= chunkStartLineIndex; k--) {
						...
						if (
							potentialChunkLength >= constants_1.MIN_BLOCK_CHARS &&
							potentialNextChunkLength >= constants_1.MIN_CHUNK_REMAINDER_CHARS
						) {
							splitIndex = k
							break
						}
	...
	_performFallbackChunking(filePath, content, fileHash, seenSegmentHashes) {
		const lines = content.split("\n")
		return this._chunkTextByLines(lines, filePath, fileHash, "fallback_chunk", seenSegmentHashes)
	}
	_chunkLeafNodeByLines(node, filePath, fileHash, seenSegmentHashes) {
	 ...
	 */
	processMarkdownSection(lines, filePath, fileHash, type, seenSegmentHashes, startLine, identifier = null) {
		const content = lines.join("\n")
		if (content.trim().length < constants_1.MIN_BLOCK_CHARS) {
			return []
		}
		// Check if content needs chunking (either total size or individual line size)
		const needsChunking =
			content.length > constants_1.MAX_BLOCK_CHARS * constants_1.MAX_CHARS_TOLERANCE_FACTOR ||
			lines.some((line) => line.length > constants_1.MAX_BLOCK_CHARS * constants_1.MAX_CHARS_TOLERANCE_FACTOR)
		if (needsChunking) {
	...
	parseMarkdownContent(filePath, content, fileHash, seenSegmentHashes) {
		const lines = content.split("\n")
		const markdownCaptures = (0, markdownParser_1.parseMarkdown)(content) || []
...
exports.codeParser = new CodeParser()

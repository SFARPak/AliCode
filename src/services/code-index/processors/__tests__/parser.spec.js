...
describe("CodeParser", () => {
	...
	beforeEach(() => {
		...
		languageParser_1.loadRequiredLanguageParsers.mockResolvedValue(mockLanguageParser)
		// Set up default fs.readFile mock return value
		vi.mocked(promises_1.readFile).mockResolvedValue("// default test content")
	})
	describe("parseFile", () => {
		...
		it("should use provided content instead of reading file when options.content is provided", async () => {
			...
			expect(vi.mocked(promises_1.readFile)).not.toHaveBeenCalled()
		...
		it("should read file when no content is provided", async () => {
			...
			vi.mocked(promises_1.readFile).mockReset()
			vi.mocked(promises_1.readFile).mockResolvedValue(testContent)
			const result = await parser.parseFile("test.js")
			expect(vi.mocked(promises_1.readFile)).toHaveBeenCalledWith("test.js", "utf8")
			expect(result.length).toBeGreaterThan(0)
		})
		it("should handle file read errors gracefully", async () => {
			// Reset the mock and set it to reject
			vi.mocked(promises_1.readFile).mockReset()
			vi.mocked(promises_1.readFile).mockRejectedValue(new Error("File not found"))
			const result = await parser.parseFile("test.js")
			expect(result).toEqual([])
		})
	...
	describe("parseContent", () => {
		it("should wait for pending parser loads", async () => {
			...
			parser["pendingLoads"].set(".js", pendingLoad)
			const result = await parser["parseContent"]("test.js", "const test = 123", "hash")
			expect(result).toBeDefined()
		})
		it("should handle parser load errors", async () => {
			languageParser_1.loadRequiredLanguageParsers.mockRejectedValue(new Error("Load failed"))
			const result = await parser["parseContent"]("test.js", "const test = 123", "hash")
			expect(result).toEqual([])
		})
		it("should return empty array when no parser is available", async () => {
			languageParser_1.loadRequiredLanguageParsers.mockResolvedValue({})
			const result = await parser["parseContent"]("test.js", "const test = 123", "hash")
			expect(result).toEqual([])
		})
	})
	describe("_performFallbackChunking", () => {
		...
		it("should respect 50-character minimum threshold for all languages", async () => {
			...
			const mockCapture = (content, startLine = 0) => ({
				node: {
					text: content,
	...
	describe("singleton instance", () => {
		it("should maintain parser state across calls", async () => {
			const result1 = await parser_1.codeParser.parseFile("test.js", { content: "const a = 1" })
			const result2 = await parser_1.codeParser.parseFile("test.js", { content: "const b = 2" })
...
Each section should have its own unique hash based on its content.`
			vi.mocked(markdownParser_1.parseMarkdown).mockReturnValue([
				{
...
Additional content to ensure we exceed the minimum block size requirements for proper indexing.`
			vi.mocked(markdownParser_1.parseMarkdown).mockReturnValue([])
			const result = await parser.parseFile("test.md", { content: markdownContent })
			expect(markdownParser_1.parseMarkdown).toHaveBeenCalledWith(markdownContent)
		...
		it("should chunk large markdown files with no headers", async () => {
			...
			expect(markdownParser_1.parseMarkdown).toHaveBeenCalledWith(largeMarkdownContent)

...
describe("DirectoryScanner", () => {
	...
	beforeEach(async () => {
		...
		scanner = new scanner_1.DirectoryScanner(
			mockEmbedder,
			mockVectorStore,
			mockCodeParser,
			mockCacheManager,
		...
		vi.mocked(promises_1.stat).mockResolvedValue(mockStats)
	...
	describe("scanDirectory", () => {
		it("should skip files larger than MAX_FILE_SIZE_BYTES", async () => {
			...
			vi.mocked(promises_1.stat).mockResolvedValueOnce(largeFileStats)
		...
		it("should parse changed files and return empty codeBlocks array", async () => {
			...
			const scannerNoEmbeddings = new scanner_1.DirectoryScanner(
				null, // No embedder
		...
		it("should delete points for removed files", async () => {
			mockCacheManager.getAllHashes.mockReturnValue({ "old/file.js": "old-hash" })
			await scanner.scanDirectory("/test")
		...
		it("should filter out files in hidden directories", async () => {
			...
			mockCodeParser.parseFile.mockImplementation((filePath) => {
		...
		it("should stop processing batches when signal is aborted mid-scan", async () => {
			...
			mockCodeParser.parseFile.mockImplementation(async () => {
				controller.abort()
				return mockBlocks
			})

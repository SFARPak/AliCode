...
describe("FileWatcher", () => {
	...
	beforeEach(() => {
		...
		fileWatcher = new file_watcher_1.FileWatcher(
			"/mock/workspace",
			mockContext,
			mockCacheManager,
			mockEmbedder,
	...
	describe("file filtering", () => {
		it("should ignore files in hidden directories on create events", async () => {
			...
			mockVectorStore.upsertPoints.mockImplementation(async (points) => {
		...
		it("should ignore files in hidden directories on delete events", async () => {
			...
			const deletedFiles = []
			mockVectorStore.deletePointsByFilePath.mockImplementation(async (filePath) => {

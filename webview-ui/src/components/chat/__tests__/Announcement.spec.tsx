...
vi.mock("react-i18next", () => ({
	Trans: ({ i18nKey, components }: { i18nKey: string; components?: Record<string, React.ReactElement> }) => {
		if (i18nKey === "chat:announcement.finalRelease.intro") {
			return (
				<span>
					This is the last Ali Code release.{" "}
		...
		if (i18nKey === "chat:announcement.finalRelease.alternatives") {
			return (
				<span>
					...
					{components?.clineLink && React.cloneElement(components.clineLink, {}, "Cline")} (where Ali Code
					originally started).
				</span>
			)
		}
	...
	useAppTranslation: () => ({
		t: (key: string, options?: { version?: string }) => {
			const translations: Record<string, string> = {
				"chat:announcement.finalRelease.title": "The last Ali Code release",
...
describe("Announcement", () => {
	it("renders the final release announcement", () => {
		...
		expect(screen.getByText("The last Ali Code release")).toBeInTheDocument()
		expect(screen.getByText(/This is the last Ali Code release/)).toBeInTheDocument()
		expect(
			screen.getByText(
				"This extension should continue to work indefinitely, but it won't receive bug fixes, new features, or model updates.",
			),

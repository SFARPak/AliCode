		...
		const unsub = vscode.onMessage((msg) => {
			...
			if (msg.type === "agentManager.worktreeSetup") {
				...
				if (ev.status === "ready" || ev.status === "error") {
					...
					if (ev.worktreeId)
						setBusyWorktrees((prev) => new Map([...prev].filter(([k]) => k !== ev.worktreeId)))
					setSetup({
						active: true,
						message: ev.message,
						branch: ev.branch,
					...
					if (ev.worktreeId) {
						setBusyWorktrees(
							(prev) =>
								new Map([
									...prev,
									[ev.worktreeId!, { reason: "setting-up", message: ev.message, branch: ev.branch }],
								]),
						)
			...
			if (msg.type === "agentManager.applyWorktreeDiffResult") {
				...
				if (ev.status === "conflict") {
					const summary =
						count > 0
							? t("agentManager.apply.conflictToast", { count, files: Math.max(files, 1) })
							: ev.message
						...
						<Show
							...
							<Show when={isGitRepo()}>
								{(() => {
									...
									const home = () =>
										new Map(sortedWorktrees().map((w) => [w.id, w.sectionId] as const))
									...
									return (
										<DragDropProvider
															...
															<div
																...
																<WorktreeItem
																	...
																	onMoveToSection={(secId) =>
																		moveToSection([wt.id], secId)
																	}
																	onMoveToNewSection={metrics.click(
																		"new_section",
																		"worktree_menu",
																		() => newSection(),
																	)}
																	...
																	onStartRename={(current) =>
																		startRename(wt.id, current)
																	}
											...
											<DragOverlay>
												{(() => {
													const wt = sortedWorktrees().find(
														(w) => w.id === draggingWorktree(),
													)
													if (!wt) return null
													return (
										...
										<ContextMenu>
											<ContextMenu.Trigger as="div" style={{ display: "contents" }}>
												<button
													...
													<span class="am-item-title">
														{s.title || t("agentManager.session.untitled")}
													</span>
											...
											<ContextMenu.Portal>
												<ContextMenu.Content class="am-ctx-menu">
													<ContextMenu.Item
														onSelect={() => handlePromote(s.id, new MouseEvent("click"))}>
														<Icon name="branch" size="small" />
													...
													<ContextMenu.Item
														onSelect={metrics.click(
															"open_session_locally",
															"unassigned_session_menu",
															() => openLocally(s.id),
														)}>
					...
					<DragDropProvider
						...
						<div class="am-tab-bar" onPointerLeave={releaseTabs}>
							...
							<div class="am-tab-scroll-area">
								<div
									class={`am-tab-fade am-tab-fade-left ${tabScroll.showLeft() ? "am-tab-fade-visible" : ""}`}
								/>
								...
								<div
									class={`am-tab-fade am-tab-fade-right ${tabScroll.showRight() ? "am-tab-fade-visible" : ""}`}
								/>
							</div>
							<Show when={selection() !== null}>
								<div class="am-tab-add-wrap">
									...
									{renderNewTabButton({
										...
										onNewTerminal: metrics.click("embedded_terminal", "new_tab_menu", () =>
											termHandlers.requestNew(),
										),
									})}
								</div>
							</Show>
										...
										<>
											...
											<Show when={sel()}>
												{(() => {
													...
													const active = () =>
														rs()?.state === "running" || rs()?.state === "stopping"
									...
									<IconButton
										...
										onClick={() => {
											...
											if (id)
												...
												vscode.postMessage({ type: "agentManager.showLocalTerminal" })
										}}
									/>
								</TooltipKeybind>
							</div>
								...
								<div class="am-setup-overlay">
									<div class="am-setup-card">
										...
										<div class="am-setup-title">
											{state().error
												? t("agentManager.setup.failed")
												: t("agentManager.setup.settingUp")}
										</div>
										...
										<div class="am-setup-status">
											...
											<span>
												{state().errorCode
													? t(`agentManager.setup.error.${state().errorCode}`)
													: state().message}
											</span>
										</div>
									</div>
				...
				<Show when={history()}>
					<HistoryView
						onSelectSession={(id) => {
							...
							const ms = worktreeSessionIds().has(id)
								? managedSessions().find((s) => s.id === id)
								: undefined
							if (ms?.worktreeId) {
									...
									<div class="am-diff-panel-wrapper">
										<Show when={sidePanel() === "diff"}>
											<DiffPanel
												...
												onClose={metrics.click("side_review_close", "side_review", () =>
													setSidePanel(null),
												)}
												onExpand={
													selection() !== null
						...
						<Show when={reviewOpen()}>
							<div
								...
								<FullScreenDiffView
									...
									revertingFiles={revertCtl.reverting()}
									activeTerminalId={terms.activeId()}
									onClose={metrics.click("fullscreen_review", "fullscreen_review", closeReviewTab, {
										action: "close",
									})}
								/>
							</div>

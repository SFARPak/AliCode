					...
					<Show when={props.diffs.length > 0}>
						<>
							<RadioGroup
								...
								label={(style) =>
									style === "unified"
										? t("ui.sessionReview.diffStyle.unified")
										: t("ui.sessionReview.diffStyle.split")
								}
								onSelect={(style) => {
									if (!style) return
			...
			<Show when={props.diffs.length > 0}>
				<div class="am-diff-content" data-component="session-review" ref={setScroller}>
					<Accordion
						multiple
						value={open()}
						onChange={(files) => setOpen(sanitizeOpenFiles(props.diffs, files))}>
						<VirtualDiffList
												...
												<div data-slot="session-review-trigger-content">
													...
													<div data-slot="session-review-trigger-actions">
														...
														<Show when={diff.kind === "image"}>
															<span class="am-diff-summary-pill">
																{t("agentManager.review.image")}
															</span>
														</Show>
														<Show when={isLargeCollapsed()}>
															<span class="am-diff-large-pill">
																{t("agentManager.review.largeFileCollapsed")}
															</span>
														</Show>
														...
														<Show when={props.onOpenFile && !isDeleted()}>
															<Tooltip
																value={t("agentManager.diff.openFile")}
																placement="top">
																<IconButton
																	icon="go-to-file"
																	size="small"
														...
														<Show when={props.onRevertFile}>
															<Tooltip
																value={t("agentManager.diff.revertFile")}
																placement="top">
																<IconButton
																	icon="discard"
																	size="small"
																	...
																	disabled={
																		props.revertingFiles?.has(diff.file) ?? false
																	}
																	onClick={(e: MouseEvent) => {
																		e.stopPropagation()
														...
														<Show
															when={
																isMarkdownFile(diff.file) &&
																props.onMarkdownRenderChange
															}>
															<Tooltip
																value={
													...
													fallback={
														<div class="am-diff-summary-state">
															<Show
																...
																fallback={<span>Diff preview loads on demand.</span>}>
																<>
																	<Spinner />
															...
															<Show
																...
																fallback={
																	<Diff<AnnotationMeta>
																		...
																		onGutterUtilityClick={(result) =>
																			handleGutterClick(diff.file, result)
																		}
																		onLineNumberClick={(event) => {
																			if (event.annotationSide === "deletions")
																				return
																			props.onOpenFile?.(
																				diff.file,
																				event.lineNumber,

			...
			<Popover
				...
				<div ref={root} class="am-sidebar-search" data-agent-manager-native-text-shortcuts>
					<List<SidebarSearchItem>
						...
						groupHeader={(group) =>
							group.category === "sessions" ? props.labels.sessions : props.labels.contexts
						}
							...
							return (
								<span
									...
									<span class="am-sidebar-search-icon">
										<Show when={!working} fallback={<Spinner class="am-sidebar-search-spinner" />}>
											<Show
												...
												<Icon
													name={item.kind === "worktree" ? "branch" : "speech-bubble"}
													size="small"
												/>
											</Show>
										</Show>
											...
											<Show when={item.section}>
												{(section) => (
													<span
														...
														style={{
															background:
																colorCss(section().color) ?? "var(--border-weak-base)",
														}}
													/>
												)}
											</Show>
									...
									<Show when={item.kind !== "session" && item.state === "idle"}>
										<span class="am-sidebar-search-count">

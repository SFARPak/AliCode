	...
	return (
		<Show when={kind()}>
			<span class="am-run-badge" data-run-state={kind()}>
				<Icon
					name={kind() === "success" ? "check-small" : kind() === "error" ? "warning" : "play"}
					size="small"
				/>
				<Show when={props.status?.state === "running" || props.status?.state === "stopping"}>
					<span class="am-run-badge-label">
						{props.status?.state === "stopping" ? "Stopping" : "Running"}
					</span>
				</Show>
			</span>
		</Show>
									...
									<div class="am-wt-row1">
										...
										<div class="am-wt-actions-cell">
											...
											<Show when={props.pendingDelete && !props.busy}>
												<span class="am-worktree-delete-hint">
										...
										<Show
											...
											fallback={
												<span
													...
													style={{
														background: colorCss(sec.color) ?? "var(--vscode-panel-border)",
													}}
												/>
											}>
											<Icon name="check" size="small" />

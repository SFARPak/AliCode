...
1. **Write logs to a temporary file instead of console**:
    ...
    - Create a log file at a known location, e.g., `/tmp/ali-cli-debug.log`
    - Use `fs.appendFileSync()` to write timestamped log entries
    - Example logging utility:

        ```typescript
        import fs from "fs"
        const DEBUG_LOG = "/tmp/ali-cli-debug.log"
...
2. **Clear the log file before each debugging session**:
    - Run `echo "" > /tmp/ali-cli-debug.log` or use `fs.writeFileSync(DEBUG_LOG, "")` at app startup during debugging
...
3. **Read the log file** after the user completes testing:
    - Run `cat /tmp/ali-cli-debug.log` to retrieve the captured output
...
7. **Repeat** until the alit cause is identified
